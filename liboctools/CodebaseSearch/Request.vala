...
namespace OLLMtools.CodebaseSearch
{
	...
	public class RequestCodebaseSearch : OLLMchat.Tool.RequestBase
	{
		...
		private OLLMfiles.ProjectManager project_manager {
			get { return (this.tool as OLLMtools.CodebaseSearch.CodebaseSearchTool).project_manager; }
		}
		...
		private OLLMvector.Database vector_db {
			get { return (this.tool as OLLMtools.CodebaseSearch.CodebaseSearchTool).vector_db; }
		}
		...
		protected override async string execute_request() throws Error
		{
			...
			var result_title = results.size == 1 ? "1 result returned for %s".printf(this.query) : "%d results returned for %s".printf(results.size, this.query);
			this.agent.add_message(new OLLMchat.Message("ui", 
