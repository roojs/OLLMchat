...
if ! grep -q 'ollmchat-android-bugs-v14' < <(strings "$apk_extract/lib/arm64-v8a/libgtk-4.so"); then
  echo "libgtk-4.so missing android-bugs patch tag (ollmchat-android-bugs-v14)." >&2
  echo "Pixiewood compile cache should have been discarded automatically; check pixiewood_prefix_has_patched_gtk." >&2
  exit 1
fi

if ! grep -q 'ollmchat-android-bugs-v14' "$apk_extract/assets/share/ollmchat-android-runtime.tag"; then
  echo "APK missing assets/share/ollmchat-android-runtime.tag (ollmchat-android-bugs-v14)." >&2
  exit 1
fi
