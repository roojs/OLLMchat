...
/**
 * Path B — run each {@link Task.Tool} in the execution queue, then merge when N > 1.
 * Copied from {@link Task.Details.run_exec}. Removed in plan 7.16.3 when {@link Iterate} ships.
 */
public class RunTools : Base
{
	public RunTools (Task.Details task)
	{
		base (task);
	}
