...
namespace OLLMchat.Settings
{
	...
	public class PullManager : Object
	{
		 ...
		 * 
		 * @param status PullStatus object with all status information (including model_name)
		 */
		 ...
		 */
		private PullManagerThread pull_thread;
		...
		/**
		 * Creates a new PullManager.
		 * 
		 * @param app ApplicationInterface instance (provides config and data_dir)
		 */
		public PullManager(OLLMchat.ApplicationInterface app)
		{
			Object(app: app);
		...
		/**
		 * Gets or creates a PullStatus object for a model.
		 * 
		 * @param model_name Model name
		 * @return PullStatus object (always non-null)
		 */
		...
		)
		{
			...
			if (status == "pulling") {
				...
				// Method checks internally if already initialized
		 ...
		 * 
		 * @param status_obj PullStatus object with current status (includes model_name)
		 * @param old_status Previous status (to detect changes)
		 */
		private void schedule_progress_update(PullStatus status_obj, string old_status)
		{
			var now = GLib.get_real_time() / 1000000;
