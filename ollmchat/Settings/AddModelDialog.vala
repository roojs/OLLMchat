...
namespace OLLMchat.Settings
{
	...
	public class AddModelDialog : Adw.PreferencesDialog
	{
		...
		private AvailableModel? selected_model = null;
		 ...
		 */
		public AddModelDialog(SettingsDialog settings_dialog)
		{
		...
		this.connection_dropdown = new Gtk.DropDown(this.connection_list, connection_factory) {
			...
			this.size_list_store = new GLib.ListStore(typeof(ModelTag));
			this.size_dropdown = new Gtk.DropDown(this.size_list_store, null) {
				vexpand = false,
				valign = Gtk.Align.CENTER
			};
			
			// Create factory for rendering ModelTag objects
			...
			size_factory.bind.connect((item) => {
				...
				var tag = list_item.item as ModelTag;
		...
		private void setup_model_chain()
		{
			...
			this.model_filter = new Gtk.StringFilter(
				new Gtk.PropertyExpression(typeof(AvailableModel), null, "name")
			) {
			...
			this.model_sorter = new Gtk.CustomSorter((a, b) => {
				...
				if (search_text == "") {
					var name_a = (a as AvailableModel).name.down();
					var name_b = (b as AvailableModel).name.down();
					if (name_a < name_b) {
			...
			this.model_pulldown.item_selected.connect((position) => {
				...
				var model_from_sorted = this.sorted_models.get_item(position) as AvailableModel;
				...
				var model_from_selection = (selection_pos != Gtk.INVALID_LIST_POSITION) ? 
					this.selection.get_item(selection_pos) as AvailableModel : null;
		...
		private void update_size_dropdown()
		{
			...
			this.selected_model = this.selection.get_item(selected_pos) as AvailableModel;
			if (this.selected_model == null) {
			...
			var sorted_tags = new Gee.ArrayList<ModelTag>();
			...
			// Populate size list with sorted ModelTag objects and find default index
