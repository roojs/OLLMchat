 ...
 *
 * Android compiles this file instead of ''namespace.vala'' (meson, not
 * ''#if''). {@link to_value} takes a va_list pointer so a multi-letter
 * {@link args} call advances the cursor on aarch64.
 *
...
namespace OLLMrpc
{
	 ...
	 *
	 * {@link val} and {@link args} call this once per letter. The cursor
	 * is a pointer: aarch64 passes va_list by value, so a by-value
	 * parameter would leave the caller on the first argument.
	 *
	 * @param tag one complete type (''s'', ''o'', ''as'', …)
	 * @param cursor varargs cursor shared with the caller
	 * @return boxed value
	 */
	private GLib.Value to_value(string tag, va_list* cursor)
	{
		switch (tag) {
			case "s":
				var s_val = GLib.Value(typeof(string));
				s_val.set_string((*cursor).arg<string>());
				return s_val;

			case "b":
				var b_val = GLib.Value(typeof(bool));
				b_val.set_boolean((*cursor).arg<bool>());
				return b_val;

			case "y":
				var y_val = GLib.Value(typeof(uint8));
				y_val.set_uchar((uint8) (*cursor).arg<int>());
				return y_val;

			case "n":
				var n_val = GLib.Value(typeof(int));
				n_val.set_int((*cursor).arg<int>());
				return n_val;

			case "q":
				var q_val = GLib.Value(typeof(uint));
				q_val.set_uint((uint) (*cursor).arg<int>());
				return q_val;

			case "i":
				var i_val = GLib.Value(typeof(int));
				i_val.set_int((*cursor).arg<int>());
				return i_val;

			case "u":
				var u_val = GLib.Value(typeof(uint));
				u_val.set_uint((*cursor).arg<uint>());
				return u_val;

			case "x":
				var x_val = GLib.Value(typeof(int64));
				x_val.set_int64((*cursor).arg<int64>());
				return x_val;

			case "t":
				var t_val = GLib.Value(typeof(uint64));
				t_val.set_uint64((*cursor).arg<uint64>());
				return t_val;

			case "f":
				var f_val = GLib.Value(typeof(float));
				f_val.set_float((float) (*cursor).arg<double>());
				return f_val;

			case "d":
				var d_val = GLib.Value(typeof(double));
				d_val.set_double((*cursor).arg<double>());
				return d_val;

			case "o":
				var obj = (*cursor).arg<GLib.Object>();
				if (obj == null) {
			...
			case "g":
				...
				g_val.set_string((*cursor).arg<string>());
				return g_val;

			case "h":
				var h_val = GLib.Value(typeof(int));
				h_val.set_int((*cursor).arg<int>());
				return h_val;

			case "S":
			case "as":
				var as_val = GLib.Value(typeof(string[]));
				as_val.set_boxed((*cursor).arg<string[]>());
				return as_val;

			case "ay":
				var ay_val = GLib.Value(typeof(GLib.Bytes));
				ay_val.set_boxed((*cursor).arg<GLib.Bytes>());
				return ay_val;

			case "v":
				var v_val = GLib.Value(typeof(GLib.Variant));
				v_val.set_variant((*cursor).arg<GLib.Variant>());
				return v_val;

			case "V":
				var held = (*cursor).arg<GLib.Value>();
