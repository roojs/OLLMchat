	...
	public interface Serializable : GLib.Object
	{
		...
		public virtual void bin_read (Stream ctx) throws GLib.Error
		{
			...
			while ((t = ctx.read_tag (out prop_name)) != Stream.TOKEN_END) {
				while (ctx.try_consume_name_intro ()) {
				}
		...
		) throws GLib.Error
		{
			...
			switch ((GLib.Type) base_type) {
				...
				case GLib.Type.BOOLEAN:
					...
					val.set_boolean (ctx.read_byte () == 1);
					this.set_property (prop.name, val);
					return;
				...
				case GLib.Type.CHAR:
					...
					val.set_schar ((int8) ctx.read_byte ());
					this.set_property (prop.name, val);
					return;
				...
				case GLib.Type.UCHAR:
					...
					val.set_uchar ((uchar) ctx.read_byte ());
					this.set_property (prop.name, val);
					return;
				...
				case GLib.Type.ENUM:
					...
					width = ctx.read_byte ();
					if (width == 1) {
						val.set_enum ((int) (int8) ctx.read_byte ());
						this.set_property (prop.name, val);
						return;
					}
					if (width != 8) {
				...
				case GLib.Type.FLAGS:
					...
					width = ctx.read_byte ();
					if (width == 1) {
						val.set_flags ((uint) ctx.read_byte ());
						this.set_property (prop.name, val);
						return;
					}
					if (width != 8) {
				...
				case GLib.Type.INT:
					...
					width = ctx.read_byte ();
					if (width == 1) {
						val.set_int ((int) (int8) ctx.read_byte ());
						this.set_property (prop.name, val);
						return;
					}
					if (width != 8) {
				...
				case GLib.Type.INT64:
					...
					width = ctx.read_byte ();
					if (width == 1) {
						val.set_int64 ((int64) (int8) ctx.read_byte ());
						this.set_property (prop.name, val);
						return;
					}
					if (width != 8) {
				...
				case GLib.Type.UINT:
					...
					width = ctx.read_byte ();
					if (width == 1) {
						val.set_uint ((uint) ctx.read_byte ());
						this.set_property (prop.name, val);
						return;
					}
					if (width != 8) {
				...
				case GLib.Type.UINT64:
					...
					width = ctx.read_byte ();
					if (width == 1) {
						val.set_uint64 ((uint64) ctx.read_byte ());
						this.set_property (prop.name, val);
						return;
					}
					if (width != 8) {
