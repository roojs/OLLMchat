...
**Status:** ✅ FIXED — user archived 2026-09-17 (`/usr/include/ocrpc.h`; gnome-shell-rpc `-Docrpc_libdir=` builds; `value-v-gate` PASS)  
...
### 1. `libocrpc/meson.build` — `library('ocrpc', …)`: install generated `ocrpc.h` to includedir

**Why:** `install: true` alone only installs the `.so`; the generated `ocrpc.h` (from `vala_header:`) needs a matching `install_dir` slot so Meson's ninja backend maps `install_dir[1] is True` → `get_includedir()`. Slot 2 is `false` to skip `ocrpc-meson.vapi` (consumers use the `custom_target` `ocrpc.vapi`).
**Where:** `ocrpc_base_lib = library('ocrpc', …)` block — the `install: true,` line immediately before the closing `)`.
**Depends on:** none.
...
```meson
  # Meson Vala: install:true alone installs only the .so. Secondary outputs
  # (header, vapi) need matching install_dir slots — true = default path,
  # false = skip (ocrpc.vapi comes from custom_target below).
  install: true,
...
- ✅ 2026-09-16 — system reinstall verified from gnome-shell-rpc:
  `/usr/include/ocrpc.h` present; `-Docrpc_libdir=` full rebuild OK;
  links `/lib/x86_64-linux-gnu/libocrpc.so`; `value-v-gate` PASS.

---

## Next

ℹ️ Optional follow-up (not this bug): same `install_dir` hole for other `liboc*` installs.
