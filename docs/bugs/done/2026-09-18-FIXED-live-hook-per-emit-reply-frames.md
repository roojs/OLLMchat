...
**Status:** ✔️ applied in `libocrpc` 2026-09-18 — per-emit frames + `Callback.reply` → `complete`
...
- ✔️ Applied centrally 2026-09-18: `libocrpc` compiles; `meson test -C build test-rpc-callback` **OK**.
...
💩 This supersedes the `virtual` on `emit` (§3 of the archived report) as far as consumers are concerned; keeping or reverting the keyword is your call, the consumer override is deleted either way. **Applied with `virtual` kept** (same as the proven override).
...
- ✔️ Applied §1–§2 in `libocrpc` 2026-09-18. `emit` stays `virtual`. `complete` docblock uses `''…''` instead of `{@code}` (valadoc rejects `{@code}`).

---

## Next

- ✔️ 🔷 §1–§2 applied 2026-09-18.
...
- ℹ️ Consumer regression (`same-hook-reentrant-emit-gate`, storm rejected replies = 0) lives in gnome-shell-rpc; not re-run from this tree.
