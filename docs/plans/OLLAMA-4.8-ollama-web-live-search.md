...
**Status:** ✅ **Phase 1 complete** — `libollamaweb` + offline `ollamaweb` tests; ✅ **Phase 2.1** (**4.8.1**); **Phase 2.2** not started; **Phase 2.3** proposed (**4.8.2** local pull from catalog)

**Layout:** `docs/guide-to-writing-plans.md`

**Coding standards:** `docs/coding-standards.md` — implementers must satisfy Vala rules there. Verify against **`docs/guide-to-writing-plans.md`** — **Checklist for plans** before marking implementation complete.
...
**ℹ️ Implementation spec:** [`docs/plans/OLLAMA-4.8.1-ollama-web-phase-2-1-add-model.md`](OLLAMA-4.8.1-ollama-web-phase-2-1-add-model.md)
...
### Phase 2.3 — local pull from catalog data (**4.8.2**)

**ℹ️** Spec: [`docs/plans/OLLAMA-4.8.2-local-pull-from-catalog.md`](OLLAMA-4.8.2-local-pull-from-catalog.md)

- **🔷** Keep ollama.com search + tags refine as the catalog source (no registry API replacement).
- **🔷** Build **`/api/pull`** reference from **`OllamaWeb.Model.slug`** + selected **`ModelVariant.name`**.
- **🔷** Guard Start Download until refine provides local-pullable tags; exclude cloud-only variants.
- **ℹ️** Reuses existing **`PullManager`** / **`Call.Pull`** (**1.3.4**); Ollama daemon + **`registry.ollama.ai`** handle on-disk weights.
