...
⏳ **IN PROGRESS** - Phase 1 complete, Phase 2 in progress (updating references).
...
**Phase 2: ⏳ IN PROGRESS**
- New files exist and use new namespaces (`OLLMchat.Agent.*`, `OLLMcoder.AgentFactory`, `OLLMcoder.Agent`)
- Old files still exist and are still in build system
- Many references still use old namespaces:
  - `OLLMchat.Prompt.BaseAgent` → needs `OLLMchat.Agent.Factory`
  - `OLLMchat.Prompt.AgentHandler` → needs `OLLMchat.Agent.Base`
  - `create_handler()` → needs `create_agent()`
- Key files needing updates: Manager.vala, Session.vala, SessionBase.vala, Call/Chat.vala, Tool/RequestBase.vala, ollmapp/Window.vala

**Phase 3: ⏳ NOT STARTED**
- Old files still exist and need to be deleted after Phase 2 is complete
...
1. ⏳ Update all imports from `OLLMchat.Prompt.*` to `OLLMchat.Agent.*`
   - ⚠️ Still using `OLLMchat.Prompt.BaseAgent` in: Manager.vala, SessionBase.vala, Session.vala, ollmapp/Window.vala
   - ⚠️ Still using `OLLMchat.Prompt.AgentHandler` in: SessionBase.vala, Session.vala, Call/Chat.vala, Tool/RequestBase.vala
2. ⏳ Update all imports from `OLLMcoder.Prompt.*` to `OLLMcoder.*`
   - ✅ New files (AgentFactory.vala, Agent.vala) use new namespace
   - ⚠️ Old files still exist in Prompt/ directory
3. ⏳ Replace `BaseAgent` → `Factory` throughout codebase
   - ⚠️ Still referenced in: Manager.vala, SessionBase.vala, Session.vala, ollmapp/Window.vala
4. ⏳ Replace `AgentHandler` → `Base` throughout codebase
   - ⚠️ Still referenced in: SessionBase.vala, Session.vala, Call/Chat.vala, Tool/RequestBase.vala
5. ⏳ Replace `JustAsk` (factory) → `JustAskFactory` throughout codebase
6. ⏳ Replace `create_handler()` → `create_agent()` throughout codebase
   - ✅ New files use `create_agent()`
   - ⚠️ Still using `create_handler()` in: Session.vala, Prompt/BaseAgent.vala, Prompt/JustAsk.vala
7. ⏳ Replace `agent` property → `factory` property in Base class usage
   - ✅ Base.vala uses `factory` property
   - ⚠️ Still using `agent` property in old files and references
8. ⏳ Replace `CodeAssistant` (factory) → `AgentFactory` in liboccoder
   - ✅ New file uses `AgentFactory`
   - ⚠️ Old file still exists
9. ⏳ Replace `CodeAssistantHandler` → `Agent` in liboccoder
   - ✅ New file uses `Agent`
   - ⚠️ Old file still exists
10. ⏳ Remove old files from meson.build:
    - ⚠️ Still in build: `libollmchat/Prompt/BaseAgent.vala`
    - ⚠️ Still in build: `libollmchat/Prompt/AgentHandler.vala`
    - ⚠️ Still in build: `libollmchat/Prompt/JustAsk.vala`
    - ⚠️ Still in build: `liboccoder/Prompt/CodeAssistant.vala`
    - ⚠️ Still in build: `liboccoder/Prompt/CodeAssistantHandler.vala`
    - ⚠️ Still in build: `liboccoder/Prompt/CodeAssistantProvider.vala`
...
- [ ] Old files deleted
- [ ] Build succeeds
- [ ] Final functionality test passes
