...
- **🔷** `⏳` TLS on `Soup.Server` using a **private** server identity (our apps trust each other).
- **🔷** `⏳` **`Transport.Cert.ensure(dir)`** creates or loads that identity — no hand PEM setup, no Let’s Encrypt, no public CA.
- **🔷** `⏳` Same RPC paths over **`https://`**.
- **🔷** `⏳` Plain HTTP when `tls_certificate` is null (existing smokes unchanged).
- **ℹ️** Phase 4 = encrypted transport + private trust. Not mTLS client certs (Phase 6), not RPC login (Phase 5).

---

## Trust model

- **🔷** Trust is **between our applications**, not the public Web PKI.
- **🔷** On disk under `dir`: `server.pem` + `server-key.pem` (self-signed leaf).
- **🔷** `trust_pem_path` = `server.pem` (clients build `TlsFileDatabase` from it).
- **🔷** Generation uses **GnuTLS library** APIs (`gnutls_x509_privkey_generate` / crt sign / PEM export) once when files are missing — no subprocess.
- **🚫** Let’s Encrypt / ACME / public CA.
- **🚫** Shelling out to `openssl` (or any CLI) to mint keys.
- **🚫** Operators creating PEMs by hand for normal use.
...
- **🔷** `OLLMrpc.Transport.Cert.ensure(string dir)` → `Cert` (throws `GLib.Error`).
- **🔷** `Cert.certificate` — `GLib.TlsCertificate` for `HttpServer.tls_certificate`.
- **🔷** `Cert.trust_pem_path` — PEM path for the client trust DB.
- **🔷** `Cert.dir` — directory passed to `ensure`.
- **🔷** `HttpServer.tls_certificate` — non-null → HTTPS listen.
- **🔷** Existing `start` / `stop` / `port` / `rpc_path`.

### `HttpServer`

- **🔷** App: `var c = Cert.ensure(tls_dir); http.tls_certificate = c.certificate; http.start();`
...
- **🚫** Extra public methods on `Cert` beyond `ensure`.

---

## Suggested implement order

1. **Phase 0** — `Transport/Cert.vala` + meson.
2. **Phase 1** — `HttpServer.tls_certificate` + HTTPS listen.
3. **Phase 2** — smoke with **8.2.3.6**.

---

## Phase 0 — `Transport.Cert` — **⏳**

Edits are **Remove** / **Replace with** / **Add** from the tree; verify surrounding context before applying.

### 1. `libocrpc/Transport/Cert.vala` — new file

**Why:** Load-or-generate private server identity for HTTPS RPC.
...
{
	/**
	 * Private TLS identity for HTTPS RPC (app↔app trust, not public CA).
	 *
	 * Files under {@link dir}: ''server.pem'' (cert) and ''server-key.pem''
	 * (key). {@link trust_pem_path} is ''server.pem'' for client
	 * {@link GLib.TlsFileDatabase}.
	 *
	 * == Example ==
	 *
	 * {{{
	...
	public class Cert : GLib.Object
	{
		...
		/** Path to the PEM clients trust (same as server cert for self-sign). */
		public string trust_pem_path { get; private set; default = ""; }

		private Cert(string dir)
		{
			GLib.Object(dir: dir);
		}

		/**
		 * Load existing PEMs under {@link dir}, or generate a self-signed
		 * RSA pair via GnuTLS and load them.
		 *
		 ...
		 */
		public static Cert ensure(string dir) throws GLib.Error
		{
			var self = new Cert(dir);
			...
			self.certificate = new GLib.TlsCertificate.from_files(
				cert_path, key_path);
			self.trust_pem_path = cert_path;
			return self;
		}
	}
}
...
### 2. `libocrpc/meson.build` — source + GnuTLS dep
...
```vala
		 ...
		 *
		 * Typical: {@link Cert.ensure} then assign {@link Cert.certificate}.
		 */
...
Shared executable with **8.2.3.6** — full test body lives in that plan’s Phase 1 (uses `Cert.ensure` + `HttpClient.tls_database`). Meson target name: `test-rpc-http-https`.
...
- **🔷** `⏳` Phase 6 — issue client certs (extend `Cert`).
...
- **ℹ️** GLib/`GTlsCertificate` only **loads** PEMs — generation is GnuTLS (`gnutls` pkg + `--pkg gnutls`).
- **🚫** Do not spawn `openssl` / any CLI for keygen.
