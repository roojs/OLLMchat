...
**Implementation**: ocvector uses `Settings.ModelUsage` class for its configuration. ocvector must register the GType for ModelUsage with Config2 using `register_type("ocvector", typeof(Settings.ModelUsage))`. The configuration is stored in Config2's `usage` map.
