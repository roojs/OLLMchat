...
**Suggested Location**: `libollmchat/Settings/ModelUsage.vala`
...
**Status**: ✅ **EXISTS** - Copy of Config.vala with `toV2()` method added
...
- `external_configs` (Gee.Map<string, Object>) - Map of section name → external config objects (handled by ConfigHandler interfaces)
...
### 9. External Configuration Sections

**Purpose**: Allow external libraries (like ocvector) to have configuration sections in Config2 without creating dependencies. Uses GType registration for deserialization of any property.

**Status**: ⏳ **NEW** - Not yet implemented

**Usage Pattern**:
- Libraries register key/GType pairs for any property/section with Config2
- Config2 stores external config sections as `Gee.Map<string, Object>` (section name → deserialized object)
...
**Note**: This allows Config2 to reference external configurations without creating dependencies. Libraries register key/GType pairs, and standard GObject JSON serialization handles the rest. Can be used for any property, not just external configs.
...
**Implementation**: ocvector uses `Settings.ModelUsage` class for its configuration. ocvector must register the GType for ModelUsage with Config2 using `register_type("ocvector", typeof(Settings.ModelUsage))`.
...
**Note**: The `ocvector` section (and other external configs) are handled by registered ConfigHandler interfaces. The actual storage may be elsewhere, but references are maintained inDuring JSON deserialization, Config2 uses registered GType to deserialize via standard Json.Serializable
 Config2.
...
**Created** (not yet in build system):
...
- `Config2` is the main serializable holder that contains:
  ...
  - `external_configs` map: `Gee.Map<string, Object>` (section name → external config objects)
...
- External libraries (like ocvector) register ConfigHandler implementations to handle their config sections
...
- ocvector settings (and other external configs) are stored elsewhere but referenced in Config2 via key/GType registration
