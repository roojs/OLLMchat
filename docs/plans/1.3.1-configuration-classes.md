...
### 6. Config1 (Version 1 Configuration)
...
**Status**: ✅ **EXISTS** - Copy of Config.vala

**Location**: `libollmchat/Settings/Config1.vala`

**Note**: This is a copy of the original `Config.vala` class, moved to Settings namespace. Same code as Config, but in Settings namespace. Used for reading old Config format and converting to Config2. **Note**: In phase 1, these files are not added to the build system - will be reviewed after implementation.
...
- **Settings/** subdirectory: All ollmchat configuration classes go here (Connection, ChatDefaults, Config1, ModelOptions if needed)
...
- All ollmchat configuration classes go in `Settings/` subdirectory (Connection, ChatDefaults, Config1, ModelOptions if needed)
