...
ℹ️ Parent: [6.1](6.1-agent-pi.md). Inject today: [done/6.1.2](done/6.1.2-DONE-pi-like-agent.md) Phase 1.  
ℹ️ Banner offer: [6.1.9](6.1.9-offer-agents-md.md). Default offer list: [6.1.10](6.1.10-skills-configuration.md).  
ℹ️ Indexed summary: [done/6.1.7](done/6.1.7-DONE-project-summary-tool.md) (`codebase_search` `element_type = "project"`).  
ℹ️ Wiki skill (separate plan): [5.5](5.5-wiki-maker.md).
...
- 🔷 **`docs/wiki/`** — LLM memory. Build/read is the **wiki skill** in [5.5](5.5-wiki-maker.md), not this plan.
...
- 🚫 Do **not** write a packaged stub body and call it `AGENTS.md` ([6.1.9](6.1.9-offer-agents-md.md) stub is out).
...
- 💩 Banner Create in [6.1.9](6.1.9-offer-agents-md.md) starts this skill path (user message / agent turn), not a template write.
...
- 💩 CLAUDE-only at project root: inject already works; do not hint. Creating `AGENTS.md` beside it stays the [6.1.9](6.1.9-offer-agents-md.md) edge case.
...
[5.5](5.5-wiki-maker.md) is the wiki **skill** (build / read under `docs/wiki/`). This plan only locks what **`AGENTS.md` tells agents** about it.
...
- ℹ️ [5.5](5.5-wiki-maker.md) already wanted wiki as first port of call for coding prep. `AGENTS.md` is how Agent Pi **always** sees that rule.
...
- 🔷 `⏳` Default **offered** (add to [6.1.10](6.1.10-skills-configuration.md) seed). Critical enough to be in the catalog.
...
- ℹ️ Wiki skill ([5.5](5.5-wiki-maker.md)) is also a procedure. Wiki **pages** are memory, not always-on.
...
### 6. [6.1.10](6.1.10-skills-configuration.md) — default offered list
...
4. 🔷 `⏳` Default offer seed ([10](6.1.10-skills-configuration.md) + Factory).
5. 💩 `⏳` `codebase_search` description line.
6. 🔷 `⏳` Point [6.1.9](6.1.9-offer-agents-md.md) Create at this skill (no stub).
7. ℹ️ Wiki build/read remains [5.5](5.5-wiki-maker.md).
...
- 🚫 Do not implement [5.5](5.5-wiki-maker.md) here.
