...
4. **Task execution** — given a fully specified task (query + reference files only; no tool use inside the agent), produce a **result summary** and optionally one details file (markdown or code). All tool calls for the task are run by the orchestrator before the executor (there may be **multiple** tool runs); the executor receives the output from all of them and interprets it.

Design principles:

- **Skills and tools:** A task may declare a **skill**, which **may use more than one tool** (not limited to one). **Task refinement** is responsible for generating **all** tool calls for that task (which may be multiple). The orchestrator runs each of those tools, turns the results into reference content, then passes (query, reference_files including all tool outputs) to the executor. Executors never call tools themselves; they receive the output from the tool run(s) and interpret it; they only read files and produce one result summary plus optionally one details file.
...
- **Prompts** for task creation, refinement, post-completion, and task execution (interpreter) live under **`resources/skill-prompts`** (markdown prompt templates): `task_creation_initial.md`, `task_refinement.md`, `task_post_completion.md`, `task_execution.md`. These are not full “skill files”; they drive the planner, refiner, post-completion, and interpreter stages.
- **Skills** (executors only) live under **`resources/skills`**; skill files are YAML frontmatter (name, description) + markdown body. The **skill catalog** injected into the initial prompt is assembled from these files' frontmatter (name + description). Refinement gets **skill input requirements** from the skill file (e.g. a dedicated section); the interpreter gets the full **skill definition** (file content) for summation. Only the **executor** (interpreter) phase uses the actual executable skills; task creation, refinement, and post-completion are prompt-driven.
...
| **Task execution** | Interpret tool outputs; produce summary + optional details | Query + reference files (may include output from **multiple** tool runs) | Result summary (required) + optionally one file (e.g. `.md`, `.jsx`) |
...
- **Task execution** is “executor” behaviour (output = result summary + optionally one details file). **Executors are the only real skills**; a skill may use **one or more tools** (e.g. code search, web search, shell). The orchestrator runs **all** tool calls for the task (as generated by refinement), attaches their outputs as reference content, then invokes the executor to interpret those outputs.
...
- **Executors** — these are the **only real skills**. They live in **`resources/skills`** as YAML frontmatter + markdown body. A skill **may use more than one tool** (e.g. multiple code searches, mixed tools). The orchestrator runs **all** tool calls for the task (refinement generates them), attaches the results as reference content, then invokes the executor. The executor receives (query, reference_files including output from all tool runs) and produces a result summary plus optionally one details file.

So:

- **Tool calls** (e.g. semantic search, read_file) are generated by **refinement** for the task; the orchestrator runs each of them and their results become reference content. The **executor** runs after and interprets the combined output from all of those tool runs; it produces a result summary and optionally one details file.
...
| **1.23.6** | [Task execution prompt](1.23.6-DONE-task-execution-prompt.md) | **Interpreter prompt:** what we tell the interpreter (query + skill definition + precursor; may receive output from **multiple** tool calls), output format (result summary + optional details file). Prompt file: **`resources/skill-prompts/task_execution.md`**. Runner injects {query}, {skill_definition}, {precursor}. |
| **1.23.11** | [Runner implementation](1.23.11-runner-implementation.md) | **Single plan** for all Runner work: parsing (1.23.3–1.23.5), link resolution, precursor injection, refinement (Phase 3), executor, continuation, multi-round and error handling (Phase 4). Former 1.23.8, 1.23.9, 1.23.10 merged here. Runner passes **code blocks** (tool call content from refinement) to tool execution. |
| **1.23.12** | [Prompt refinements from design](1.23.12-prompt-refinements-from-design.md) | **Go back and change prompts** (and prompt files in `skill-prompts`) based on the modified design: multiple tools, refinement outputs all tool calls in code blocks, executor receives multiple tool outputs, context restriction/relevance. All detailed prompt-change requirements live here; Runner behaviour (including passing code blocks) stays in 1.23.11. |

Work order: **Prompts first** (1.23.3–1.23.6: create each prompt and the output format it describes; can be done in parallel), then **1.23.11** (Runner implementation — single plan; Phases 2–4: single-path, refinement, multi-round). **1.23.12** (prompt refinements from design): go back and update prompts and prompt files to match the modified design (tool call code blocks, multiple tools, context restriction); detail lives in 1.23.12. Each sub-plan contains “to expand” sections for full prompt text and behaviour before implementation.
...
### Phase 1 — Prompt types and markdown contracts (no new code) → **1.23.3, 1.23.4, 1.23.5, 1.23.6**
...
- **1.1** Define the **markdown schema** for the three prompt types:
  ...
  - **Task execution output:** Result summary (required) + optionally one artifact (one file: markdown or code); no task list. Executor may receive output from **multiple** tool runs.
...
- **1.3** Add **prompts** in **`resources/skill-prompts`** (no full skill files for these):
  ...
  - **1.4** Keep **executor skills** in **`resources/skills`** only (e.g. `codebase_research.md`, `plan_create.md`): a skill **may use more than one tool**; refinement generates all tool calls for the task; query + refs (including all tool outputs) → result summary + optional details file.

Deliverable: Prompts created (1.23.3–1.23.6: one plan per prompt); Runner unchanged. **Expand in 1.23.3–1.23.6.**
...
- **Executors:** Real skills in `resources/skills`; a skill **may use multiple tools**. Refinement generates all tool calls for the task; Runner runs them, then executor gets (query, reference_files including all tool outputs) and outputs result summary + optional details file.
- **Markdown I/O:** All inputs and outputs follow our existing task list and Result summary / Tasks / Edit conventions.
- **Phase flow:** Schemas (1.23.3–1.23.6) and skill-prompts (Conventions above) → Runner implementation (1.23.11, Phases 2–4). **1.23.12**: prompt refinements from design (update prompts to match code blocks, multiple tools, context restriction).
