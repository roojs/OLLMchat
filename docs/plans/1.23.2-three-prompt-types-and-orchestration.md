...
- **Planning involves no execution:** Task creation (initial) **does not execute anything**. It only uses gathered information (user goal, optional reference files) to **devise a plan** (the task list). The only state it produces is the **plan** itself. The continuation step is the **task iterator**: it receives the completed task list + executor outputs and adjusts the plan (continue with new tasks or conclude).
...
| **1.23.5** | [Task creation (continuation) prompt](1.23.5-task-creation-continuation-prompt.md) | **Create the prompt:** what we tell the model, output (Decision: continue/conclude, optional summary, optional new coarse tasks). Prompt file in `skill-prompts`. |
