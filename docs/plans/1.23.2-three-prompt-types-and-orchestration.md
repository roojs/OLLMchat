...
2. **Task creation (continuation)** — given a **completed** task list and executor outputs, interpret what was done and decide: **continue** (add more coarse tasks) or **conclude** the flow. Uses a **different prompt** from the initial one.
...
| **Task creation (continuation)** | Interpret results, decide next step | **Completed** task list + executor outputs | **Decision:** continue (and new coarse tasks) or **conclude**. Optionally a short summary. |
...
- **Task creation** has **two prompts**: (1) **initial** — goal → coarse task list; (2) **continuation** — completed tasks + results → interpret and decide continue (emit new tasks) or conclude. The continuation prompt is different because it starts from a completed task list and must interpret it and choose whether to add more work or finish.
...
- **Back to task creation:** Executor outputs are **passed back** to a **task creation (continuation)** step. This step uses a **different prompt** from the initial task creation: its input is the **completed** task list and the executor results, not the raw user goal. It must **interpret** what was done and what was produced, then decide: **continue** (emit more coarse tasks for the next round) or **conclude** (flow done). So the loop is: creation (initial) → refinement (if needed) → executors → **task creation (continuation)** with completed list + results → if continue, refinement → executors → … ; if conclude, stop.
...
- **`resources/skill-prompts`:** Prompt content for:
  ...
  - **Task creation (continuation):** e.g. `task_creation_continue.md` — completed task list + executor outputs → interpret and decide: **continue** (new coarse tasks) or **conclude**. Different prompt because the input is “what we did and what we got”, not the original goal.
...
## 6. Phase flow: developing skills, prompts, and code

Phases are ordered so we can test with minimal orchestration first, then add refinement and full flow.

### Phase 1 — Prompt types and markdown contracts (no new code)
...
- **1.3** Add **prompts** in **`resources/skill-prompts`** (no full skill files for these):
  - **Task creation (initial)** prompt (e.g. `task_creation_initial.md`): goal → coarse task list with optional "skill needed".
  - **Task creation (continuation)** prompt (e.g. `task_creation_continue.md`): completed task list + executor outputs → interpret and decide continue or conclude; different prompt because input is completed tasks + results.
...
Deliverable: Markdown specs, `skill-prompts` for creation/refinement, executor skills in `skills`; Runner (orchestrator) unchanged in this phase.

### Phase 2 — Single-path execution (Runner as orchestrator: creation → executor)
...
- **2.5** Executor results are **passed back** using the **task creation (continuation)** prompt so it can interpret results and decide **continue** or **conclude**.

Deliverable: End-to-end flow: goal → task creation (prompt) → (optional skill run) → executor (skill) → results → task creation (continue/conclude); all I/O in markdown.

### Phase 3 — Task refinement and full flow
...
Deliverable: Full two-step decomposition (creation → refinement with precursor data → exact calling conventions) and skill-before-executor behaviour; refinement can be iterated with feedback.

### Phase 4 — Multi-round and robustness

- **4.1** **Results → task creation (continuation):** Runner passes executor outputs back using the **continuation** prompt from `skill-prompts` (not the initial one). Input is the completed task list + results; the prompt instructs the model to interpret them and decide **continue** (emit new coarse tasks) or **conclude** (done). This closes the loop: creation (initial) → refinement → executors → **creation (continuation)** → if continue, repeat; if conclude, stop.
...
- **4.3** Add more pre-execution skills and executor skills as needed (executors as wrappers around shell, web search, codebase search, etc.); keep the same contract (skill run by Runner → file → executor).

---

## 7. Use case scenarios for basic testing
...
- **Prompt types:** Task creation in **two variants** — (1) **initial**: goal → coarse task list; (2) **continuation**: completed task list + executor outputs → interpret and decide continue or conclude (different prompt). Task refinement (exact calling conventions from precursor data); task execution (one file). **Only the executor is a real skill**; creation and refinement use **prompts** in **`resources/skill-prompts`**.
...
- **Phase flow:** Define contracts and skill-prompts (Phase 1) → Runner as orchestrator, creation → executor → back to creation (Phase 2) → refinement + precursor data + re-run with feedback (Phase 3) → full loop and robustness (Phase 4).
