...
- `libocfiles/Tree.vala` (TreeBase class) - Add AST path resolution methods
- Add `resolve_ast_path(string ast_path)` method to TreeBase
- Add `resolve_ast_name(string name)` method to TreeBase
- Add `get_ast_node(string ast_path)` method to return AST node
- Add `build_ast_path_map()` method to build path → node mapping
- Store AST path mapping in TreeBase for efficient lookups

**2. Update VectorMetadata**
- Add `ast_path` property to VectorMetadata (e.g., "OLLMchat/Client/chat")
- Populate ast_path when creating VectorMetadata from AST nodes
- Use ast_path for search result identification
- Store ast_path in database for search queries

**3. Update Codebase Search Tool**
- `libocvector/Tool/CodebaseSearchTool.vala` - Add AST path support
- Search results include AST paths for matched elements
- Allow searching by AST path: "find all methods in Client class"
- Return AST paths in search results for easy reference
- Support AST path filters in search queries

**4. Search Result Format**
- Include AST path in search result metadata
- Format: `file_path:ast-path: namespace/class/method`
- Enable direct reference to search results using AST paths
- Link search results to ReadFile/EditFile operations via AST paths

**5. AST Path Indexing**
- Index AST paths during codebase indexing
- Store AST paths in VectorMetadata database records
- Enable fast AST path lookups during search
- Support reverse lookups (find file from AST path)
...
- `libocfiles/Tree.vala` - Extend TreeBase with AST path resolution methods
  - `resolve_ast_path(string ast_path)` - Resolve AST path to node/range
  - `resolve_ast_name(string name)` - Resolve name to node/range
  - `get_ast_node(string ast_path)` - Get AST node by path
  - `build_ast_path_map()` - Build path → node mapping
  - `get_ast_path_for_node(ASTNode node)` - Generate AST path for node
...
- `libocvector/Indexing/Tree.vala` - Use TreeBase AST path methods
- `libocvector/Indexing/VectorMetadata.vala` - Add ast_path property
- `libocvector/Tool/CodebaseSearchTool.vala` - Add AST path support to search
- `libocvector/Database.vala` - Store ast_path in database schema
