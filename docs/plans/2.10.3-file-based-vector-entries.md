...
- [ ] Create file-level analysis prompt template (resources/ocvector/analysis-prompt-file.txt - new file with system/user message template)
- [ ] Add analyze_file() method to Analysis class (libocvector/Indexing/Analysis.vala - method that loads prompt, builds element summary, calls LLM, creates file VectorMetadata)

### Phase 3: File Element Creation and Indexing
- [ ] Create file VectorMetadata in analyze_file() (libocvector/Indexing/Analysis.vala - create VectorMetadata with element_type='file', ast_path='', etc. and add to tree.elements)
- [ ] Integrate file analysis into Indexer pipeline (libocvector/Indexing/Indexer.vala - call analyze_file() after analyze_tree() in index_file())
...
- **File**: `libocvector/Indexing/Tree.vala`
  - In `extract_element_metadata()` method, build AST path from namespace and parent_class:
    - If namespace and parent_class exist: `ast_path = namespace + "-" + parent_class + "-" + element_name`
    - If only namespace exists: `ast_path = namespace + "-" + element_name`
    - If only parent_class exists: `ast_path = parent_class + "-" + element_name`
    - If neither exists: `ast_path = element_name`
    - Set `metadata.ast_path` before returning the VectorMetadata object
