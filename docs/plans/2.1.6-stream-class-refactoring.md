...
- ⏳ **Phase 2: PENDING** - FileChange AST path resolution implementation
...
- [ ] **FileChange Class Enhancements:**
  ...
  - [ ] **Note:** FileChange.resolve_ast_path() also handles operation_type adjustments (BEFORE, AFTER, DELETE) after resolving the AST path.
  - [ ] **Note:** FileChange now handles its own AST path resolution. Stream just creates FileChange with `ast_path` set, then calls `resolve_ast_path()` async. Project manager is accessed via `file.manager` (or similar).
