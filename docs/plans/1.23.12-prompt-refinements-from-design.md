...
- **Update task_refinement.md:** When optional sections are implemented, wrap the "Your previous output" block in **{previous_output/start}** … **{previous_output/end}** so that section is omitted when `previous_output` is empty (first attempt); only retries show the header and previous output.
