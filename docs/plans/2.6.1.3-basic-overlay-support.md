...
- Create Monitor instance: `monitor = new Monitor(project, base_path, overlay_map)` (Monitor will be started in mount())
 ...
 * 
 * Mounts the overlay filesystem at mount_point and starts the Monitor to track changes.
 * 
...
- Start Monitor: `monitor.start()` (monitor was created in constructor)
