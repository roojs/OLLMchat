...
- **PromptTemplate:** Use **template(key)** (e.g. `PromptTemplate.template("task_refinement")`) and ensure templates load from skill-prompts dir; Details/Runner must not instantiate by filename and load() if plan requires template(key). See PromptTemplate section.
- **Task-class refinements:** Bugs and alignment in List, Step, Details, ResultParser only → **1.23.15**.

**Dependencies:** 1.23.11 (parent), 1.23.13 (task-related code, DONE), 1.23.15 (task refinements), 1.23.2, 1.23.3–1.23.6 (formats), 1.23.1 (existing Runner/Manager/Definition).
...
**Runner API for Task (get_definition, env, project_manager, current_file, reference_content)**
...
/** Formatted environment string for template placeholder "environment". See prepare_tasks fill, 1.23.13 shared context. */
public string env()
{
...
/** Resolve non-file reference for Details.reference_contents(). href: project_description | current_file | #section | output:task | plan:... */
public string reference_content(string href)
{
	if (href == "project_description")
		return this.project_manager.active_project != null ? this.project_manager.active_project.summary() : "";
	if (href == "current_file")
		return this.current_file;  // or resolve via project_manager/buffer_provider
...
// Validation in prepare_tasks ensures skill exists; Details may cast or assume non-null.
```
