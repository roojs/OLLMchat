...
### Indexing Flow:
```
1. Source code file (OLLMfiles.File)
   ↓
2. Tree layer: Parse with tree-sitter → extract structure (VectorMetadata objects)
   - Element type, name, signature, namespace, parent class
   - Line numbers, documentation ranges
   ↓
3. Analysis layer: Generate descriptions for complex elements (LLM)
   - Skip LLM for simple elements (enum types without docs, simple properties, etc.)
   - Store description in VectorMetadata.description
   ↓
4. VectorBuilder: Format as text document (using structure from Tree + description from Analysis)
   ↓
5. Generate embedding via Ollama API → float[768]
   ↓
6. Store vector in FAISS → gets vector_id (e.g., 42)
   ↓
7. Store metadata in SQL:
...
- Basic vector search
- SQL database schema for metadata (`VectorMetadata.initDB()`)
- SQL insert/select methods (`VectorMetadata.saveToDB()`, `VectorMetadata.query()`)
- Metadata lookup after search (`Search.execute()` uses `VectorMetadata.lookup()`)
- Integration with `libocsqlite` (via `SQ.Database` and `SQ.Query`)
...
- ❌ `signature` - Already in code snippet (read from file), also extracted by Tree layer but not needed in DB
- ❌ `namespace` - Extracted by Tree layer, included in vectorized text, but not needed in DB
- ❌ `parent_class` - Extracted by Tree layer, included in vectorized text, but not needed in DB
- ❌ `description` - Already in vectorized text, code snippet shows what it does (generated by Analysis layer for complex elements)
- ❌ `codedoc_start`, `codedoc_end` - Extracted by Tree layer, used during analysis, but not needed in DB
...
- `signature` - In code snippet (also extracted by Tree layer but not stored in DB)
- `namespace` - Extracted by Tree layer, included in vectorized text, but not needed in DB
- `parent_class` - Extracted by Tree layer, included in vectorized text, but not needed in DB
- `description` - In code snippet/vectorized text (generated by Analysis layer for complex elements)
- `codedoc_start`, `codedoc_end` - Extracted by Tree layer, used during analysis, but not needed in DB
