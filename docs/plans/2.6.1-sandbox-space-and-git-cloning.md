...
✅ **DONE** – Implemented. All four phases (Playground, Working Directory, Documentation, Network Parameter) are in place. Manual testing recommended (see Section 8).
...
- [x] Add method to get playground path (fixed: `~/.local/share/ollmchat/playground`)
- [x] Create playground directory if it doesn't exist
- [x] Add playground mount to `Bubble.build_bubble_args()`
- [ ] Test playground is writable in sandbox

### Phase 2: Working Directory Parameter
- [x] Add `working_dir` property to `Request`
- [x] Add normalization method to convert to absolute path
- [x] Implement special case: normalize "playground" to `$HOME/playground` (user's home directory)
- [x] Add validation in `execute()` to check if normalized directory exists
- [x] Return error message if directory doesn't exist
- [x] Update `Tool.parameter_description` (document absolute path requirement)
- [x] Modify `Bubble.exec()` to accept and use normalized `working_dir`
- [x] Update `build_bubble_args()` to set `--chdir`
- [x] Update subprocess fallback to use normalized `working_dir` with validation
...
- [x] Update `Tool.description` with file system permissions
- [x] Update `Tool.description` with network access information
- [x] Update `Tool.parameter_description` with new parameters

### Phase 4: Network Parameter
- [x] Add `one_time_only` property to `RequestBase`
- [x] Add `network` property to `Request`
- [x] Modify `build_perm_question()` to handle network requests
- [x] Set `one_time_only = true` when network is requested
- [x] Implement unique permission path for network requests (bypass cache)
- [x] Update `execute()` to request permission for network access
- [x] Pass `allow_network` to `Bubble` based on permission
- [x] Modify `ChatPermission.request()` to accept `RequestBase` instead of string
- [x] Update permission widget to show/hide buttons based on `one_time_only`
- [x] Update `Permission.request_user()` to pass `RequestBase` to widget
...
- **`working_dir="playground"` validation:** Normalization maps "playground" to `$HOME/playground`, and we validate that path on the host. The real playground is `~/.local/share/ollmchat/playground`; `$HOME/playground` exists only inside the sandbox (bind mount). If `$HOME/playground` does not exist on the host, validation will fail. Consider validating against the real playground path for the "playground" case, or ensuring the path used for `--chdir`/subprocess is correct for both Bubble and subprocess fallback.
- **ChatPermission API:** The plan suggested passing `RequestBase` into `ChatPermission.request()`. The implementation uses `request(string question, bool one_time = false)` and `Permission.request_user()` passes `request.permission_question` and `request.one_time_only`; behavior matches the intended one-time-only UI.
