...
- **Methods**:
  - `get_tool_config(string tool_name)` - Get configuration for a tool (returns BaseToolConfig or subclass)
  - `configure_tool(string tool_name)` - Configure tool settings
  - `get_tool_config_ui(Tool tool, BaseToolConfig config)` - Generate dynamic UI for tool-specific config
  - `create_model_usage_editor(ModelUsage usage)` - Helper to create ModelUsage editor widget (reusable for nested ModelUsage properties)
  - Note: Tools are auto-discovered from Client registry, no refresh needed
  - Note: Each tool has one configuration entry in `Config2.tools` map
...
- Codebase search currently uses two separate ModelUsage entries in `Config2.usage` map:
  - `usage["ocvector.embed"]` → ModelUsage (embedding model)
  - `usage["ocvector.analysis"]` → ModelUsage (analysis model)
- Needs to be consolidated into single CodebaseSearchToolConfig in `Config2.tools` map:
  - `tools["codebase_search"]` → CodebaseSearchToolConfig
  - CodebaseSearchToolConfig has `embed` (ModelUsage) and `analysis` (ModelUsage) as sub-settings
- ModelUsage is still used, but as sub-settings within tool configs (not as top-level entries in usage map)
- Other tools using ModelUsage in usage map may also need migration to tools map
