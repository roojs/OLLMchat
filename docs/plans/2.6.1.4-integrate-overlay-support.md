...
1. **Pre-execution**:
   - Create Overlay instance (with project folder and list of project root folders)
   - Create overlay directory structure (including subdirectories in upper: overlay1, overlay2, etc.)
   - Mount overlay filesystem
   - Get base_path and overlay_map from Overlay instance
   - Create Monitor instance for overlay upper directory (pass project_folder, base_path, overlay_map)
   - Start monitoring
...
- Monitor overlay upper directory (starts empty, with subdirectories overlay1, overlay2, etc.)
- Monitor converts overlay paths to real project paths using overlay_map
- Track all changes during command execution
- Provide change lists for file copying (with real project paths)
