...
- Bubble uses overlay.overlay_map.entries to get both subdirectory names (keys) and project root paths (values)
- Bubble binds overlay subdirectories (e.g., `merged/overlay1`) to project root paths, not the entire mount point
...
- Use `--bind /home/alan/.cache/ollmchat/overlay-12345/merged/overlay1 /home/alan/project` (overlay subdirectory bind)
- Writes to `/home/alan/project/file.c` in sandbox → overlay subdirectory → upper/overlay1/file.c
- Monitor sees change in `upper/overlay1/file.c` and converts to `/home/alan/project/file.c`

**Multiple project roots:**
- For each entry in `overlay.overlay_map.entries`:
  - Bind overlay subdirectory (e.g., `merged/overlay1`) at that root's path in sandbox
  - Example: `--bind {overlay_mount_point}/overlay1 /home/alan/project`
  - Example: `--bind {overlay_mount_point}/overlay2 /home/alan/other-project`
- Each project root gets its own overlay subdirectory binding
- Overlay filesystem routes writes to correct upper subdirectory based on which subdirectory was bound
- `roots` HashMap is removed from Bubble - Overlay already has all root information in `overlay_map`
- Bubble uses `overlay.overlay_map.entries` to get both subdirectory names and project root paths
...
- [ ] Modify `build_bubble_args()` in Bubble to use `overlay.overlay_map.entries` instead of `roots.entries` (binds subdirectories, not entire mount point)
