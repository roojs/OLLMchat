...
- **Add skill-prompts to the build:** They must be registered in **resources/gresources.xml** and included in the **meson.build** that compiles resources. Without this, resource:/// URIs for skill-prompts will not resolve.
  - **resources/gresources.xml:** Add a `<gresource prefix="/skill-prompts">` block and list each template file (e.g. `task_creation_initial.md`, `task_refinement.md`, `task_list_iteration.md`, `task_execution.md`) as `<file>` entries.
...
```xml
  <gresource prefix="/skill-prompts">
    ...
    <file>task_list_iteration.md</file>
...
- Prompt files live in **resources/skill-prompts**: `task_creation_initial.md`, `task_refinement.md`, `task_list_iteration.md`, `task_execution.md`.
...
// Details (post_evaluate): "task_list_iteration.md"
