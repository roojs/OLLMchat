...
- **ℹ️** `ARRAY` / float / double / width / `ENUM` / `FLAGS` / list OUT → [`8.4.6-DONE`](8.4.6-DONE-rpc-ffi-leftovers.md). List IN → [`8.4.9`](8.4.9-rpc-ffi-glist-in.md). `Gio.File` reconstruct is **not** libocrpc. INOUT boxed is skipped there too.
...
- **ℹ️** `Gio.File` / `GLocalFile`: not registered by `Gi.register`. Reconstruct is the caller’s serializer, not [`8.4.6-DONE`](8.4.6-DONE-rpc-ffi-leftovers.md).
...
- **ℹ️** `GValue.transform` width, `FLOAT` / `DOUBLE` / `ARRAY`, `ENUM` / `FLAGS`: [`8.4.6-DONE`](8.4.6-DONE-rpc-ffi-leftovers.md).
...
**💩** This hunk is caller-allocates OUT only. INOUT boxed is skipped in [`8.4.6-DONE`](8.4.6-DONE-rpc-ffi-leftovers.md) too.
...
- **🚫** `ARRAY` / float / double / width / `ENUM` / `FLAGS` — [`8.4.6-DONE`](8.4.6-DONE-rpc-ffi-leftovers.md). `Gio.File` reconstruct and INOUT boxed are not that plan either.
