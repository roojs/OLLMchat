...
**Status:** `✔️` agent-done — boxed / GObject `INTERFACE` / `StreamValue` `Bytes` landed. Invoke return still **⏳**. Awaiting user **✅**.

**Parent:** [`8.4-rpc-positional-values-and-ffi.md`](8.4-rpc-positional-values-and-ffi.md)

**Depends on:** [`8.4.2`](8.4.2-DONE-rpc-ffi-typelib-invoke.md) / [`8.4.3`](8.4.3-DONE-rpc-ffi-typelib-method.md) — landed. Invoke **errors** are [`8.4.4`](8.4.4-rpc-invoke-errors.md). Remaining convert tags (float, double, array, ENUM/FLAGS, width) are [`8.4.6`](8.4.6-rpc-ffi-leftovers.md).
...
- **ℹ️** `ARRAY` / float / double / width / `ENUM` / `FLAGS` → [`8.4.6`](8.4.6-rpc-ffi-leftovers.md). `Gio.File` reconstruct is **not** libocrpc. INOUT boxed is skipped there too.
...
- **ℹ️** `Gio.File` / `GLocalFile`: not registered by `Gi.register`. Reconstruct is the caller’s serializer, not [`8.4.6`](8.4.6-rpc-ffi-leftovers.md).
...
- **ℹ️** `GValue.transform` width, `FLOAT` / `DOUBLE` / `ARRAY`, `ENUM` / `FLAGS`: [`8.4.6`](8.4.6-rpc-ffi-leftovers.md).
...
**💩** This hunk is caller-allocates OUT only. INOUT boxed is skipped in [`8.4.6`](8.4.6-rpc-ffi-leftovers.md) too.
...
- **🚫** `ARRAY` / float / double / width / `ENUM` / `FLAGS` — [`8.4.6`](8.4.6-rpc-ffi-leftovers.md). `Gio.File` reconstruct and INOUT boxed are not that plan either.
