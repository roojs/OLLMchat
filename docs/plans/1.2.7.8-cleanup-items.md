...
### Inline Temporary Variables in add_message() Calls

**Issue**: Many locations in the code create a temporary variable just to pass to `add_message()`, and the variable is never used afterward. These should be inlined directly into the `add_message()` call.
...
### Code Cleanup
- All files containing `add_message()` calls - Inline temporary variables used only for `add_message()` calls
