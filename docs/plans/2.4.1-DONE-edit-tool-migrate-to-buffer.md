...
- [ ] Edit mode (when `--edit-complete-file` is NOT set):
  - Reads JSON from stdin (array of FileChange objects) or uses `--edit-change=START:END:CONTENT_FILE` (can be specified multiple times)
...
- [ ] JSON format (read from stdin):
  ...
  ```
  - If `--edit-change` options are provided, they take precedence over stdin
  - If no `--edit-change` options and stdin is empty, error
