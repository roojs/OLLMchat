...
- [ ] `--edit-complete-file=PATH` - Complete file mode (create new file or overwrite existing)
...
- [ ] `--edit=PATH` - Edit mode (apply edits to existing file)
  - Requires `--edit-changes=FILE` (JSON file with array of changes) or `--edit-change=START:END:CONTENT_FILE` (can be specified multiple times)
  - Each change has: `start` (1-based, inclusive), `end` (1-based, exclusive), `replacement` (text or file path)
  - Output: `FILE: path`, `FILE_ID: id`, `LINE_COUNT: count`, `CHANGES_APPLIED: count`, `MODE: edit`

- [ ] `--edit-changes=FILE` - JSON file containing array of FileChange objects:
  ```json
  [
...
- [ ] `--edit-change=START:END:CONTENT_FILE` - Single change specification (can be specified multiple times)
  - Format: `START:END:CONTENT_FILE` where START and END are line numbers (1-based), CONTENT_FILE is path to file with replacement text
  - Alternative: `--edit-change-text=START:END:TEXT` for inline text (no newlines)
...
- [ ] **Test 1: Complete file mode (new file creation)**
  - Create test file with `--edit-complete-file` pointing to non-existent file
...
- [ ] **Test 2: Complete file mode (overwrite existing file in project)**
  ...
  - Overwrite with `--edit-complete-file --overwrite`
...
- [ ] **Test 4: Edit mode (multiple edits)**
  - Create test file with known content
  - Create project and add file
  - Apply multiple edits using `--edit-changes` JSON file
  - Verify backup is created
