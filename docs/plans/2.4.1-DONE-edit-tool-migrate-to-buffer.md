...
- [ ] `--edit-complete-file` - Flag: enable complete file mode (overwrite entire file)
  ...
  - Reads complete file content from stdin (raw text, not JSON)
...
- [ ] Stdin format:
  - **Complete file mode** (`--edit-complete-file`): Raw file content (plain text)
  - **Edit mode**: JSON array of FileChange objects:
  ```json
  [
