...
| `Folder` | ✅ [`done/2.10.4.9.3.1`](done/2.10.4.9.3.1-DONE-daemon-folder-file-relays.md) — `Folder.fetch`, `contains_folder`, `fetch_file_list`, `fetch_pending_approvals`, **`project_description`** ✔️ |
...
| `vector.*` | ⏳ other methods; search → **`semantic.search`** [`2.10.4.18`](2.10.4.18-ACTIVE-vector-search-rpc.md) |
| `semantic.search` | ⏳ [`2.10.4.18`](2.10.4.18-ACTIVE-vector-search-rpc.md) |
...
## `semantic` — search relay ([`2.10.4.18`](2.10.4.18-ACTIVE-vector-search-rpc.md))

**🔷** Search only — wire object **`semantic`** because **`OLLMfilesd.Vector`** is an indexing namespace, not an RPC class.

- **`semantic.search`** — `Search.execute`, `RequestCodebaseSearch` — **⏳** [`2.10.4.18`](2.10.4.18-ACTIVE-vector-search-rpc.md)
  - Vala class **`Semantic`**, signal **`rpc_search`**
...
Daemon hosts FAISS + SQL. Codebase search uses **`semantic.search`** (see above).
