...
| `vector.*` | **ACTIVE** — [`2.10.4.29`](2.10.4.29-ACTIVE-vector-cli-rpc-migration.md); **`Codebase.search`** ✅; **`Codebase.start`**/**`stop`** ⏳ |
| `Codebase.search` | **✅** [`done/2.10.4.18`](done/2.10.4.18-DONE-vector-search-rpc.md) · [`2.10.4.29`](2.10.4.29-ACTIVE-vector-cli-rpc-migration.md) |
...
## `Codebase` — vector search + admin ([`done/2.10.4.18`](done/2.10.4.18-DONE-vector-search-rpc.md) · [`2.10.4.29`](2.10.4.29-ACTIVE-vector-cli-rpc-migration.md))
...
| **`Codebase.file_info`** | **`call_file_info`** | ⏳ | **`--show-info`** — port from **`File.vector_metadata`** |
| **`Codebase.debug_get`** | **`call_debug_get`** | ⏳ | **`--dump-vector`** |
| **`Codebase.reset`** | **`call_reset`** | **✅** | **`Indexer.reset_database`** |
| **`Codebase.start`** | **`call_start`** | ⏳ | **`stop`** (pause) → **`read_dir`** → append to queue → resume |
| **`Codebase.stop`** | **`call_stop`** | ⏳ | **pause** — halt loop, **preserve** **`file_queue`** (**🚫** no drain) |

- register **`"Codebase"`** once in **`Application.vala`** (already done for search)
- params: **`VectorParams`** — **`file_path`**, **`ast_path`**, **`path`**, **`only_file`**, **`recurse`**; optional **`skip_scan`** to resume without **`read_dir`** (**💩**)
...
- **`read_dir`** on **`start`** refreshes DB so enqueue finds newly stale files; **not** because **`stop`** cleared the queue
...
**Deprecate:** **`File.vector_metadata`** once **`Codebase.file_info`** ships.
