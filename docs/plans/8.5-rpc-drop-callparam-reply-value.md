...
- **🔷** Remaining in §2.4: keyword / ABI / Live (`lease_id` as `this`). Do not teach the library `_sync`.

### 2.4 — Remaining methods

**🔷** Scan columns. `wrap` = `call_*` lambda only `.begin`s or calls an existing method. `rpc-only` = that existing method is only reached from this lambda. `block` = what actually stops a hello-pattern FFI lift.

| Method | async | wrap | clash | rpc-only | block |
| --- | --- | --- | --- | --- | --- |
| `File.delete` | n | n | n | — | keyword |
| `Folder.fetch_files` | y | y | n | — | ABI |
| `Live.Remote.ref` / `unref` | n | n | y | — | GObject+lease |
| `Live.Subscribe.signal` | n | n | n | — | keyword+lease |
| `Live.Subscribe.unsubscribe` | n | n | n | — | lease |

#### Async C labels (Vala `.begin` / `.end`) — leftover `fetch_files`
...
| Method | C start | C finish | extra args | dlsym |
| --- | --- | --- | --- | --- |
| `fetch_files` | `oll_mfilesd_folder_fetch_files_reply` | `…_finish` | `Request`, `Folder*` | n |

**🔷** A C trampoline could be the `GAsyncReadyCallback`: call `_finish(source, res)`. FFI would pass that function plus `user_data` after the Vala args (two extra pointer slots vs today’s `(self, Request, …)`).
