...
- **🔷** Methods that are not a straight lift stay on `call_*`. Reasons in §2.4. Do not rename, wrap, or teach the library `_sync`.

### 2.4 — Not converted (name clash, keyword, or ABI)

Mechanical rule: lift the lambda onto an instance method named exactly as the wire suffix (dots → underscores in C only). List that suffix in `add_class`. Drop that `call_*`. Stop if the class already has a method of that name, if Vala/GObject forbids the name, or if the C ABI is not one FFI pointer per signature letter.

- **🔷** `ProjectManager.load_projects_from_db` — existing `async void load_projects_from_db()` used in-process (`Application`, `BackgroundScan`, `ProjectMigrate`).
- **🔷** `ProjectManager.create_project` — existing `Folder create_project(string path)` used in-process (`Application`, `ProjectMigrate`). RPC is `void` + `request.reply`.
- **🔷** `ProjectManager.remove_project` — existing `void remove_project(Folder)`. Wire is `args("s", path)`, not a `Folder` argument.
- **🔷** `ProjectManager.activate_project` — existing `async void activate_project(...)` is the in-process worker. RPC wraps `.begin`.
- **🔷** `File.write` — existing `private async void write(Request)`.
- **🔷** `File.delete` — `delete` is a Vala keyword.
- **🔷** `Folder.fetch_files` — wire `as` (`string[]`). Vala arrays pass a hidden C length. FFI `as` is one pointer. Not a one-letter-per-arg lift.
- **🔷** `Folder.project_description` — existing `string project_description()`.
- **🔷** `Folder.roots` — existing `Gee.ArrayList<Folder> roots()`.
- **🔷** `Codebase.search` — existing `private async void search(Request)`.
- **🔷** `Codebase.debug_get` — existing `private async void debug_get(Request)`.
- **🔷** `FileHistory.approve` — existing `void approve(SQ.Database, File)`.
- **🔷** `FileHistory.revert` — existing `async void revert(...)`.
- **🔷** `Live.Remote.ref` / `unref` — clash with `GLib.Object.ref` / `unref`. `lease_id` is the target object, not the `Remote` handler `this`.
- **🔷** `Live.Subscribe.signal` / `unsubscribe` — `signal` is a Vala keyword. Same lease-id-as-target, not handler `this`.
...
- **🚫** Rename or overload an existing in-process method so FFI can reuse the name.
