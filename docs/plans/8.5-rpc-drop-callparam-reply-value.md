...
- **🔷** `Request.add_class`: wire prefix, `GLib.Type`, then Vala `...` pairs of method name + D-Bus signature (same letters as `OLLMrpc.args`). End with `null`. That is the new FFI table. Not an overload of `register`.
...
- **🔷** `⏳` Leave `Request.register(name, instance)` as today. Add `Request.add_class(name, type, ...)` and the method table.
...
- **🔷** Unpack: `if` `s` then string and `continue`. The rest is `i` (enough for `hello`). Later letters get their own `if` above that.

### 5. `libocrpc/ffi.vapi` — libffi C types

**Why:** Ubuntu/valac ship no `libffi.vapi`. `pkg-config libffi` is 3.4.7. In-tree binding (**🔷**).
...
- **🚫** Copy `OLLMrpc.args`’s `f` scan into `invoke` without unpacking float. This cut is `s` / `i` only (`hello`).
...
- **🚫** Overload `register`. The new table is `add_class`.
