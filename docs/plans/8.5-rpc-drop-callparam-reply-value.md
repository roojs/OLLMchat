...
**Status:** **PROPOSED** — Phase 1 (8.5.1) agent ✔️ consumer cut; Phase 2 (FFI dispatch + `hello`) agent ✔️; Phase 3 (reply `GLib.Value`) ⏳.
...
- **🔷** Remaining methods: inventory in §2.4 before any `rpc_` prefix cut. Do not teach the library `_sync`.

### 2.4 — Remaining methods (async / wrapper / clash)

**🔷** Two columns: wire suffix, and whether async, a short `call_*` wrapper, or a name clash is actually the blocker. “RPC-only” means the existing method is only reached from that `call_*` lambda (not Application / migrate / other daemon code).

| Method | Note |
| --- | --- |
| `ProjectManager.load_projects_from_db` | Async wrapper (`.begin` + reply). Clash with `async void load_projects_from_db()` — **in-process** (`Application`, `BackgroundScan`, `ProjectMigrate`). Clash is the issue. |
| `ProjectManager.create_project` | Sync wrapper around `Folder create_project(string path)`. **In-process** (`Application`, `ProjectMigrate`). RPC is `void` + reply. Clash is the issue. |
| `ProjectManager.remove_project` | Sync wrapper around `void remove_project(Folder)`. Wire is `args("s", path)`. **RPC-only** call site; clash is signature (`Folder` vs path), not a second caller. |
| `ProjectManager.activate_project` | Async wrapper (`.begin`). Existing `async void activate_project(Request, Folder?, bool)` already takes `Request`. **RPC-only**. Async is the issue, not a competing API. |
| `File.write` | Async wrapper (`.begin`). Existing `private async void write(Request)`. **RPC-only**. Async is the issue, not a competing API. |
| `File.delete` | Inline lambda (not a wrapper). No `delete()` on daemon `File`. **Keyword** is the issue. |
| `Folder.fetch_files` | Async wrapper of `fetch_files_reply`. No method named `fetch_files`. **ABI** (`as` / hidden Vala length) is the issue, not a name clash. |
| `Folder.project_description` | Sync wrapper around `string project_description()`. **In-process** (`Codebase`). Clash is the issue. |
| `Folder.roots` | Sync wrapper around `Gee.ArrayList<Folder> roots()`. **In-process** (`FileWithHistory`). Clash is the issue. |
| `Codebase.search` | Async wrapper (`.begin`). Existing `private async void search(Request)`. **RPC-only**. Async is the issue, not a competing API. |
| `Codebase.debug_get` | Async wrapper (`.begin`). Existing `private async void debug_get(Request)`. **RPC-only**. Async is the issue, not a competing API. |
| `FileHistory.approve` | Lambda looks up a row, then `row.approve(db, file)`. Existing `void approve(SQ.Database, File)` is **RPC-only** as a call site, but it is the row API on the same class as the handler singleton. Clash is the issue (same type, different signature). |
| `FileHistory.revert` | Async wrapper (`row.revert.begin`). Existing `async void revert(Request, ProjectManager)` already takes `Request`. **RPC-only**. Async is the issue; also runs on the **row**, not the `for_rpc` singleton. |
| `Live.Remote.ref` / `unref` | Inline lambdas. Clash with `GLib.Object.ref` / `unref`. **Keyword/GObject** is the issue. Also `lease_id` is the target object, not handler `this`. |
| `Live.Subscribe.signal` | Inline lambda. **`signal` is a Vala keyword.** Same `lease_id`-as-target. |
| `Live.Subscribe.unsubscribe` | Inline lambda. **No name clash**, no async. Same `lease_id`-as-target as `signal`. |
