...
**Next:** [`8.4.3`](8.4.3-DONE-rpc-ffi-typelib-method.md). Leftovers: [`8.4.4`](8.4.4-rpc-invoke-errors.md) (invoke errors), [`8.4.5`](8.4.5-DONE-rpc-ffi-leftovers.md) (boxed / GObject `INTERFACE` / invoke return), [`8.4.6-DONE`](8.4.6-DONE-rpc-ffi-leftovers.md) (float, array, width).
...
- **ℹ️** Method-on-handle is [`8.4.3`](8.4.3-DONE-rpc-ffi-typelib-method.md).
...
- **🔷** `register` and `types` stay static. Per-call: `new Gi(request)` then `dispatch()`. `dispatch` finds the `FunctionInfo` and routes constructors to `dispatch_new`. Non-constructors return false ([`8.4.3`](8.4.3-DONE-rpc-ffi-typelib-method.md) `dispatch_function`). `convert` is one TypeTag slot; the arg walk stays in `dispatch_new`.
...
- **🔷** `⏳` Keep a `Gi.types` map (`alias` → `GLib.Type`) for Phase 2 dispatch (and [`8.4.3`](8.4.3-DONE-rpc-ffi-typelib-method.md)). Skip `GType.INVALID`. Skip a second `Bin.register` of the same alias (`alias_to_gtype.has_key`).
...
- **ℹ️** Constructor-with-args (`Gio-SimpleAction.new`) / `INTERFACE` args → [`8.4.5`](8.4.5-DONE-rpc-ffi-leftovers.md).
...
- **🔷** `⏳` `dispatch`: prefix in `types`, `find_method`, constructors → `dispatch_new`. Non-constructor returns false ([`8.4.3`](8.4.3-DONE-rpc-ffi-typelib-method.md) `dispatch_function`). Missing method → `METHOD_NOT_FOUND`.
- **🔷** `⏳` `dispatch_new`: `live_handles`, walk IN args, `convert` per slot, invoke, export, reply.
- **🔷** Phase 2 tags in `convert`: `BOOLEAN`, `INT32`/`INT64`, `UINT32`/`UINT64`, `UTF8`/`FILENAME`. `INTERFACE` (GObject / boxed) → [`8.4.5`](8.4.5-DONE-rpc-ffi-leftovers.md). `ARRAY` / float / `GValue.transform` width → [`8.4.6-DONE`](8.4.6-DONE-rpc-ffi-leftovers.md).
...
- **🔷** `⏳` Do **not** call a method on it yet — that is [`8.4.3`](8.4.3-DONE-rpc-ffi-typelib-method.md).

This plan does **not** add `Request.lease_id` — that is [`8.4.3`](8.4.3-DONE-rpc-ffi-typelib-method.md).
...
- **🚫** Fence `dispatch_function` here. That is [`8.4.3`](8.4.3-DONE-rpc-ffi-typelib-method.md).
...
- **ℹ️** `fn.invoke` `return_value` by value — leftover [`8.4.5`](8.4.5-DONE-rpc-ffi-leftovers.md). Constructor-with-args / `INTERFACE` also there. `ARRAY` / float / width → [`8.4.6-DONE`](8.4.6-DONE-rpc-ffi-leftovers.md). Invoke throw → client is [`8.4.4`](8.4.4-rpc-invoke-errors.md).
