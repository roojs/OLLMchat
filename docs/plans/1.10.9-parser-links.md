...
```vala
			if (matched_format == FormatType.LINK) {
				...
				this.formatmap.handle_link(chunk, chunk_pos, seq_pos, link_result, this.renderer, (t) => {
					this.process_inline_content(t);
				});
...
### 5d. handle_link — call from Parser (libocmarkdown/Parser.vala)

**Purpose:** When **eat_link** returns >0, Parser calls **FormatMap.handle_link** (link extraction and rendering live in FormatMap so they can share the same regex as **eat_link**).

- **Parser** does not define handle_link or any link regex; it calls `this.formatmap.handle_link(chunk, chunk_pos, seq_pos, link_result, this.renderer, (t) => { this.process_inline_content(t); });`
- **FormatMap.handle_link** extracts link text, href, title, inline vs reference; emits `on_a(true, ...)`, then invokes the callback with link_text, then `on_a(false, ...)` (see §6).

**Concrete code (Parser):**

- Where the plan previously had `this.handle_link(chunk, chunk_pos, seq_pos, link_result);`, call FormatMap instead:
...
- **handle_link:** `public void handle_link(string chunk, int chunk_pos, int seq_pos, int end_offset, RenderBase renderer, void process_inline_content(string link_text))` — extracts and emits `on_a(true, ...)`, then `process_inline_content(link_text)`, then `on_a(false, ...)`.
...
- **FormatMap class:** Add delegate type for link-text callback, shared pattern constant, and two static regexes (lazy-initialized; both use the same inner pattern):

```vala
		public delegate void InlineContentFunc(string text);
...
- Add **handle_link** to FormatMap, inside the class, before the constructor. Use a delegate type for the callback (e.g. `public delegate void InlineContentFunc(string text);` then parameter `InlineContentFunc process_inline_content`):
...
```vala
		...
		public void handle_link(
			...
			int end_offset,
			RenderBase renderer,
			InlineContentFunc process_inline_content
		) {
			...
			GLib.MatchInfo mi;
			if (c1 == '(' && inner_link_regex.match_full(href, -1, 0, 0, out mi)) {
				var dest = mi.fetch(1);
			...
			renderer.on_a(true, href, title, is_reference);
			process_inline_content(link_text_val);
			renderer.on_a(false, href, title, is_reference);
		}
```
...
| **libocmarkdown/Parser.vala** | **handle_line_break**: reset **state_stack** before flushing text. Add **process_inline_content(string text)** (5a). When eat() returns LINK, call **eat_link**; when >0 call **formatmap.handle_link(..., this.renderer, (t) => { this.process_inline_content(t); })** (5d). Add do_format/got_format LINK cases (7, 8). No handle_link or link regex in Parser. |
| **libocmarkdown/FormatMap.vala** | Remove 2‑char keys `[x`, `[X`, `[ `; add `"[?"` and `"[??"` (LINK). Add **LINK_INNER_PATTERN** constant and **inner_link_regex** / **inline_link_regex** (shared). Add **handle_link**(chunk, chunk_pos, seq_pos, end_offset, renderer, process_inline_content callback) and **eat_link**; both use the shared regex(es). |
