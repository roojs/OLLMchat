			...
			if (c1 != '(') {
				...
				if (remainder.length > 0) {
					...
					if (first == '"' || first == '\'') {
						...
						if (close != -1) title = remainder.substring(1, close - 1).replace("\\\\", "\\").replace("\\" + first.to_string(), first.to_string());
					} else if (first == '(') {
						var depth = 1;
						var i = 1;
...
- **Implemented in §5d handle_link and §6 eat_link:** `handle_link` parses inner content: destination (angle-bracket `<...>` or bare until space/quote/paren) then optional title in `"..."`, `'...'`, or `(...)` with unescape. `eat_link` for inline link finds the **matching** closing `)` (depth scan) so title in `(...)` works.
