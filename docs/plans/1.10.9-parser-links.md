...
- For a simple implementation, **use a simple regex or character rules:**
...
- Don't change current design where already approved. For **new** link-parsing code: prefer regex or string methods (`index_of`, `contains`, `substring`, `GLib.Regex`) over character loops (CODING_STANDARDS.md). See CommonMark 6.3.
...
**Concrete code** (add this private method to Parser):
...
```vala
		...
		private void handle_link(string chunk, int chunk_pos, int seq_pos, int end_offset)
		{
			...
			if (c1 != '(') {
				...
				// Inline: ](destination "title") or ](destination 'title') or ](destination (title))
				var inner = chunk.substring(after_close + 1, (end_offset - 1) - (after_close + 1));
				var dest_end = 0;
				if (inner.length > 0 && inner.get_char(0) == '<') {
					var gt = inner.index_of_char('>', 1);
					if (gt != -1) {
						href = inner.substring(1, gt - 1);
						dest_end = gt + 1;
					} else {
						href = inner.strip();
						dest_end = inner.length;
					}
				} else {
					// Bare destination: until first space or " or ' or (
					var sp = inner.index_of_char(' ');
					var dq = inner.index_of_char('"');
					var sq = inner.index_of_char('\'');
					var lp = inner.index_of_char('(');
					dest_end = inner.length;
					if (sp != -1 && sp < dest_end) dest_end = sp;
					if (dq != -1 && dq < dest_end) dest_end = dq;
					if (sq != -1 && sq < dest_end) dest_end = sq;
					if (lp != -1 && lp < dest_end) dest_end = lp;
					href = inner.substring(0, dest_end).strip();
				}
				// Optional title: after destination, whitespace then "..." or '...' or (...)
				var remainder = inner.substring(dest_end).strip();
				if (remainder.length > 0) {
					var first = remainder.get_char(0);
					if (first == '"' || first == '\'') {
						var close = remainder.index_of_char(first, 1);
						if (close != -1) title = remainder.substring(1, close - 1).replace("\\" + first.to_string(), first.to_string()).replace("\\\\", "\\");
					} else if (first == '(') {
						var depth = 1;
						var i = 1;
						while (i < remainder.length && depth > 0) {
							var ch = remainder.get_char(i);
							if (ch == '(') depth++;
							else if (ch == ')') depth--;
							i += ch.to_string().length;
						}
						if (depth == 0) title = remainder.substring(1, i - 2).replace("\\\\", "\\").replace("\\\)", ")");
					}
				}
			}
...
- Use only string methods (`index_of_char`, `substring`); no character loops.

**Concrete code** (add this method to FormatMap, inside the class, before the constructor):
...
```vala
		...
		public int eat_link(
			...
			if (c1 == '(') {
				// Find matching closing ) so title in (...) works (e.g. ](url (title)))
				var depth = 1;
				var pos = after_close + 1;
...
### 1. Optional title: `](url "title")` — how most parsers handle it

- **Goal:** "optional `[link text](url "title")`"; Verification: "ensure `on_a(..., title)` receives the title."
- **Current concrete code:** `handle_link` sets `title = ""`; `eat_link` finds the first `)` and returns that offset. For `](url "title")` the substring between `(` and `)` is `url "title"`, so href would incorrectly include the title. We need to parse **destination** and **optional title** inside the parentheses.

**How regex / typical parsers handle it (CommonMark 6.3):**
...
4. **Implementation:** Use **string scan** (no character loops per CODING_STANDARDS): e.g. `index_of_char` for `>`, `"`, `'`, `(`, `)`; or one pass over the inner string to split destination vs title. Many implementations do: parse inner = chunk between `(` and last `)` (or match parens); then (1) if inner[0] == '<', href = inner[1 .. index_of('>')-1], rest = after '>'; (2) else href = inner up to first space or quote or '(', rest = after that; (3) rest = strip(rest); if rest starts with '"' or "'" or '(', title = content to matching delimiter, unescape.

**Concrete:** Add this parsing in **handle_link** (or a small helper) when `c1 == '('`: inner = chunk between `after_close+1` and `end_offset-1`. Parse destination then optional title as above; set href and title. **eat_link** must return the correct end offset (byte after the matching `)`); for title in `(...)` the matching `)` is the link’s closing paren, so end_offset is already that. For bare URL + `"title"` or `'title'`, end_offset is byte after the closing `)` of the link. No change to eat_link return convention; handle_link receives the full range and parses inner.
