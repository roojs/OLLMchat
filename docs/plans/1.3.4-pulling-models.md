...
2. **Create `SearchablePulldown` widget** (`ollmchat/Settings/SearchablePulldown.vala`)
...
4. **Create `AddModelDialog` class** (`ollmchat/Settings/AddModelDialog.vala`)
   ...
   - Three rows in `Adw.PreferencesGroup`:
     ...
     2. **Model row** (`Adw.ActionRow`):
        - `SearchablePulldown` widget (updated to work with `GObject.ListModel`)
        - Pass `AvailableModels.store` (Gee.ArrayList<AvailableModel>) to widget
        - Custom factory to display: model name + size (e.g., "llama3:8b 4.7GB" or "tinyllama 637MB")
        - Format size helper: convert bytes to GB (if >= 1GB) or MB (if < 1GB), show 1 decimal place
        - Placeholder text shows example format: "Search models (e.g., llama3:8b 4.7GB)"
        - Filter/search by model name
     3. **Start Download button** (in footer `Adw.PreferencesGroup`):
        ...
        - Will be wired up in Phase 5 to start pull operation
   - Creates `AvailableModels` instance in constructor, calls `load()` async to populate model list
   - Takes `SettingsDialog` (or `Config2`) as parameter to access connections
...
### Phase 7: Integration
8. **Wire up Add Model button** (`ollmchat/Settings/ModelsPage.vala`)
   - Enable the existing "Add Model" button (currently disabled)
   - Connect `clicked` signal to show `AddModelDialog`
   - Pass `ModelPullManager` instance to dialog and ModelsPage
   - Connect `ModelPullManager` progress signals to update banner

9. **Update ModelsPage initialization**
   - Create `ModelPullManager` instance in constructor
   - Check `loading.json` on page activation
   - Resume any incomplete pulls if threads aren't running
   - Connect progress updates to banner display
...
- `ollmchat/Settings/SearchablePulldown.vala` - Generic searchable dropdown with ListModel support
- `ollmchat/Settings/AddModelDialog.vala` - Dialog for selecting models to pull
- `ollmchat/Settings/ModelPullManager.vala` - Manages background pull operations

### Modified Files
- `examples/meson.build` - Add `oc-test-pull` executable
- `ollmchat/Settings/ModelsPage.vala` - Enable Add Model button, add banner, integrate pull manager
- `libollmchat/Client.vala` - Add `pull()` method for `/api/pull` endpoint (if needed, or use Call.Pull)
- `libollmchat/Call/Pull.vala` - Create new Call class for pull endpoint (if following existing pattern)
