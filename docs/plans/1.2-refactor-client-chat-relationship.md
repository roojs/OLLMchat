...
#### Step 1: Remove Configuration and Conversation Concerns from Client
- Remove `model`, `stream`, `format`, `think`, `keep_alive`, `options` from Client
- These become Chat/Embed/Generate-only properties (each Call class has its own)
- Remove `tools` from Client - Move to Chat (tools are conversation-specific, not connection-specific)
  - Tools are used by Chat when executing tool calls (`chat.tool_reply()`)
  - **Caller manages tools**: The caller (AgentHandler, Session, etc.) adds tools directly to Chat
  - Remove `Client.add_tool()` method - tools are added to Chat, not Client
  - Chat should have `add_tool(Tool.BaseTool)` method instead
  - Each Chat can have different tools (conversation-specific)
- Remove `permission_provider` from Client - Move to Session (permission handling is UI/session-specific, not connection-specific)
  - Permission provider is tied to UI (ChatWidget) and session state
  - Tools access it via `chat_call` context, so it should be on Session/Chat, not Client

**Impact on other Call classes**:
- `Call.Embed` - Currently uses `this.client.model` and loads options from `client.config.model_options`
  - Should take `model` and `options` as constructor parameters
  - `Client.embed()` and `Client.embed_array()` should create `Call.Embed` with explicit model/options
  - **Note**: Both `embed()` and `embed_array()` currently exist and use `this.model` - need to add `model` parameter
- `Call.Generate` - Currently uses realized properties from Client (`client.model`, `client.options`, etc.)
  - Should have real properties set via constructor/initializer
  - `Client.generate()` should create `Call.Generate` with explicit properties
  - **Note**: `generate()` currently exists and uses `this.model` - need to add `model` parameter

#### Step 2: Remove Unnecessary Signals from Client
- Remove `chat_send(Call.Chat)` signal - caller knows when they call `send()`
- Remove `message_created(Message, ChatContentInterface?)` signal - caller creates messages, they know when
- Remove `stream_content(string, Response.Chat)` signal - redundant, `stream_chunk` has `is_thinking` parameter
- Keep only asynchronous signals: `stream_chunk(string, bool, Response.Chat)`, `stream_start()`, `tool_message(Message)`

#### Step 2b: Consider Config and available_models

**Config Property Analysis**:
- **Current usage**: `client.config.model_options` is accessed by:
  - `Call.Chat` constructor - loads per-model options if not provided
  - `Call.Embed` constructor - loads per-model options if not provided
- **Config contents** (not all connection-related):
  - `connections` (Map) - Connection configs (connection-related)
  - `model_options` (Map) - Per-model option overrides (NOT connection-related, global config)
  - `usage` (Map) - Model usage configs (NOT connection-related)
  - `tools` (Map) - Tool configs (NOT connection-related)
- **Decision needed**: 
  - Option A: Keep `config` on Client, but only for `model_options` lookup
  - Option B: Remove `config` from Client, pass `model_options` map to Call constructors
  - Option C: Call classes access Config2 directly (via Manager or passed parameter)
- **Recommendation**: Option B - Pass `model_options` map to Call constructors. Client shouldn't need full Config2.

**available_models Property Analysis**:
- **Current location**: Client
- **Nature**: Connection-specific (models available on that server)
- **Usage**:
  - Tool capability checks (`model.can_call`) in Chat serialization
  - Model validation in ModelUsage.verify_model()
  - ConnectionModels refresh (creates Client, fetches models)
- **Consideration**: Move to Connection as non-serialized property?
  - Pros: More logical (connection-specific data on Connection)
  - Cons: Client needs it for tool checks, would need `connection.available_models`
- **Recommendation**: Keep on Client for now (Client is the interface to Connection), but document it's connection-specific

#### Step 3: Update Call.Base and Chat Constructor
- **Call.Base** constructor takes `Connection` directly (not Client)
- Update `OllamaBase` to store `Connection` instead of `Client`
- Update all `Call.Base` methods to use `this.connection.*` instead of `this.client.connection.*`
- **Chat constructor** takes `Connection` directly (not Client)
- Chat properties are set via object initializer or constructor parameters
- No default copying from Client (Client doesn't have these anymore)
- **Note**: Client still exists as a convenience wrapper, but Call objects take Connection directly

#### Step 4: Update All Chat Creation
- All places creating Chat must explicitly set properties
- Use Config2 or defaults as needed
- Example: `new Call.Chat(connection) { model = "...", stream = true }`

#### Step 5: Update Callers to Handle State Directly
- Remove signal connections for `chat_send` and `message_created`
- Callers update state directly when they:
  - Create messages (caller knows when)
  - Send requests (caller knows when they call `send()`)
- Keep signal connections only for streaming events

#### Step 6: Update Legacy Methods
- `Client.chat(string)` creates Chat internally with defaults
