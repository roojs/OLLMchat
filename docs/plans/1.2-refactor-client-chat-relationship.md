...
- `soup` (Soup.Session?) - HTTP session (created lazily by Call.Base when needed)
...
**Signals to Keep** (for non-agent usage only):
- `stream_chunk(string, bool, Response.Chat)` - Asynchronous event for loosely coupled components
- `stream_start()` - Asynchronous event for loosely coupled components
- `tool_message(Message)` - Asynchronous event for loosely coupled components

**Note**: For agent usage, Chat calls handler methods directly (no signals). Signals are only for loosely coupled components (non-agent usage).
...
6. **Signals**
   ...
   - Keep asynchronous signals: `stream_chunk`, `stream_start`, `tool_message` (for non-agent usage)
