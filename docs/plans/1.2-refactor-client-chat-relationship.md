...
**Note**: For agent usage, Chat calls handler methods directly (no signals). Signals are only for loosely coupled components (non-agent usage). These signals are kept on Client during refactoring for backward compatibility, but will be moved to Chat in [1.2.7. Move Signals to Chat](./1.2.7-move-signals-to-chat.md).
...
7. **[1.2.7. Move Signals to Chat](./1.2.7-move-signals-to-chat.md)** - Final tidy-up: Move remaining asynchronous signals from Client to Chat
