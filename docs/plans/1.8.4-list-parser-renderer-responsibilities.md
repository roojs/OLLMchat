...
```vala
// In do_block(), REMOVE the UNORDERED_LIST and ORDERED_LIST cases (lines that call on_li(false), on_node_int(LIST_ITEM, ...), on_node_int(UNORDERED_LIST|ORDERED_LIST, ...)).
// ADD instead:
...
**2.1** Helper to get list number from list marker (ordered: "1. ", "11. " → 1, 11; unordered/task: 0). Add to `BlockMap` or use inline in the list case:

```vala
// Parse list_number from list_marker for ORDERED_LIST: replace then strip then parse (no character loop).
int list_number_for_marker(FormatType matched_block, string list_marker) {
	if (matched_block != FormatType.ORDERED_LIST) {
		return 0;
	}
	string num_part = list_marker.replace(".", "").strip();
	if (num_part == "") {
		return 1;  // Defensive: invalid ordered marker (e.g. no digits); default to 1.
	}
	return int.parse(num_part);
}
```

**2.2** `libocmarkdown/BlockMap.vala` — in `handle_block_result()`, replace the ORDERED_LIST/UNORDERED_LIST/TASK_LIST/TASK_LIST_DONE case with logic that:
...
Concrete replacement for the list case (structure only — adjust to your exact variable names and helper):
...
case FormatType.TASK_LIST_DONE:
	...
	int list_number = (matched_block == FormatType.ORDERED_LIST) ? list_number_for_marker(matched_block, list_marker) : 0;
...
**2.3** In the same file, where the “End list when we see a non-list line” block runs, change the emit from `do_block(false, this.parser.current_block)` to:
...
**2.4** Parser flush / end of input: when parser is “inside a list” (e.g. `current_block == ORDERED_LIST|UNORDERED_LIST`), emit `on_li(false)` and `do_block(false, FormatType.LIST_BLOCK)` before clearing state. Add or extend `Parser.flush()` (or the path that runs at end of last chunk) accordingly.
...
**4.1** `libocmarkdowngtk/Render.vala` — **remove** the bodies of `on_ul` and `on_ol` (or delete the overrides so base no-ops are used). **Add** `on_list` override and change `on_li` signature and implementation:
...
Remove any logic in the old `on_ul`/`on_ol` that advanced counters; the parser now passes `list_number` per item. Adjust `ensure_indent_level` / `list_stack` so the value at `depth_index` is the **display number** for ordered (parser sends 1, 2, 3…); for unordered keep 0.

**4.2** `libocmarkdown/PangoRender.vala` — **remove** or no-op `on_ul` and `on_ol`. **Add** `on_list`, and change `on_li` to the new signature and drive state from `list_number` and `space_skip`:
...
Ensure `ensure_indent_level` and `list_stack` exist and are keyed by `space_skip` (same pattern as GTK). Remove the old `on_ul`/`on_ol` logic that set list type/counter.

**4.3** `libocmarkdown/HtmlRender.vala` — **remove** or no-op `on_ul` and `on_ol`. **Add** `on_list`; change `on_li` to `(is_start, list_number, space_skip, task_checked)` and open/close `<ul>`/`<ol>` based on `space_skip` depth and `list_number` (0 = ul, &gt;0 = ol), and output `<li>` per item:
...
**5.1** `libocmarkdown/DummyRenderer.vala` — **remove** or no-op `on_ul` and `on_ol` overrides. Add `on_list` override and update `on_li` signature:
