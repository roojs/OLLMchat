...
🚧 **IN PROGRESS** - Phase 1-2 complete, Phase 3-4 pending.
...
### Phase 3: Hide all ollmchat-temp from model lists
- [ ] Update `Client.models()` to filter out models starting with `ollmchat-temp/`
- [ ] Update all model list sources to exclude `ollmchat-temp/` models
- [ ] Update all model dropdown/selection UI components to exclude temp models
- [ ] Verify temp models never appear in model selection UI

### Phase 4: Create on chat when num_ctx is not auto
- [ ] Identify chat entry point(s)
- [ ] Add call to `model.customize(connection, options)` before chat execution
- [ ] Use returned model name for chat execution
- [ ] Test temp model creation when num_ctx is set (not auto)
- [ ] Test reuse of existing temp models
- [ ] Implement error handling: fall back to base model if create fails
- [ ] Add logging for temp model creation and errors
...
### Phase 3
- `Client.models()` (and any other list source): filter out names that are `ollmchat-temp/...` or under `ollmchat-temp/`.
