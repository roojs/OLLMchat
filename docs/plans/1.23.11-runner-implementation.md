...
**Dependencies:** 1.23.2, 1.23.3–1.23.6 (formats), 1.23.1 (existing Runner/Manager/Definition), 1.23.12 (context restriction format), 1.23.13 (task-related code), 1.23.14 (existing skill code). **project_description** source: active project's **summary()** (2.20.5 project-level summaries in vector_metadata); if not completed, summary may be empty.
...
- **PromptTemplate** — Resource location, load_dir() (no arg), template(key). See **1.23.14**.
