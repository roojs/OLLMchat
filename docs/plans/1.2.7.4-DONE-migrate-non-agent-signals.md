...
✅ **DONE** - Third phase of signal migration completed.
...
- [x] `RequestBase` has `agent` property
- [x] `RequestBase` no longer has `chat_call` property
- [x] Tool requests have `agent` property set when created (not `chat_call`)
- [x] All tools access chat_call via `agent.chat` (not `this.chat_call`)
- [x] All tools access permission_provider via `agent.chat.permission_provider` (will be `agent.permission_provider` after 1.2.7.9)
- [x] `RequestEditMode` uses agent property for all communication (no signals)
- [x] `RequestEditMode` still works correctly when processing streaming responses
- [x] All tool execution flows work correctly with agent-based access
- [x] Client signals still exist (for backward compatibility during transition)
...
- [1.2.7.5. Rename exec_chat() to send()](./1.2.7.5-DONE-rename-exec-chat-to-send.md) - Next phase
