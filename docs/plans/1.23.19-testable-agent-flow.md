...
### Bundle 2: refinement_prompt + current file only when in References

**liboccoder/Task/Details.vala**

```vala
private string current_file_block_if_referenced(OLLMcoder.Skill.PromptTemplate tpl)
{
    ...
    foreach (var link in this.reference_targets) {
        if (GLib.Path.is_absolute(link.href) && link.href == file.path) {
            return tpl.header_file("Current File - " + file.path, file);
        }
    }
    return "";
}
...
public OLLMcoder.Skill.PromptTemplate refinement_prompt() throws GLib.Error
{
    ...
    tpl.fill(
        ...
        "current_file", this.current_file_block_if_referenced(tpl),
...
**liboccoder/Task/Details.vala** — in `refine()`, replace the fill so "current_file" uses the helper:
...
```md
If a task needs the current (open) document, add a reference to it in that task's References.
```

**examples/oc-test-skill-agent.vala** — §3.4 and §3.5:

```vala
// §3.4 --refine-task N: resolve task N from parsed task list
var detail = /* task N Details */;
var tpl = detail.refinement_prompt();
stdout.printf("=== system ===\n%s\n=== user ===\n%s\n", tpl.filled_system, tpl.filled_user);

// §3.5 --run-refine N: same then send to model
var tpl = detail.refinement_prompt();
// send (system, user) to model, print response
```
