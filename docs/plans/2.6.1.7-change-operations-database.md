...
Create a new database table to track **file change history** - what happened to each file during command execution. This is organized around **files** (what users understand), not operations (implementation details). Each entry represents a change event in a file's history.

**Key Purpose**: This table is primarily for **user review, approval, and restoration** of changes - not just tracking what happened during command execution. It enables:
...
**Backup/Restoration:**
- `backup_path` TEXT - **Backup file location** (where original file was backed up, empty string "" for new files)
  - Used for restoration if change is rejected
  - For new files: `backup_path` = "" (empty string, not NULL)
  - For modified/deleted files: `backup_path` = path to backup file
...
- Add `file_history` table initialization in `libocfiles/FileBase.vala` (or in FileHistory class)
