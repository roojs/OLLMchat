...
- **🔷** Live object references in notifications use existing **`Notification.id`** + **`object_type`** after caller **`export()`** — see [`8.3.3`](8.3.3-notification-gobject-payload.md) (**withdrawn**; no new `payload` field).
...
- **🚫** [`8.3.3`](8.3.3-notification-gobject-payload.md) — **withdrawn**. Caller `export()` + existing `Notification` `id` / `object_type`; no `payload` property.
- **🔷** `✔️` [`8.3.5`](8.3.5-client-apply-notify-proxies.md) — `Client.proxies`; apply inbound `notify::` as `set_property` from `Notification.message`.
...
- **🔷** `⏳` When the flag is **off**, ollmfilesd wire stays as today.
- **🔷** `⏳` When the flag is **on**, caller **`export()`** + `Notification` **`id`** for live object refs; subscribe and `RPC-Live-Remote.ref` / `RPC-Live-Remote.unref` are valid. ([`8.3.2`](8.3.2-subscribe-unsubscribe.md))
...
- **ℹ️** Nested `Serializable` objects encode **by value**. Live handles use caller **`export()`** and existing notification **`id`** ([`8.3.3`](8.3.3-notification-gobject-payload.md) withdrawn).
