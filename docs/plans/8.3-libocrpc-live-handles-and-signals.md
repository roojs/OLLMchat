...
- **ℹ️** They pass `live_handles: true` when constructing `SocketListen` and `Client`. They call `Live.RemoteParams.rpc_register()` and `Request.register("Remote", new Live.Remote(), typeof(Live.RemoteParams))` at plugin boot, same split as ollmfilesd `Application`. This repo’s ollmfilesd constructors stay at the default. Types live in `OLLMrpc.Live`. Wire prefixes stay `Remote` / `Subscribe`.
