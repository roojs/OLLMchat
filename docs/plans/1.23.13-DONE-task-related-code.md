...
- **API**
  ...
  - **run_all_tasks(Runner runner)** — for each Step: if children.size == 1 yield run_child(…); else num_exec_running = children.size, run_child.begin per child, yield **wait_exec_done()**. After the loop, Runner calls **run_post_completion()** directly (no List method).
