...
### For Step 6.2 (Remove Config from Client):
- [ ] Client no longer has `config` property
- [ ] Config2.create_client() method removed and inlined at call sites
- [ ] Config2.create_chat() method removed (unused)
- [ ] Manager inlines create_client() logic to create base_client
- [ ] Examples inline create_client() logic
- [ ] CodebaseSearchTool gets config from Manager/Session instead of client.config
- [ ] GoogleSearchRequest gets config from Manager/Session instead of client.config
- [ ] BackgroundScan gets config from Manager/Session instead of embedding_client.config
- [ ] All references to `client.config` updated to use alternative approach (e.g., `session.manager.config` or `manager.config`)
- [ ] All functionality still works correctly
- [ ] No regressions in tool usage or vector operations
