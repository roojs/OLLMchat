...
**Status:** ✅ **Phase 1 complete** — `libollamaweb` + offline `ollamaweb` tests; **Phase 2.1** / **2.2** (app integration) not started

**Layout:** `docs/guide-to-writing-plans.md`

**Coding standards:** `.cursor/rules/CODING_STANDARDS.md` — implementers must satisfy the **Checklist for all plans** (nullable avoidance, `this.`, `GLib.*`, multiline docblocks, no `@"` interpolation, etc.). Verify against that checklist before marking implementation complete.
...
- [ ] **⏳** **Remove** `AvailableModels` / `AvailableModel` / `ModelTag` from app (**4.8.1**); bundled JSON unused by Add Model.
