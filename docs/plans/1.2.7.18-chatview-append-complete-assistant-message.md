...
- [ ] Step 1a: `process_new_chunk_direct()` method created and works correctly
- [ ] Step 1a: `end_block_direct()` method created and works correctly
- [ ] Step 1a: `finalize_assistant_message_direct()` method created and works correctly
- [ ] Step 1b: Performance metrics stored as "ui" message in `finalize_streaming()` when response completes
- [ ] Step 1b: "ui" message with metrics is added to messages array and saved
- [ ] Step 1c: `append_complete_assistant_message()` updated to use new direct methods
- [ ] Step 1c: Messages loaded from history (with null `message_interface`) display correctly
- [ ] Step 1c: Thinking content is rendered correctly (using message.thinking)
- [ ] Step 1c: Regular content is rendered correctly (using message.content)
- [ ] Step 1c: Performance metrics are displayed from "ui" role messages in messages array
- [ ] Step 1d: Old methods removed or refactored to use new direct methods

### Step 2: is_active Checks
- [ ] `Session.on_message_created()` only relays to Manager if `is_active`
- [ ] `Session.handle_stream_chunk()` only relays to Manager if `is_active`
- [ ] `Session.handle_stream_chunk()` still increments `unread_count` when inactive
- [ ] `SessionBase.add_message()` only relays to Manager if `is_active`
- [ ] Inactive sessions don't update UI when messages/streaming arrive
- [ ] Active sessions still update UI correctly

### Step 3: Unread Count Display
- [ ] Step 3a: `display_info` updated to include unread count in text
- [ ] Step 3a: `css_classes` property added to SessionBase
- [ ] Step 3a: `css_classes` returns "oc-has-unread" when `unread_count > 0`
- [ ] Step 3a: `css_classes` property is not serialized
- [ ] Step 3a: `notify_property("display_info")` called when `unread_count` changes
- [ ] Step 3a: `notify_property("css_classes")` called when `unread_count` changes
- [ ] Step 3a: `notify_property("display_info")` called in `activate()` when clearing unread_count
- [ ] Step 3a: `notify_property("css_classes")` called in `activate()` when clearing unread_count
- [ ] Step 3b: `css_classes` bound to row_box widget classes in HistoryBrowser
- [ ] Step 3c: Unread label widget added to HistoryBrowser factory in `on_setup_row()`
- [ ] Step 3c: Unread label positioned in secondary_box (after info_label, before date_label)
- [ ] Step 3c: `css_classes` bound to unread_label widget classes
- [ ] Step 3c: CSS styling applied correctly (background color, unread label visibility)
- [ ] Step 3c: UI updates when `css_classes` property changes

### Step 4: Running State Indicator
- [ ] Step 4a: `is_running` property added to SessionBase
- [ ] Step 4a: `is_running` property is not serialized
- [ ] Step 4b: `is_running` set to true in `Session.send()` when starting request
- [ ] Step 4b: `is_running` set to false in `Session.finalize_streaming()` when done
- [ ] Step 4b: `is_running` reset to false if agent not available in `send()`
- [ ] Step 4c: Spinner widget added to HistoryBrowser factory in `on_setup_row()`
- [ ] Step 4c: Spinner positioned in secondary_box
- [ ] Step 4c: `is_running` bound to spinner `visible` property
- [ ] Step 4c: `is_running` bound to spinner `spinning` property
- [ ] Step 4c: Spinner shows and spins when `session.is_running == true`
- [ ] Step 4c: Spinner hides and stops when `session.is_running == false`
...
**Goal**: Display unread_count in the history browser UI to show when sessions have unread messages. The unread count will be shown both in `display_info` text and via a separate "unread" label widget with CSS styling.

#### Step 3a: Update display_info to Include Unread Count

**File**: `libollmchat/History/Session.vala`

**Update `display_info` property** (line 52) to include unread count when there are unread messages:
...
public override string display_info {
    owned get {
        ...
        // Include unread count if there are unread messages
        if (this.unread_count > 0) {
            return "%s - %d %s (%d unread)".printf(
                this.model,
                reply_count,
                reply_count == 1 ? "reply" : "replies",
                this.unread_count
...
**Note**: When `unread_count` changes, `display_info` will automatically update because it's a computed property that reads `unread_count`. However, we need to notify the property change when `unread_count` changes.

**Update unread_count handling** to notify display_info when unread_count changes:
...
if (!this.is_active) {
    this.unread_count++;  // Auto-property setter automatically emits property change notification
    this.notify_property("display_info");  // Notify display_info change when unread_count changes
}
```

**Also update** `SessionBase.activate()` (line 199) to notify display_info when unread_count is cleared:
```vala
this.unread_count = 0; // Clear unread count when activated
this.notify_property("display_info");  // Notify display_info change when unread_count cleared
```

#### Step 3b: Add css_classes Property to SessionBase
...
**Proposed code** (update the code from Step 3a):
```vala
if (!this.is_active) {
    this.unread_count++;  // Auto-property setter automatically emits property change notification
    this.notify_property("display_info");  // Notify display_info change when unread_count changes
...
**Note**: See `docs/reference-gtk-css.md` for GTK CSS property reference and examples. GTK CSS does **not** support `::after` pseudo-elements or the `content` property (these are web CSS features). Instead, we use a separate label widget with CSS `visibility` property to show/hide it based on the parent's `oc-has-unread` class. The `display_info` text will also show the unread count (e.g., "llama3.2 - 3 replies (2 unread)"), and the CSS styling will provide visual highlighting with background color and red/bold text.
