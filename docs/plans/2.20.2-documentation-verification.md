...
- [ ] Use `file.is_documentation()` in `Indexer.index_file()` routing
...
3. **Phase 3: Documentation Indexing** - Implement documentation file processing
...
This phase implements the documentation file processing pipeline.
...
#### 3.3. DocumentationAnalysis Class
...
namespace OLLMvector.Indexing
{
	 ...
	 * 
	 * Processes DocumentationTree objects and generates summaries using LLM
	 * in three levels: document → sections → leaf sections.
	 */
	public class DocumentationAnalysis : VectorBase
	{
		 ...
		 * Constructor.
		 * 
		 * @param resource_path Relative path from RESOURCE_BASE_PREFIX (e.g., "analysis-documentation-prompt.txt")
		 */
		public PromptTemplate(string resource_path)
		{
			this.resource_path = resource_path;
...
**Note**: The PromptTemplate class is defined in Phase 1 and should be imported/used here.
	...
	public class DocumentationAnalysis : VectorBase
	{
		...
		private async void analyze_sections(DocumentationTree tree) throws GLib.Error
		{
			...
			foreach (var section in sections_to_process) {
				...
				// Collect child summaries (build string directly, not array)
				string child_summaries_text = "";
				bool first_child = true;
				foreach (var child in section.children) {
					var child_desc = child.description.strip();
					if (child_desc != "") {
						if (!first_child) {
							child_summaries_text += "\n";
						}
						child_summaries_text += child.element_name + ": " + child_desc;
						first_child = false;
					}
				}
				if (child_summaries_text == "") {
					child_summaries_text = "(no subsections)";
				}
				...
				var user_message = cached_template.fill(
					...
					"child_sections", child_summaries_text
				);
...
#### 3.4. DocumentationVectorBuilder Class
...
#### 3.5. File.is_documentation Method
