...
- [x] **Fix mouse wheel scrolling on dropdown popup passing through to background SourceView** - RESOLVED
  - **Implementation**: Added scroll event controller to `SearchableDropdown` popup (`Gtk.Popover`) that stops scroll event propagation
  - **Solution**: Added `Gtk.EventControllerScroll` to popup that returns `true` from scroll handler and calls `event.stop_propagation()` in `scroll_begin` and `scroll_end` handlers
  - **Location**: `SearchableDropdown` popup widget
