...
- [ ] **Never add `is_ignored` files to project list** (git ignore files should not appear)
...
- [ ] **Exclude non-text files from project list - only show text files**
...
#### File Scanning
- [ ] **Add gitg as dependency to support `is_ignored` functionality**
  - **Steps**:
    1. Add gitg dependency to `meson.build` or build configuration
    2. Use `Gitg.Repository` or similar API to check if file is ignored during scanning
    3. Set `is_ignored` property (already added in previous task) during file scanning in `Folder.read_dir_scan()` or `File.new_from_info()`
    4. `is_ignored` is automatically stored in database when file is saved (property already added)
  - **Dependency**: Requires `is_ignored` property and database column to be added first (see projectlist-exclude-ignored task)
  - **Location**: Build config (`meson.build`), `Folder.vala` (scanning), `File.vala` (if needed)
