...
🔴 **ACTIVE** - Multiple bugs identified, investigation in progress
...
#### File List UI Issues
- [x] **Fix wrong icons in file list** (currently using folder icon for most files) - RESOLVED
  - **Investigation**: In `FileDropdown.create_factory()`, the `file_icon` is bound to `project_file.icon_name` property
  - **Root Cause**: `ProjectFile` was inheriting `FileBase.icon_name` (which defaults to "folder") instead of delegating to `File.icon_name`. Also, `icon_name` was not being stored in the database.
  - **Fix**: 
    1. Changed `icon_name` from virtual property to regular property in `FileBase` with default "folder"
    2. Added `icon_name` column to `filebase` table in database schema
    3. Set `icon_name` during file creation in `File.new_from_info()` using `GLib.ContentType.get_generic_icon_name()`
    4. Added `icon_name` to `copy_db_fields_to()` to persist when updating database
    5. Fixed `ProjectFile` to delegate `icon_name` to wrapped `file.icon_name` using `new` property
    6. Fixed `FileBase.get_target_info()` to request `STANDARD_CONTENT_TYPE` attribute for aliased files
  - **Location**: `FileBase.vala`, `File.vala`, `ProjectFile.vala`, `FileBase.get_target_info()`
  - **Status**: Icons now correctly display based on content type and are persisted in database

- [x] **Fix relative path display in file list** (can't see relative path, need to verify projectfiles bound property) - RESOLVED
  - **Root Cause**: `ProjectFile.display_with_indicators` was delegating to `File.display_with_indicators` instead of showing the relative path
  - **Fix**: 
    1. Changed `FileBase.display_with_indicators` default to return "oops" (should be overridden)
    2. Implemented `ProjectFile.display_with_indicators` to show basename + indicators on first line, relative directory path in grey small text on second line
    3. Enabled `use_markup = true` on label in `FileDropdown` to render pango markup
  - **Location**: `ProjectFile.display_with_indicators`, `FileDropdown.create_factory()`
  - **Status**: Relative path now displays correctly with pango markup

- [x] **Improve text search and sorting in file dropdown** - RESOLVED
  - **Search Requirements**:
    1. Search on basename first, then full path (not just display_name) ✓
    2. Use custom filter to check basename match first, then path match ✓
  - **Sort Requirements**:
    1. **If no search**:
       - Files opened in the last day sorted by most recently opened first ✓
       - Add `is_recent` property to `ProjectFile` (based on `last_viewed` timestamp) ✓
       - Bind CSS class to `ProjectFile.is_recent` for visual indicator ✓
       - Style recent files with light yellow background ✓
       - Rest sorted by name (basename) ✓
    2. **If search**:
       - Matched filenames first (basename matches, with starts_with prioritized) ✓
       - Matched fullpaths second (path matches) ✓
  - **Implementation**: All completed
  - **Location**: `FileDropdown.set_item_model()`, `FileDropdown.create_factory()`, `ProjectFile.is_recent`, CSS file

- [x] **Fix filtered list being empty after clearing text search** - RESOLVED
  - **Fix**: Filter now resets correctly when entry text is cleared or when popup is opened
  - **Implementation**: Added filter reset logic in `set_popup_visible()` and `refresh()` methods
  - **Location**: `FileDropdown.set_popup_visible()`, `FileDropdown.refresh()`
 
- [x] **Fix file list not sorted by basename** - RESOLVED
  - **Fix**: Sorter now uses `display_basename` for sorting instead of full path
  - **Update**: `CustomSorter` comparison function updated to use basename comparison
  - **Location**: `FileDropdown.set_item_model()`, `FileDropdown.create_sorter()`
...
- [ ] **Fix mouse wheel scrolling on dropdown popup passing through to background SourceView**
  ...
  - **Status**: Added scroll event controller to popup - needs verification

-- other criitcal ones
- [x] **Just ask is broken at start - as the provider is null** - RESOLVED
  - **Root Cause**: In `Manager` constructor, `EmptySession` was created before the JustAsk agent was registered. When `EmptySession` calls `new_client()`, it tries to get "just-ask" from `this.agents`, but the agent hasn't been registered yet, so it returns null.
  - **Fix**: Moved JustAsk agent registration to before `EmptySession` creation in `Manager` constructor. Simplified `new_client()` to directly get the agent (no null check needed since agent is always registered first).
  - **Location**: `libollmchat/History/Manager.vala` constructor and `new_client()` method
  - **Status**: Tested and confirmed fixed

** another one
  * selcting project should fill placeholder instantly - currently it pauses while loading the files
  * would be nice to show a loading indicator where the files would be to indicate it's doing that as well

- [ ] **Restore project and file on switching on code editor**
  - **Description**: When switching to the code editor view, the previously selected project and file should be restored
  - **Current Issue**: Project and file selection is lost when switching away from code editor and back
  - **Location**: Code editor integration, project/file state management
  - **Status**: Needs investigation - changes may have been reverted by mistake

- [ ] **File dropdown needs to be able to browse files (search is pretty broken)**
  - **Description**: File dropdown should allow browsing files, and the search functionality needs to be fixed
  - **Current Issues**:
    - Search functionality is broken or not working properly
    - File browsing capabilities are limited
  - **Location**: `FileDropdown`, `SearchableDropdown`
  - **Status**: Needs investigation - changes may have been reverted by mistake
...
- [ ] **Window resize broken** (in progress)
  - **Description**: Coding assistant window does not expand correctly when right pane is shown
  - **Location**: `ollmchat/WindowPane.vala`
  - **Status**: Updated to use `set_default_size()` to explicitly resize window (needs verification)
  - **Solution**: Updated `show_right_pane()` and `hide_right_pane()` to calculate new window size and use `set_default_size()` to resize the window, similar to approach used in roobuilder's WindowManager
...
- [x] **Search files visible without project** - PARTIALLY RESOLVED
...
- [ ] **Improve code type icon selection and display**
  - **Current Issue**: Icon selection for code types is not great - generic icons don't clearly indicate file types
  - **Proposed Improvements**:
    1. **Overlay file extensions**: Consider overlaying file extensions (like `.vala`, `.py`, `.js`) on top of generic icons to make file types more visible
    2. **Better icon selection**: Could use more specific icons for common code file types instead of generic content type icons
    3. **Larger icons**: Icons need to be bigger for better visibility in the file list
  - **Location**: `FileDropdown.create_factory()`, icon rendering and sizing
  - **Considerations**: 
    - May need custom icon rendering with text overlay
    - Icon size configuration in GTK4
    - Balance between icon size and list item height

### 🟢 Tidy / Polish

- [ ] **Make project list items more compact**
  - **Current Issue**: Spacing between file name and directory path is too large, list items take up too much vertical space
  - **Proposed Changes**:
    1. Reduce spacing between basename line and directory path line in `ProjectFile.display_with_indicators`
    2. Make list items more compact by reducing padding/margins in `FileDropdown.create_factory()`
    3. Consider using smaller font size or tighter line spacing for directory path
  - **Location**: `ProjectFile.display_with_indicators`, `FileDropdown.create_factory()`
  - **Goal**: More files visible in dropdown without scrolling

- [ ] **Fix popup width expanding with long filenames**
  - **Current Issue**: Popup width grows and expands when filenames are long, causing layout issues
  - **Proposed Changes**:
    1. Set fixed or maximum width on the popup in `FileDropdown` or `SearchableDropdown`
    2. Ensure text wrapping or ellipsis for long filenames
    3. Keep popup width standard/consistent regardless of content
  - **Location**: `SearchableDropdown` popup creation, CSS styling
  - **Goal**: Consistent popup width that doesn't expand with long filenames

- [ ] **Add search match highlighting in file dropdown**
  - **Current Issue**: No visual indication of which part of the filename/path matches the search
  - **Proposed Approaches**:
    1. **set_data approach**: Store search entry widget reference in ProjectFile via `set_data<Gtk.Entry>()`, use CSS to highlight matches
    2. **CSS approach**: Use CSS selectors to highlight matching text (may require clever CSS)
    3. **display_with_indicators approach**: Update `ProjectFile.display_with_indicators` to include pango markup highlighting when search text changes
  - **Implementation Notes**:
    - Need to track current search text in ProjectFile or pass it when rendering
    - Use pango markup (e.g., `<span background="yellow">match</span>`) to highlight matches
    - Consider performance implications of updating markup on every search change
  - **Location**: `FileDropdown.create_factory()`, `ProjectFile.display_with_indicators`, CSS styling
  - **Goal**: Visual highlighting of search matches in filenames and paths
