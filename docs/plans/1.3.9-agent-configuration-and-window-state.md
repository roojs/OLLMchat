...
- **2.10.4.7** — Active project/file off daemon + Config2 [`Window`](2.10.4.7-URGENT-active-project-file-outside-db.md) map — **owns window chrome persistence** (`project` / `file` / `agent`). This plan must **not** put open project/file on `AgentConfig`.
