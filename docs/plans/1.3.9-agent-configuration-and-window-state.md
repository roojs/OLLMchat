...
- **2.10.4.7** — Active project/file off daemon + Config2 [`Window`](done/2.10.4.7-DONE-active-project-file-outside-db.md) map — **owns** `project` / `file` / `agent` persistence. This plan must **not** put open project/file on `AgentConfig`. Multi-window → [`2.10.4.32`](2.10.4.32-FUTURE-multi-window-support.md).
