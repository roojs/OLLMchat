...
🟡 **IN PROGRESS** - Core data layer and dropdown widgets complete. Source view and integration pending.
...
| Phase 2A: Data Layer (liboccoder) | 🔴 NEEDS REDOING | FileBase, File, Folder (with is_project flag), ProjectFiles, ProjectFile, and ProjectManager classes - architecture updated, needs implementation |
| Phase 2B: Database Schema and Sync | 🟡 PARTIAL | Database methods implemented, missing background sync/watcher |
| Phase 2C: Searchable Dropdown Widgets | 🟡 PARTIAL | SearchableDropdown, ProjectDropdown, FileDropdown implemented. Sections/separators for recent items pending |
| Phase 3: Source View Component | ⏳ TODO | SourceView component with header bar and code editor (in liboccoder, not libollmchatgtk) |
...
🔴 **NEEDS REDOING** - Architecture has been updated. Basic data layer library exists but needs significant refactoring to match new design.

**Status**: Existing implementation needs to be updated to match the new architecture described below.

#### 🔴 Critical Updates Required

**Architecture Changes:**
1. **Remove Project class** - Merge all functionality into Folder with `is_project` flag
2. **Create ProjectFiles class** - New class to handle all async file I/O and manage tree structure + hashmap
3. **Add alias properties to FileBase** - `is_alias`, `points_to`, `points_to_id` for symlink tracking
4. **Move async operations** - Move `read_dir()` from Folder to ProjectFiles
5. **Update database schema** - Add alias fields and update base_type handling
6. **Exclude .git directories** - Skip hidden/system directories during scanning
7. **Fix alias parent handling** - Don't set target folder's parent to alias source, use lookup instead

**Files Requiring Updates:**
- ✅ `FileBase.vala` - Add alias properties, update schema
- ✅ `File.vala` - Remove `is_project` flag (only folders can be projects)
- ✅ `Folder.vala` - Add `is_project` flag, add `project_files`, remove `read_dir()`, merge Project methods
- ✅ `ProjectFiles.vala` - **CREATE NEW** - Handle all async I/O, scanning, alias handling, exclude .git
- ✅ `ProjectManager.vala` - Update to use Folder instead of Project
- ✅ `ProjectDropdown.vala` - Update types to use Folder
- ✅ `FileDropdown.vala` - Update to use `project_files` instead of `all_files`
- ✅ `Project.vala` - **REMOVE** - Delete this file
- ✅ `SourceView.vala` - Update Project references to Folder (if exists)
- ✅ `ProjectMigrate.vala` - Update to create Folder with `is_project = true` (if creates Project objects)
...
- **Classes**:
  ...
  - `OLLMcoder.Files.ProjectFile` - Represents a file within a project (tracks alias/target relationship)
...
- `is_alias` (bool) - **Computed property**: Whether this is an alias/symlink. Returns true if this is a FileAlias or FolderAlias. (Database still stores is_alias for queries, but property is read-only computed)
- `points_to` (FileBase?) - Reference to the FileBase object this alias points to (nullable, only set when `is_alias = true`). This is the actual object reference (pointer), not a path string. Loaded from database via `points_to_id` or set directly when target object is known.
- `points_to_id` (int64) - The ID of the FileBase object this alias points to (stored in database, default: 0, foreign key reference). Used to load `points_to` object reference from database.
...
- Alias properties (`is_alias`, `points_to`, `points_to_id`) allow tracking symlinks and aliases in the file system.
- When `is_alias = true`, `points_to` is a reference to the target FileBase object (loaded from database via `points_to_id`).
- `points_to_id` stores the database ID of the target object for persistence and loading.
- For alias objects, use FileAlias or FolderAlias wrapper classes that delegate properties to `points_to`.

##### OLLMcoder.Files.FileAlias (extends File)
- Wrapper class for File objects that are aliases/symlinks
- Properties delegate to `points_to` (the actual File object this alias points to, which is `FileBase?` cast to `File`)
- **Delegated properties** (read from `points_to`):
  - `language` - Programming language
  - `text_buffer` - Text buffer
  - `cursor_line`, `cursor_offset`, `scroll_position` - Cursor/scroll state
  - `last_viewed` - Last viewed timestamp
  - `needs_approval`, `is_unsaved` - Approval/unsaved state (needs_approval: true = needs approval, false = approved)
  - `last_approved_copy_path` - Last approved copy path
  - `read_async()`, `write()` - File I/O methods delegate to target (read() removed, use async version)
- **Own properties** (not delegated):
  - `is_alias` - Always true
  - `points_to` - Reference to target File object (FileBase? cast to File)
  - `points_to_id` - Target file ID (for database persistence)
  - `path` - The alias path (not the target path)
  - `parent` - The alias's parent folder

**Note**: 
- FileAlias wraps a File that is an alias/symlink.
- Most properties delegate to the target File object (`points_to`).
- The alias maintains its own path and parent (where the alias exists), but content and state come from the target.

##### OLLMcoder.Files.FolderAlias (extends Folder)
- Wrapper class for Folder objects that are aliases/symlinks
- Properties delegate to `points_to` (the actual Folder object this alias points to, which is `FileBase?` cast to `Folder`)
- **Delegated properties** (read from `points_to`):
  - `children` - Children list (FolderChildren)
  - `is_project` - Whether target is a project
  - `project_files` - ProjectFiles manager (if target is project)
  - `last_viewed` - Last viewed timestamp
  - `last_check_time` - Last check time
- **Own properties** (not delegated):
  - `is_alias` - Always true
  - `points_to` - Reference to target Folder object (FileBase? cast to Folder)
  - `points_to_id` - Target folder ID (for database persistence)
  - `path` - The alias path (not the target path)
  - `parent` - The alias's parent folder

**Note**: 
- FolderAlias wraps a Folder that is an alias/symlink.
- Most properties delegate to the target Folder object (`points_to`).
- The alias maintains its own path and parent (where the alias exists), but children and project state come from the target.
...
- `is_project` flag allows files to be marked as projects (though typically projects are folders).
- **Methods**: 
  - `read_async()` - Read file contents asynchronously (synchronous read() removed)
  - `write(string)` - Write file contents
- **Signals**: `changed` - Emitted when file content changes

**Note**: Files can be in multiple projects (due to softlinks/symlinks). Alias/symlink information is tracked via FileBase properties (`is_alias`, `points_to`, `points_to_id`), and file-to-project relationships are tracked via ProjectFile objects.

##### OLLMcoder.Files.FolderChildren (extends GLib.ListStore)
- Extends `GLib.ListStore<OLLMcoder.Files.FileBase>` to manage folder children with integrated map
- `name_map` (Gee.HashMap<string, OLLMcoder.Files.FileBase>) - Hashmap of basename => FileBase for quick lookup by filename
- **Methods**:
  - `append(OLLMcoder.Files.FileBase item, string basename)` - Add item to list and map (overrides ListStore.append)
  - `remove(uint position)` - Remove item from list and map (overrides ListStore.remove)
  - `remove_all()` - Clear both list and map (overrides ListStore.remove_all)
  - `get_by_name(string basename)` - Get FileBase by basename from map (returns FileBase or null)

**Note**: 
- FolderChildren combines ListStore functionality with a basename lookup map.
- The map stores basename (filename in directory) => FileBase for O(1) lookup.
...
- `children` (OLLMcoder.Files.FolderChildren) - FolderChildren ListStore that manages children (files and subfolders) with integrated basename map - used for tree view hierarchy
- `project_files` (OLLMcoder.Files.ProjectFiles?) - ProjectFiles manager for this project (only set when `is_project = true`, nullable). Handles async scanning, loading, and synchronization.
- `all_files` (GLib.ListStore<OLLMcoder.Files.File>) - **DEPRECATED** - Flat ListStore of all files (replaced by `project_files`, kept for backward compatibility)
- `last_viewed` (int64) - Unix timestamp of last view (stored in database, default: 0, used for projects)
- `last_check_time` (int64) - Last check time for this folder (prevents re-checking during recursive scans)
- **Methods**:
  - `get_active_file()` - Get the currently active file in this project (only when `is_project = true`)
- **Signals**: 
  - `child_added` - Emitted when a child is added
  - `child_removed` - Emitted when a child is removed

**Note**: 
- Folder uses `FolderChildren` (extends ListStore) which combines list and map functionality - maintains both ordered list and basename lookup map.
- `children` is used for the tree view hierarchy - folders contain their child files and subfolders.
- `children.get_by_name(basename)` provides O(1) lookup by filename.
- When `is_project = true`, Folder acts as a project with special project management capabilities.
- Projects use `project_files` (ProjectFiles class) to handle all async file I/O, scanning, loading, and synchronization.
- The async `read_dir()` method is moved to ProjectFiles - ProjectFiles handles all directory scanning and file system operations.
- Folder.children provides the hierarchical structure for tree views, while ProjectFiles manages the overall project file structure and synchronization.
- The `all_files` property is deprecated but kept for backward compatibility; new code should use `project_files`.
- Tracks active state (via `is_active` from FileBase) and last viewed timestamp for showing recent projects in dropdowns.
- Project functionality (previously in separate Project class) is now merged into Folder.

##### OLLMcoder.Files.ProjectFile
- `id` (int64) - Database ID
- `file_id` (int64) - Reference to the real File object (foreign key)
- `path` (string) - Alias path (the path within the project)
- `alias` (string) - Alias name/identifier
- `target` (string) - Target path (the real file path that this alias points to)
- `last_accessed` (int64) - Unix timestamp of last access (stored in database, default: 0)

**Note**: 
- ProjectFile represents a file's relationship to a project, tracking file-to-project associations.
- Allows the same file to appear in multiple projects with different paths/aliases.
- Replaces the previous `alias_map` approach with explicit relationship tracking.
- Used to track which files belong to which projects and how they're referenced.
- Note: Individual file/folder alias/symlink information is tracked via FileBase properties (`is_alias`, `points_to`, `points_to_id`).

##### OLLMcoder.Files.ProjectFiles
- Implements `GLib.ListModel` interface to manage project files with tree structure and handles all async file I/O operations
- Uses `Gee.ArrayList<OLLMcoder.Files.FileBase>` as backing store (private `items` property) - implements ListModel interface
- `file_map` (Gee.HashMap<string, OLLMcoder.Files.File>) - Hashmap of full path (full name) => File object for all files in project (for O(1) quick lookup by full path)
- `project` (OLLMcoder.Files.Folder) - Reference to the project folder (folder with `is_project = true`)
...
- **Methods**:
  - `async scan_project()` - Scan the entire project directory asynchronously, populating Folder.children and updating tree structure and hashmap. **Excludes hidden/system directories**: Skips `.git` directories and other hidden/system folders from scanning.
  - `async read_dir(OLLMcoder.Files.Folder folder, int64 check_time, bool recurse = false)` - Load children from filesystem for a folder (moved from Folder.read_dir()). Handles async file enumeration, symlink resolution, and updates Folder.children. **Excludes hidden/system directories**: Skips `.git` directories and other hidden/system folders. **Alias handling**: When encountering a symlink/alias, creates FileAlias/FolderAlias objects when target exists, sets `points_to` (FileBase object reference), `points_to_id`. For folder aliases, scans the target folder but does NOT set the target folder's parent to the alias source (performs lookup to find real parent).
  - `add_file(OLLMcoder.Files.File file, string full_path)` - Add file to both tree structure (at appropriate parent location) and hashmap
  - `remove_file(string full_path)` - Remove file from both tree structure and hashmap
  - `get_file(string full_path)` - Get file by full path from hashmap (O(1) lookup)
  - `get_tree_model()` - Get the tree-structured ListModel (returns self, implements ListModel interface)
  - `get_flat_file_list()` - Get a flat ListStore of all files (for dropdowns, derived from hashmap)
  - `populate_from_folder()` - Populate tree structure and hashmap from project folder's children hierarchy
  - `sync_with_filesystem()` - Synchronize project files with filesystem (detect changes, additions, removals)
  - `clear()` - Clear both tree structure and hashmap

**Note**: 
- ProjectFiles implements `GLib.ListModel` interface using `Gee.ArrayList<FileBase>` as the backing store.
- ProjectFiles manages both a hierarchical tree structure (for UI tree views) and a flat hashmap (for fast lookups by full path).
- **All async file I/O operations are handled by ProjectFiles**, including directory scanning, file enumeration, and filesystem synchronization.
- The tree structure uses Folder.children for the hierarchical structure - each Folder maintains its own children list, and ProjectFiles builds the items ArrayList from these relationships.
- The hashmap (`file_map`) provides O(1) lookup by full path (full name) for all files in the project, regardless of their position in the tree.
- The tree structure reflects the folder hierarchy: folders contain their child files and subfolders (via Folder.children), making it suitable for tree view widgets.
- Used by projects (folders with `is_project = true`) to manage their file structure efficiently and handle all filesystem operations.
- Replaces the flat `all_files` ListStore with a more structured approach that supports both tree views (hierarchical) and flat views (for dropdowns).
- **Separation of concerns**: Folder.children provides the hierarchical structure for tree views, while ProjectFiles handles all async operations, synchronization, and provides the flattened hashmap for quick lookups.
- **ListModel implementation**: ProjectFiles itself can be used directly as a ListModel in GTK widgets, eliminating the need for a separate ListStore wrapper.
- **Alias/symlink handling**: When scanning and encountering a symlink/alias:
  - Set `is_alias = true`, `points_to = symlink_target`, and `points_to_id = target_file_id` on the alias object
  - For folder aliases: Scan the target folder, but do NOT set the target folder's parent to the alias source (the alias source is not the real parent)
  - Perform a lookup (via path_map or filesystem) to find the real parent of the target folder
  - The target folder may not have a parent_id set initially - this is correct, as it should reflect the actual filesystem structure, not the alias structure
- **Hidden/system directory exclusion**: `.git` directories and other hidden/system folders are excluded from scanning. These directories are skipped during `read_dir()` and `scan_project()` operations to avoid cluttering the project file tree with version control and system files.
...
- **Multi-project files**: Files can belong to multiple projects (via softlinks/symlinks), managed through ProjectFile relationship tracking
- **GTK-specific properties**: `text_buffer` and `sourceview` are nullable and only set when file is open in GTK UI
- **Project/Folder unification**: Projects are now folders with `is_project = true`, eliminating the need for a separate Project class
- **ProjectFile relationships**: Explicit ProjectFile objects track file-to-project relationships, replacing the previous alias_map approach
- **ProjectFiles class**: New wrapper class that manages project files with both tree structure (hierarchical ListStore) and hashmap (full path => File) for efficient access. The tree structure maintains parent-child relationships for UI tree views, while the hashmap provides O(1) lookup by full path. Replaces the flat `all_files` ListStore with a more structured approach.
- **Async file I/O centralization**: All async file I/O operations (directory scanning, file enumeration, synchronization) are handled by ProjectFiles, not Folder. Folder.children provides the hierarchical structure for tree views, while ProjectFiles manages all filesystem operations and synchronization.
- **Tree view structure**: The tree view uses Folder.children (FolderChildren ListStore) for the hierarchical structure - each Folder maintains its children list with integrated basename map, and ProjectFiles builds the tree ListStore from these relationships. No separate "project folder" object needed.
- **FolderChildren class**: New class that extends ListStore and integrates basename map for O(1) lookup. Replaces separate `children` (ArrayList) and `child_map` (HashMap) with unified FolderChildren that maintains both.
- **Alias/symlink scanning**: When scanning directories and encountering aliases/symlinks, create FileAlias or FolderAlias wrapper objects that delegate properties to the target. The alias object is marked with `is_alias = true` and points to the target via `points_to_obj`. For folder aliases, the target folder is scanned but its parent is determined by lookup (not set to the alias source), preserving the actual filesystem structure.
- **FileAlias and FolderAlias wrapper classes**: New wrapper classes that extend File/Folder and delegate most properties to `points_to_obj` (the target object). The alias maintains its own path and parent (where the alias exists), but content, state, and children come from the target object.
- **Hidden directory exclusion**: `.git` directories and other hidden/system folders are excluded from project scanning to keep the file tree clean and focused on user code files.
...
- **Static stores**:
  ...
  - `projects` (Gee.ArrayList<OLLMcoder.Files.Folder>) - List of all projects (folders where `is_project = true`)
- Basic methods for accessing projects, files, and folders
- Methods to handle file changes (rename, move) and update ProjectFile relationships
- Import initial data from ProjectMigrate (Phase 1.5)
- Note: Database persistence and sync will be added in Phase 2B

**Note**: ProjectFile objects explicitly track file-to-project relationships, making it easy to find all project references when a file changes (e.g., rename). This replaces the previous alias_map approach with a more structured relationship model.
...
- `liboccoder/files/FileAlias.vala` - FileAlias class (namespace: OLLMcoder.Files) - wrapper for File aliases that delegates to points_to_obj
- `liboccoder/files/FolderChildren.vala` - FolderChildren class (namespace: OLLMcoder.Files) - extends ListStore with integrated basename map
- `liboccoder/files/Folder.vala` - Folder class (namespace: OLLMcoder.Files, extends FileBase, has `is_project` flag, merged with Project functionality)
- `liboccoder/files/FolderAlias.vala` - FolderAlias class (namespace: OLLMcoder.Files) - wrapper for Folder aliases that delegates to points_to_obj
- `liboccoder/files/ProjectFile.vala` - ProjectFile class (namespace: OLLMcoder.Files) - tracks file-to-project relationships
- `liboccoder/files/ProjectFiles.vala` - ProjectFiles class (namespace: OLLMcoder.Files) - wraps ListStore and manages project files with tree structure and hashmap
...
1. **`liboccoder/files/Project.vala`** - **REMOVE** 
   - Project class functionality merged into Folder
   - All references to `OLLMcoder.Files.Project` need to be changed to `OLLMcoder.Files.Folder` with `is_project = true`

2. **`liboccoder/files/FileBase.vala`** - **MODIFY** 
   - ✅ Add `is_alias` (bool) property - whether this is an alias/symlink
   - ✅ Add `points_to` (string) property - path this alias points to
   - ✅ Add `points_to_id` (int64) property - ID of FileBase object this alias points to
   - ✅ Update `initDB()` to include alias fields in database schema: `is_alias INTEGER`, `points_to TEXT`, `points_to_id INT64`
   - ✅ Update `base_type` handling to use `is_project` flag instead of separate Project type (remove "p" type, use "d" with is_project flag)

3. **`liboccoder/files/File.vala`** - **MODIFY**
   - ✅ Add `is_project` (bool) property - whether this file represents a project

5. **`liboccoder/files/FolderChildren.vala`** - **CREATE** (NEW FILE)
   - ✅ New class that extends GLib.ListStore<FileBase>
   - ✅ Add `name_map` (Gee.HashMap<string, FileBase>) property - basename => FileBase map
   - ✅ Override `append()` to add to both list and map (takes item and basename)
   - ✅ Override `remove()` to remove from both list and map
   - ✅ Override `remove_all()` to clear both list and map
   - ✅ Add `get_by_name(string basename)` method - returns FileBase from map

6. **`liboccoder/files/Folder.vala`** - **MODIFY**
   - ✅ Change `children` from Gee.ArrayList to FolderChildren
   - ✅ Remove `child_map` property (now integrated in FolderChildren)
   - ✅ Update all references to `child_map` to use `children.name_map` or `children.get_by_name()`
   - ✅ Add `is_project` (bool) property - whether this folder represents a project
   - ✅ Add `project_files` (OLLMcoder.Files.ProjectFiles?) property - ProjectFiles manager (only set when `is_project = true`)
   - ✅ Remove `async read_dir()` method - moved to ProjectFiles
   - ✅ Merge Project methods: `get_active_file()` (from Project class)
   - ✅ Mark `all_files` as deprecated (keep for backward compatibility)
   - ✅ Add `last_check_time` property if not already present

7. **`liboccoder/files/ProjectFiles.vala`** - **CREATE** (NEW FILE)
   - ✅ New class that handles all async file I/O operations
   - ✅ Properties: `list_store` (GLib.ListStore), `file_map` (Gee.HashMap), `project` (Folder reference)
   - ✅ Methods:
     - `async scan_project()` - Scan entire project, exclude .git directories
     - `async read_dir(Folder folder, int64 check_time, bool recurse)` - Handle async file enumeration, symlink resolution, exclude .git
     - Alias handling: Set `is_alias = true`, `points_to` (FileBase object reference from path_map), `points_to_id` on alias objects
     - For folder aliases: Scan target but don't set parent to alias source (do lookup for real parent)
     - Update to use `folder.children.append(item, basename)` instead of separate list and map operations
     - When creating aliases, look up target object in path_map and set `points_to` to the object reference
     - `add_file()`, `remove_file()`, `get_file()`, `get_tree_store()`, `get_flat_file_list()`, `populate_from_folder()`, `sync_with_filesystem()`, `clear()`

8. **`liboccoder/ProjectManager.vala`** - **MODIFY**
   - ✅ Change `projects` list type from `Gee.ArrayList<OLLMcoder.Files.Project>` to `Gee.ArrayList<OLLMcoder.Files.Folder>`
   - ✅ Update `active_project` type from `OLLMcoder.Files.Project?` to `OLLMcoder.Files.Folder?`
   - ✅ Update all methods that reference Project class to use Folder with `is_project = true`
   - ✅ Update signal types: `active_project_changed(OLLMcoder.Files.Folder? project)`

9. **`liboccoder/ProjectDropdown.vala`** - **MODIFY**
   - ✅ Change `selected_project` type from `Files.Project?` to `Files.Folder?`
   - ✅ Update signal: `project_selected(Files.Folder? project)`
   - ✅ Filter projects by `is_project = true` when populating list

10. **`liboccoder/FileDropdown.vala`** - **MODIFY**
   - ✅ Change `project` property type from `Files.Project?` to `Files.Folder?`
   - ✅ Update to use `project.project_files.get_flat_file_list()` instead of `project.all_files`
   - ✅ Handle case where `project_files` might be null

11. **`liboccoder/SourceView.vala`** - **MODIFY** (if exists)
   - ✅ Update all references to `OLLMcoder.Files.Project` to `OLLMcoder.Files.Folder`
   - ✅ Update method signatures that take Project parameter to take Folder instead

12. **`liboccoder/ProjectMigrate.vala`** - **MODIFY** (if creates Project objects)
    - ✅ Update to create Folder objects with `is_project = true` instead of Project objects
...
🟡 **PARTIALLY DONE** - Database integration implemented: ProjectManager has `db` property, FileBase has `initDB()` and `saveToDB()` methods with sync parameter. Still missing: background sync process and file system watcher for real-time updates.
...
🟡 **PARTIALLY DONE** - SearchableDropdown base class, ProjectDropdown, and FileDropdown widgets implemented. Still need: sections/separators for "recent" vs "rest" items.
...
#### FileDropdown Tree View Tab (Future Enhancement)
- **Tree View Tab**: FileDropdown popup will have a separate tab showing a nested tree view of the project structure
- **Data Source**: Uses `project_files.get_tree_store()` which reflects the Folder.children hierarchy
- **Purpose**: Allows users to browse files in a hierarchical tree structure (folders and subfolders) in addition to the flat searchable list
- **Implementation**: Will use Gtk.TreeView or similar tree widget bound to the tree-structured ListStore from ProjectFiles
...
⏳ **TODO** - SourceView component not yet implemented.
...
- **ProjectManager**: Query files from active project's `project_files` (or `all_files` for backward compatibility)
...
- `open_project(OLLMcoder.Files.Folder project)` - Switch to project (calls manager.activate_project(), project must have is_project = true)
- `get_current_file()` - Return currently active File object (from manager)
- `get_current_project()` - Return currently active project Folder object (from manager, Folder with is_project = true)
