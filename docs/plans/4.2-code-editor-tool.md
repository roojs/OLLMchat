...
| Phase 2A: Data Layer (liboccoder) | ✅ DONE | FileBase, File, Folder (with is_project flag), ProjectFile, and ProjectManager classes |
...
✅ **DONE** - Basic data layer library created with FileBase, File, Folder, ProjectFile, and ProjectManager classes.
...
**Note**: Files can be in multiple projects (due to softlinks/symlinks). All alias references are tracked via ProjectFile relationships.
...
- `all_files` (GLib.ListStore<OLLMcoder.Files.File>) - ListStore of all files in project (used by dropdowns, only populated when `is_project = true`)
...
- **Methods**:
  ...
  - `load_files_from_db(SQ.Database)` - Load project files from database into `all_files` ListStore (only when `is_project = true`)
  - `populate_all_files_from_children()` - Populate `all_files` from folder hierarchy (only when `is_project = true`)
  - `async scan_files()` - Scan project directory for files asynchronously (only when `is_project = true`)
- **Signals**: 
  - `child_added` - Emitted when a child is added
  - `child_removed` - Emitted when a child is removed

**Note**: 
- Folder maintains a list of its children and a hashmap for quick lookup by filename. Emits signals when children are added/removed.
- When `is_project = true`, Folder acts as a project with special project management capabilities.
- Projects maintain a flat list of all files (`all_files`) for efficient dropdown population.
...
**Note**: 
- ProjectFile represents a file's relationship to a project, tracking alias/symlink information.
...
- Used to track which files belong to which projects and how they're referenced (via symlinks/aliases).
...
- `liboccoder/files/Folder.vala` - **MODIFY** - Add `is_project` flag and merge Project methods (`all_files`, `get_active_file()`, `load_files_from_db()`, `populate_all_files_from_children()`, `scan_files()`)
- `liboccoder/files/File.vala` - **MODIFY** - Add `is_project` flag
- `liboccoder/files/FileBase.vala` - **MODIFY** - Update `base_type` handling to use `is_project` flag instead of separate Project type
