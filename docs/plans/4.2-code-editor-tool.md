...
- **Classes**:
  - `OLLMcoder.Files.File` - Represents a file (extends `FileBase`, has `is_project` flag)
...
- `is_alias` (bool) - Whether this is an alias/symlink (stored in database, default: false)
- `points_to` (string) - The path this alias points to (stored in database, default: empty string, only set when `is_alias = true`)
...
- When `is_alias = true`, `points_to` contains the target path and `points_to_id` references the target FileBase object.

##### OLLMcoder.Files.File (extends FileBase)
- Inherits all properties from `OLLMcoder.Files.FileBase`
- `is_project` (bool) - Whether this file represents a project (stored in database, default: false)
...
- `children` (Gee.ArrayList<OLLMcoder.Files.FileBase>) - List of children (files and subfolders) - used for tree view hierarchy
...
**Note**: 
- Folder maintains a list of its children (`children`) and a hashmap (`child_map`) for quick lookup by filename. Emits signals when children are added/removed.
...
- Wraps a GLib.ListStore to manage project files with tree structure and handles all async file I/O operations
- `list_store` (GLib.ListStore<OLLMcoder.Files.FileBase>) - Tree-structured ListStore containing files and folders (hierarchical, with children). The tree structure reflects the folder hierarchy using Folder.children for parent-child relationships.
...
- **Methods**:
  ...
  - `get_tree_store()` - Get the tree-structured ListStore (for UI binding to tree views)
...
- The tree structure uses Folder.children for the hierarchical structure - each Folder maintains its own children list, and ProjectFiles builds the tree ListStore from these relationships.
...
- **Tree view structure**: The tree view uses Folder.children for the hierarchical structure - each Folder maintains its children list, and ProjectFiles builds the tree ListStore from these relationships. No separate "project folder" object needed.
- **Alias/symlink scanning**: When scanning directories and encountering aliases/symlinks, the alias object is marked with `is_alias = true` and points to the target. For folder aliases, the target folder is scanned but its parent is determined by lookup (not set to the alias source), preserving the actual filesystem structure.
...
- `liboccoder/files/File.vala` - File class (namespace: OLLMcoder.Files, extends FileBase, has `is_project` flag)
...
4. **`liboccoder/files/Folder.vala`** - **MODIFY**
   ...
   - ✅ Keep `children` (Gee.ArrayList) and `child_map` for tree view hierarchy
...
5. **`liboccoder/files/ProjectFiles.vala`** - **CREATE** (NEW FILE)
   ...
   - ✅ Methods:
     ...
     - Alias handling: Set `is_alias = true`, `points_to`, `points_to_id` on alias objects
...
6. **`liboccoder/ProjectManager.vala`** - **MODIFY**
...
7. **`liboccoder/ProjectDropdown.vala`** - **MODIFY**
...
8. **`liboccoder/FileDropdown.vala`** - **MODIFY**
...
9. **`liboccoder/SourceView.vala`** - **MODIFY** (if exists)
...
10. **`liboccoder/ProjectMigrate.vala`** - **MODIFY** (if creates Project objects)
...
- Fields to track:
  ...
  - **Alias/symlink tracking**: `is_alias` (INTEGER), `points_to` (TEXT), `points_to_id` (INT64) for tracking symlinks and aliases
...
- `File.is_approved` - Whether file has been approved (already exists in File class)
...
- After changes applied, mark file as needing review (`is_approved = false`)

**Review List Display:**
- Query files where `is_approved = false` and `last_approved_copy_path != ""`
...
- Allow user to:
  ...
  - Approve changes (update `last_approved_copy_path`, set `is_approved = true`)
...
- `liboccoder/files/File.vala` - Already has `last_approved_copy_path` and `is_approved` properties
