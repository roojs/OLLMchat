...
1. **Agent provides UI component**: Agent has method/property to get its UI component (e.g., `get_ui_component()` or `ui_component` property)
   - CodeAssistant agent creates and manages its own ProjectManager and SourceView
   - Agent returns the UI widget when asked
2. Window calls `agent.get_widget()` and checks if result is not null
3. Add agent's UI component to WindowPane.tab_view in Window.vala (WindowPane exposes tab_view, Window is responsible for adding widgets)
