...
- **Methods** (⚠️ **NOT YET IMPLEMENTED**):
  - `sync_with_filesystem()` - ⚠️ Not implemented
  - `populate_from_folder()` - ⚠️ Not implemented
  - `clear()` - ⚠️ Not implemented
...
- ⚠️ **Some methods not yet implemented**: `sync_with_filesystem()`, `populate_from_folder()`, `clear()`.
...
- ⚠️ **FileDropdown needs update**: Currently references `get_flat_file_list()` which doesn't exist. Should use ProjectFiles directly as ListModel.
...
1. **ProjectFiles missing methods** - ⚠️ **NOT IMPLEMENTED**
   - `sync_with_filesystem()` - Synchronize project files with filesystem
   - `populate_from_folder()` - Populate from project folder's children hierarchy
...
8. **Folder.read_dir_update() has syntax errors and incomplete logic** - ⚠️ **BUGS**
   - Line 211: Extra closing parenthesis - `this.manager.file_cache.get(new_item.path))` should be `this.manager.file_cache.get(new_item.path)`
   - Line 212: Incomplete assignment - `new_item.points_to = new_item.points_to.id` should be `new_item.points_to_id = new_item.points_to.id` (and points_to should be FileBase, not int64)
   - Line 210: Logic error - checks `file_cache.has_key(new_item.path)` but should check `target_path` (alias path vs target path)
   - Line 217: Calls `new_item.points_to.saveToDB()` but `points_to` may be null (condition checks `points_to != null` but else branch doesn't guarantee it)
   - **Impact**: Alias resolution during directory scanning will fail with syntax errors
   - **Fix needed**: Fix syntax errors, correct alias resolution logic

9. **Folder.load_children() references undefined variable** - ⚠️ **BUG**
   - Line 382: References `file_base.target_id` which doesn't exist (should be `file_base.points_to_id`)
   - **Impact**: Database loading of aliased files will fail
   - **Fix needed**: Change `file_base.target_id` to `file_base.points_to_id`
