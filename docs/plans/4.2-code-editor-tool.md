...
🟡 **IN PROGRESS** - Core data layer and dropdown widgets complete. SourceView component implemented (~85%) but **NOT INTEGRATED** into main window. WindowPane exists but SourceView is not instantiated or added to it.

**Progress:** 5 of 11 phases complete (45%), 1 partially complete (Phase 3 at ~85% - implementation done, integration missing)
...
- ❌ **Integration Missing**: SourceView not instantiated in Window.vala, not added to WindowPane
- ❌ **State Restoration Not Called**: `apply_manager_state()` exists but not called from view switching code
- ⏳ **Pending Integration**: Need to connect SourceView to WindowPane and call state restoration when switching to code assistant view
...
| Phase 3: Source View Component | 🟡 PARTIAL | SourceView component implemented (~85% complete). Core functionality done: buffer switching, file/project switching, state persistence, search functionality. **NOT INTEGRATED**: Not instantiated in Window.vala, not added to WindowPane. Missing: diagnostics, user error dialogs, integration with main window |
| Phase 4: Configuration Persistence | ⏳ FUTURE | Save/restore UI state preferences |
| Phase 5: CodeAssistant Integration | ⏳ TODO | Integrate CodeAssistant with ProjectManager and editor |
...
- ⏳ `ollmchat/Window.vala` - **NEEDS UPDATE**: 
  ...
  - If widget is not null, cast to `Gtk.Widget?` and add to WindowPane.tab_view
  - Show/hide WindowPane based on whether widget is null
  - WindowPane exposes tab_view, Window is responsible for adding widgets
- ⏳ `libocagent/BaseAgent.vala` - **NEEDS UPDATE**: 
  - Add virtual method `get_widget()` returning `Object?` (default: null) - agents with UI override this
  - **No GTK types here** - BaseAgent stays GTK-free, uses Object? for widget
  - Agents without UI return null (default implementation)
- ⏳ `liboccoder/Prompt/CodeAssistant.vala` - **NEEDS UPDATE**: 
  - Override `get_widget()` to create and return SourceView with ProjectManager (cast as Object)
  - **Agent creates ProjectManager**: 
    - Create database instance (or reuse from Manager if available)
    - Create ProjectManager with database: `var project_manager = new ProjectManager(db)`
    - Store ProjectManager as instance variable for reuse
  - **Agent creates SourceView**: 
    - Create SourceView with ProjectManager: `var source_view = new SourceView(project_manager)`
    - Store SourceView as instance variable for reuse
  - **Agent handles initialization** (when widget is first created):
    - Load projects from database: `yield project_manager.load_projects_from_db()`
    - Restore active state: `yield project_manager.restore_active_state()`
    - Apply UI state: `yield source_view.apply_manager_state()`
  - **Lazy initialization**: 
    - Create widget on first call to `get_widget()`, reuse on subsequent calls
    - Check if widget already exists, return cached instance if available
  - Returns null if UI creation fails or if error occurs
- ⏳ `libollmchat/History/Manager.vala` - **NEEDS UPDATE**: Add `agent_activated(OLLMagent.BaseAgent agent)` signal (Manager is the signal hub, avoids circular dependencies)
- ⏳ `libollmchatgtk/ChatInput.vala` - **NEEDS UPDATE**: Emit `manager.agent_activated(agent)` signal when agent dropdown selection changes (after setting session.agent_name and session.client.prompt_assistant)
...
- ⚠️ Editor state restoration implemented but **NOT CALLED** from view switching code
...
1. **Agent provides UI component**: Agent implements `get_widget()` method returning `Object?`
   - CodeAssistant agent creates and manages its own ProjectManager and SourceView
   - **Agent creates ProjectManager**: Agent creates database instance and ProjectManager: `var project_manager = new ProjectManager(db)`
...
2. Window calls `agent.get_widget()` and checks if result is not null
3. If widget is not null, Window casts to `Gtk.Widget?` and adds to WindowPane.tab_view (WindowPane exposes tab_view, Window is responsible for adding widgets)
4. **Agent Activation Signal Design** (Avoiding Circular Dependencies):
   ...
   **Alternative (Not Recommended)**: Use existing `session_activated` signal and check `session.agent_name` in Window
   - Works but less explicit - agent changes aren't always tied to session changes
5. Implement view switching logic when agent provides widget:
   - Call `agent.get_widget()` to get UI component
   - If widget is not null:
     - Cast to `Gtk.Widget?` and add to WindowPane.tab_view
     - Show WindowPane via `window_pane.show_right_pane()`
   ...
   - If widget is null (or when switching away):
     - Remove UI component from WindowPane.tab_view
     - Hide WindowPane via `window_pane.hide_right_pane()`
   
6. **Add generic method to BaseAgent** (libocagent):
   - Add virtual `get_widget()` method returning `Object?` (default: null)
   - CodeAssistant agent:
     - Overrides `get_widget()` to create/return SourceView with ProjectManager (cast as Object)
     - Agent manages ProjectManager lifecycle and SourceView creation
     - Returns null if UI not yet created or if error occurs
