...
- `liboccoder/files/Project.vala` - **REMOVE** - Project class functionality merged into Folder
- `liboccoder/files/Folder.vala` - **MODIFY** - Add `is_project` flag, add `project_files` property, remove `async read_dir()` method (moved to ProjectFiles), keep `children` and `child_map` for tree view hierarchy, merge Project methods (`get_active_file()`), deprecate `all_files`
- `liboccoder/files/File.vala` - **MODIFY** - Add `is_project` flag
- `liboccoder/files/FileBase.vala` - **MODIFY** - Add `is_alias`, `points_to`, and `points_to_id` properties for alias/symlink tracking. Update `base_type` handling to use `is_project` flag instead of separate Project type. Update `initDB()` to include alias fields in database schema.
- `liboccoder/files/ProjectFiles.vala` - **CREATE** - New class that handles all async file I/O, directory scanning, synchronization, and manages tree structure and hashmap
- `liboccoder/ProjectManager.vala` - **MODIFY** - Update to use `Folder` with `is_project = true` instead of `Project` class, update `projects` list type
- `liboccoder/FileDropdown.vala` - **MODIFY** - Update to use `project_files.get_flat_file_list()` instead of `all_files` (for dropdown search)
