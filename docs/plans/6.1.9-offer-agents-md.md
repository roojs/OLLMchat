...
ℹ️ Parent: [6.1.2](done/6.1.2-DONE-pi-like-agent.md) Phase 1. Content / skill: [6.1.12](6.1.12-write-agents-md-skill.md). Split out of former Phase 9.  
...
- 🔷 `⏳` Create runs the **`writing-agents-md` skill** ([6.1.12](6.1.12-write-agents-md-skill.md)) — not a canned stub.
- ℹ️ Agent-facing missing-file hint (even without the banner) is [6.1.12](6.1.12-write-agents-md-skill.md). This plan is the **user** banner.
- ℹ️ Phase 1 still **omits** a fake agents body when missing — [6.1.12](6.1.12-write-agents-md-skill.md) adds a `missing_project_agents` hint instead.
...
- 🔷 `⏳` On Create: start [6.1.12](6.1.12-write-agents-md-skill.md) (agent turn / user message that runs `writing-agents-md`). Do **not** write a canned stub.
...
3. Create → [6.1.12](6.1.12-write-agents-md-skill.md) skill path; confirm inject picks up the real file on next send.
...
- 🔷 Agent id `agent-pi`.
- 🚫 Do not invent a default packaged AGENTS.md body for empty projects (Phase 1: omit inject). Content contract is [6.1.12](6.1.12-write-agents-md-skill.md).
