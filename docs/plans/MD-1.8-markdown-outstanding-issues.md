...
Six areas (items 1–5 are higher priority; 6–7 are low priority):
...
6. **Bold/emphasis spanning newline** — *(Low priority, rare.)* CommonMark allows emphasis to span a soft line break (e.g. `**word1\nword2**`). Our parser clears `state_stack` on newline in `handle_line_break` without emitting closing format callbacks, so bold spanning a newline is mis-parsed. Test case: `tests/markdown/known-fail-line-break-in-bold.md`. Fix would be to allow open formats to continue across paragraph-internal newlines (e.g. treat newline as soft break and do not clear format stack until block end).
7. **Hash-link label `[#`** — **✔️** [`MD-1.8.5-hash-link-label-lead.md`](MD-1.8.5-hash-link-label-lead.md) — `FormatMap` `[#?` link lead (was low priority; fixed).
...
| **Bold/emphasis across newline** | *(Low priority.)* Parser: do not clear format stack on soft line break; allow emphasis to span newline within paragraph. Test: `known-fail-line-break-in-bold.md`. |
| **Hash-link label `[#`** | **✔️** 1.8.5 — `FormatMap` `[#?` link lead. |
