...
**Depends on:** [`8.4.1`](8.4.1-DONE-rpc-positional-values.md) (`Request.values` as `GLib.Value`s).

**Next:** [`8.4.3`](8.4.3-rpc-ffi-typelib-method.md) — method on a lease (`dispatch_function`, `Request.lease_id`).
...
- **ℹ️** Method-on-handle is [`8.4.3`](8.4.3-rpc-ffi-typelib-method.md).
...
- **🔷** `register` and `types` stay static. Per-call: `new Gi(request)` then `dispatch()`. `dispatch` finds the `FunctionInfo` and routes constructors to `dispatch_new`. Non-constructors return false ([`8.4.3`](8.4.3-rpc-ffi-typelib-method.md) `dispatch_function`). `convert` is one TypeTag slot; the arg walk stays in `dispatch_new`.
...
- **🔷** `⏳` Keep a `Gi.types` map (`alias` → `GLib.Type`) for Phase 2 dispatch (and [`8.4.3`](8.4.3-rpc-ffi-typelib-method.md)). Skip `GType.INVALID`. Skip a second `Bin.register` of the same alias (`alias_to_gtype.has_key`).
...
- **🔷** `⏳` `dispatch`: prefix in `types`, `find_method`, constructors → `dispatch_new`. Non-constructor returns false ([`8.4.3`](8.4.3-rpc-ffi-typelib-method.md) `dispatch_function`). Missing method → `METHOD_NOT_FOUND`.
...
- **🔷** `⏳` Do **not** call a method on it yet — that is [`8.4.3`](8.4.3-rpc-ffi-typelib-method.md).

This plan does **not** add `Request.lease_id` — that is [`8.4.3`](8.4.3-rpc-ffi-typelib-method.md).
...
- **🚫** Fence `dispatch_function` here. That is [`8.4.3`](8.4.3-rpc-ffi-typelib-method.md).
...
- **ℹ️** `fn.invoke` `return_value` is fenced as `out ret`. If valac rejects that against `gobject-introspection-1.0.vapi`, drop `out` or use `ref` — keep the rest of the `new` branch. The system vapi takes `return_value` by value, so both `out` and `ref` fail; a pointer binding is unapproved.
