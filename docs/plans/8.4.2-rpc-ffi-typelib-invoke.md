...
- **🔷** `register` and `types` stay static. Per-call: `new Gi(request)` then `dispatch()`. `dispatch` finds the `FunctionInfo` and routes: constructors → `dispatch_new`. Phase 3 adds `dispatch_function` (not fenced here). `convert` is one TypeTag slot; the arg walk stays in `dispatch_new`.
...
**Why:** All GI work stays on `Gi`. `register` / `types` are static. Each call is `new Gi(request)`. `dispatch` finds the callable and routes. `dispatch_new` constructs. `convert` is only the TypeTag fill of one slot (the loop stays in `dispatch_new`). `dispatch_function` is Phase 3 — do not add it here. Windows/Android use §3b.
...
- **🔷** `⏳` `dispatch`: prefix in `types`, `find_method`, constructors → `dispatch_new`. Non-constructor returns false (Phase 3 `dispatch_function`). Missing method → `METHOD_NOT_FOUND`.
...
Goal: once Phase 2 has leased an object, the client can call **any method** that `ObjectInfo.find_method` finds on that type. `dispatch` already finds; Phase 3 adds `dispatch_function(fn)` (not `dispatch_new`).
...
- **🔷** `⏳` `Request.dispatch`: `handlers` first; else `new Gi(this).dispatch()`. Phase 3 adds `dispatch_function` only (finding already in `dispatch`).
- **🔷** `⏳` `dispatch_function`: not a constructor, handle is set.
- **🔷** `⏳` Lookup `this.request.connection.leases` for that id. Unknown id → error.
- **🔷** `⏳` No such method is already `METHOD_NOT_FOUND` in `dispatch` (finding).
- **🔷** `⏳` `FunctionInfo.invoke`: `this.in_args[0]` is the leased instance (set in `dispatch_function`, not `convert`); remaining IN args from `this.request.values` via the same loop + `convert`.
...
- **🚫** Fence `dispatch_function` in Phase 2. Name it in Phase 3 only.
...
- **🚫** Phase 2 invoke of instance methods — `dispatch` returns false unless `IS_CONSTRUCTOR`. Phase 3 is `dispatch_function`.
