...
- **🔷** `register` and `types` stay static. Per-call work is an instance: `new Gi(request)` then `dispatch()`. The TypeTag fill of one slot is instance `convert`; the arg walk stays in `dispatch`.
...
- **🔷** Pack `GI.Argument`s from **GIR**, not from `GValue.type()`.
  ...
  - Payload: `this.request.values` in that order. `dispatch` walks GIR args; `convert` fills one `in_args` slot from `this.request.values`.
...
- **🔷** Else `new Gi(this).dispatch()` — Phase 2 `new`, Phase 3 other methods. Stub returns false.
...
### 3. `libocrpc/Gi.vala` — unix `register` + instance `dispatch` / `convert`

**Why:** All GI work stays on `Gi`. `register` / `types` are static. Each call is `new Gi(request)`. `dispatch` walks args and invokes. `convert` is only the TypeTag fill of one slot (the loop stays in `dispatch`). Windows/Android use §3b instead of this file.
...
namespace OLLMrpc
{
	 ...
	 * {@link Request} and calls {@link dispatch}. Windows and Android
	 * compile ''windows/Gi.vala'' instead (meson, not ''#if'').
	 *
	 * == Example ==
	 *
	 * {{{
	...
	public class Gi : GLib.Object
	{
		...
		/**
		 * Apply a typelib call (Phase 2: ''new'' only).
		 *
		 * Returns false when the prefix is not in {@link types} or the
		 * method is not yet handled, so {@link Request.dispatch} can
		 * fall through to the unknown-handler critical. True means
		 * this method replied (success or {@link Transport.Connection.reply_error}).
		 *
		...
		public bool dispatch()
		{
			...
			if (!types.has_key(object_name) || method_name != "new") {
				return false;
			}
...
- **💩** Constructor-with-args (`Gio-SimpleAction.new`) is a follow-up in `convert` (`UTF8` works; `INTERFACE` / `GLib.VariantType` still 💩).
...
- **🔷** `⏳` `dispatch`: prefix in `types` and method is `new` → construct / export / reply. Otherwise return false.
- **🔷** `⏳` `ObjectInfo.find_method("new")`. Must be `GI.FunctionInfoFlags.IS_CONSTRUCTOR`.
- **🔷** `⏳` `dispatch` walks `fn.get_n_args()` (skip `is_skip` / `OUT`). IN count must equal `this.request.values.size`. Each IN slot calls `convert(arg, vi)`.
- **🔷** Phase 2 tags in `convert`: `BOOLEAN`, `INT32`/`INT64`, `UINT32`/`UINT64`, `UTF8`/`FILENAME`. Other tags (`INTERFACE`, `ARRAY`, float) → `INVALID_PARAMS` until a later edit of `convert`.
- **💩** `GValue.transform` into the GIR-expected width (e.g. wire `INT` → tag `INT64`) — in `convert`, not a second helper.
...
Goal: once Phase 2 has leased an object, the client can call **any method** that `ObjectInfo.find_method` finds on that type. Same instance `dispatch` (new branch) and per-slot `convert`.
...
- **🔷** `⏳` `Request.dispatch`: `handlers` first; else `new Gi(this).dispatch()`. Phase 3 extends instance `dispatch` (and `convert` tags) only.
- **🔷** `⏳` `dispatch`: prefix in `types`, method is **not** `new`, handle is set.
- **🔷** `⏳` Lookup `this.request.connection.leases` for that id. Unknown id → error.
- **🔷** `⏳` `find_by_gtype` / `ObjectInfo.find_method(method)`. No such method → error (GI boundary, not a soft fallback).
- **🔷** `⏳` `FunctionInfo.invoke`: `this.in_args[0]` is the leased instance (set in `dispatch`, not `convert`); remaining IN args from `this.request.values` via the same loop + `convert`.
- **🔷** Direction / skip / arg count come from the typelib (`ArgInfo.get_direction`, `is_skip`, `get_n_args`), not from `GValue.type()`.
...
- **🚫** Static `dispatch` / static `convert`. Those are instance methods. Static `register` and static `types` stay.
...
- **🚫** Phase 2 invoke of instance methods — only `new` in `dispatch`. Phase 3 is the method-on-handle slice of the same methods.
...
- **🚫** Move the GIR arg walk into `convert`. The loop stays in `dispatch`. `convert` is one TypeTag fill (`arg`, `vi`).
