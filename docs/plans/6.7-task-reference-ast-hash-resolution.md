# +6.7 Task references: `ResolveLink` — implementation plan

**Status:** Not started · **Related:** [1.23.31](1.23.31-reference-file-anchor-ast.md)
...
/*
 * See plan golden rule: no defensive validation, assert, or error in resolve — ValidateLink + preload guarantee state.
 *
 * preload_file / preload_http / preload_links are async (IO + Tree.parse + HTTP).
 * Call before resolve. Runner.load_file behavior is folded into preload_file;
 * Tool.load_http behavior is folded into preload_http.
 * Scheme handlers invoked from resolve are synchronous.
 */
...
/**
 * Reference links -> prompt markdown.
 * @see Details.reference_contents
 */
public class ResolveLink : GLib.Object
{
	...
	/**
	 * Project manager for buffers, trees, and files (same object as
	 * {{{ runner.sr_factory.project_manager }}}).
	 */
	...
	/**
	 * Resolve one reference to a markdown block. Sync: run preload_links first.
	 */
	...
	/**
	 * HTTP reference: body from {{{ runner.http_cache }}} (filled in preload_http).
	 */
	...
	/** Cached `^[Ll](\\d+)-[Ll](\\d+)$` for #L line-range fragments (not AST paths). */
	...
	/**
	 * Same work as {{{ Runner.load_file }}}: buffer create + read_async.
	 */
	...
	/**
	 * Same behavior as {{{ Tool.load_http }}}: {{{ web_fetch }}} -> {{{ runner.http_cache }}}.
	 */
	...
	/**
	 * Subsumes former {{{ Runner.load_links }}} (file) + {{{ Tool }}} HTTP prefetch + AST
	 * {{{ tree.parse }}} when hash is not a #L line range.
	 */
	...
	/**
	 * file: branch — sync after preload (buffer + optional tree already done).
	 */
	...
	/**
	 * AST fragment: tree already parsed in preload.
	 */
