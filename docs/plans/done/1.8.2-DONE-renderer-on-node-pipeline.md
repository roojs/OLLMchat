...
- **Coding standards:** All new/modified Vala code must follow `docs/coding-standards.md`: `this.` prefix, brace style (line breaks for namespace/class/method, inline for control structures), no nullable strings (use empty string), no `@"..."` except multi-line help/docs, no trivial temporaries, fully-qualified `GLib.*` where needed, property defaults.
...
- [x] **Coding standards:** Follow `docs/coding-standards.md`: `this.` prefix, brace style (line breaks for class/method, inline for switch/if), no nullable strings (empty string only), no `@"..."` except multi-line docs, no trivial temporaries, `GLib.` prefix where needed.
