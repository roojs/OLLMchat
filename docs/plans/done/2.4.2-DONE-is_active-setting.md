...
**Completed**: Fix implemented in `libollmchat/Agent/Base.vala`: `send_async()` and `execute_tools()` now set `session.is_running = true` when sending tool continuation replies, so the session correctly appears as running and the UI updates after tool completion.
...
**Root Cause Found**: 
- ❌ **NOT `is_active`** - that's only set during session switching (correct behavior)
- ✅ **IS `is_running`** - when tool reply is sent via `send_append()`, `is_running` is NOT set to `true`
- This makes the session appear "not running" even though it's still processing the LLM response after tool execution

**Investigation Findings**:
- ✅ `is_active` is ONLY set in `SessionBase.activate()` (line 278) and `SessionBase.deactivate()` (line 297)
- ✅ These methods are ONLY called from `Manager.switch_to_session()` during session switching
- ✅ No incorrect assignments found in tool request handlers
- ❌ **BUG FOUND**: When `send_append()` is called (tool reply), `is_running` is never set to `true`
- ❌ `Chat.send()` (called by `send_append()`) does NOT call `Session.send()` which sets `is_running = true`

**Key Distinction**:
- `is_active`: Which session is visible in UI (user selection) - should NOT change when tool completes ✅
- `is_running`: Whether session is processing a request (streaming state) - SHOULD be set to `true` when tool reply continues conversation ❌

**Stop/Send Button**: Controlled by `is_running`, NOT `is_active` ✅
...
### Task 1: Search for All `is_active = false` Assignments ✅ COMPLETE
...
1. **SessionBase.is_active assignments** (CORRECT):
   ...
   - ✅ These are the ONLY places where Session `is_active` is set
...
3. **activate()/deactivate() calls**:
   ...
   - ✅ All calls are in session switching logic, which is correct
...
Session.finalize_streaming() 
  → creates "done" message
  → session.messages.add(done_msg)
  → manager.message_added(done_msg, this)
    → ChatWidget.on_message_created()?
    → Tool Request.on_message_created()?
      → Does this set is_active = false? (SHOULD NOT)
```

### Task 3: Verify Session Switching Logic ✅ PARTIAL
...
1. **activate() calls**:
   ...
   - ✅ Both are in `switch_to_session()` method - correct

2. **deactivate() calls**:
   - `Manager.vala` line 215: `this.session.deactivate()` - when switching away from current session
   - ✅ Called in `switch_to_session()` before activating new session - correct
...
### Task 4: Check if Tool Reply Affects Session State ✅ FOUND THE ISSUE!
...
1. **Tool Reply Flow**:
   ...
   - ❌ **BUG**: `is_running` is never set to `true` when tool reply is sent!
...
**Flow**:
1. Tool receives "done" message → `is_running = false` (correct)
2. Tool sends reply via `send_append()` → calls `Chat.send()` directly
3. `Chat.send()` does NOT call `Session.send()` → `is_running` stays `false` ❌
4. UI sees `is_running == false` → button shows "Send" instead of "Stop"
