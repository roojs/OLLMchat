...
- **Implementation:** DONE. Details: **validate_references(MarkdownPhase)**; task/file/http/task:// per current rules. ResultParser: **fill_names()**, per-task **validate_references(LIST|REFINEMENT)**. (Older note about **has_slug** superseded by **completed/pending.slugs**.)
...
**DONE.** Details: MarkdownPhase, to_markdown(phase), refine() fill. ResultParser: parse_task_list three sections (**original-prompt**, **goals-summary**, **tasks**), **goals_summary_md**, **parse_task_list_iteration()**. List: **goals_summary_md**, **to_markdown(phase)**. Runner: **run_task_list_iteration()** fully wired (not a stub). task_list_iteration.md + iteration_prompt() as in **Runner.vala**.
