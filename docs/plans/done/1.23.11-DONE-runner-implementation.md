...
**Coding standards:** All proposed code in this plan must follow `docs/coding-standards.md`. Before implementation, verify against **`docs/guide-to-writing-plans.md`** checklist and **`docs/coding-standards.md`**. This plan’s code uses: **this.** for instance members (including in callbacks: this.run_child, this.wait_exec_done (callbacks inline num_exec_running--)); **var** for initializers where type is not required; **item.children** and **children.get(0)** (Gee .get(), not []); **continue** to avoid else in loops; inline braces for control structures (if/foreach); **GLib.Error**; no underscore names; no get_* for simple accessors.
...
- **TaskList** — steps, refine, run_until_writer, run_child, has_writer_tasks, run_all_tasks (Runner calls run_task_list_iteration() directly after). See **1.23.13**.
