...
- **Done (this plan):**
  - **libocmarkdown/TableState.vala** – New. Constructor `TableState(Parser parser)`, state (table_header_line, table_alignments, table_row_index), **split_row**, **parse_align**, **emit_row**, **process_cell** (inline formatting; uses parser.formatmap, renderer, got_format, do_format, add_html, state_stack), **feed_line** (row/phase logic). Only constructor and feed_line are internal; rest private.
  - **libocmarkdown/Parser.vala** – `internal RenderBase renderer { get; private set; }`, `internal TableState? table_state { get; set; }`, `internal` state_stack, got_format, do_format, add_html (so TableState can drive cell formatting). No process_cell on Parser; it lives in TableState.
  - **libocmarkdown/meson.build** – TableState.vala in sources.
  - **docs/meson.build** – TableState.vala in valadoc input.
- **Not in this plan (1.10.5):** Creating/clearing table_state, calling feed_line from **handle_block_result**, **add()**, **handle_line_break**, or **do_block** (table start/end and line routing).

**Coding standards:** This plan follows **docs/coding-standards.md**. All code uses:
- Brace style:
