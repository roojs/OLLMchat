...
**Status:** **DONE** (2026-03) — Implemented: empty filter short-circuit in `Search.execute`, category filter SQL + UX messages in `RequestCodebaseSearch`, documentation pipeline sets category on tree/elements; use `--dump-vector` / re-index if needed for legacy DB rows.
...
1. **Empty filter → no search, and say why**  
   When `filtered_vector_ids.size == 0`: (a) Don’t run the search—return empty immediately (e.g. in `libocvector/Search/Search.vala` `execute()` at the start). (b) In the CLI/tool, before calling search (or when results are empty due to empty filter), output a clear message, e.g. **No document matches the criteria**, instead of continuing and showing a generic “No results found.”
...
2. **Propagate category to sections**  
   ...
   **Fix:** After `analyze_tree()` or at the start of `DocumentationVectorBuilder.process_file()`, set every element’s category from the document, e.g. `foreach (var el in tree.elements) { el.category = tree.category; }`. Then chunk metadata (and saved rows) get the document’s category.
...
4. **Category value / UX**  
   Optional: normalize `-c plans` → `plan` when binding so the filter matches. Document valid values.
...
- With 0 filtered IDs we still call FAISS; null selector → search all. We keep all results (post-filter only runs when `filtered_vector_ids.size > 0`).
- Those results can have `file_id` not in `file_id_list` → file not in `project_files.items` → `get_by_id()` returns null → crash.

---

## Details (reference)

- **Where we look:** `SearchResult.file()` → `folder.project_files.get_by_id(metadata.file_id)`; only searches `project_files.items`. Null if file not in items.
...
- **Category flow:** DocumentationAnalysis sets `root_element.category` and `tree.category`; sections are created earlier with `category = ""` and never updated. VectorBuilder saves only leaf sections; chunk_meta copies `section.category` → always `""`.
