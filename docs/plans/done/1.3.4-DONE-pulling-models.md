...
5. **Implement `PullManager` class** ✅ **COMPLETED** (`ollmchat/Settings/PullManager.vala`)
   ...
   - ✅ Split into three classes for clarity:
     - ✅ `PullStatus` - Status data class with JSON serialization
     - ✅ `PullManager` - Main thread logic, UI interaction, persistence
     - ✅ `PullManagerThread` - Background thread logic, network operations
...
7. **Implement loading.json persistence** ✅ **COMPLETED**
   - ✅ `PullManager` writes to `data_dir/loading.json` on:
   ...
   - ✅ Format: JSON array `[{...}, {...}]` using `Json.gobject_to_data()` for each `PullStatus` object
   - ✅ Automatic loading on `PullManager` construction
   - ✅ Uses `model_name` property in `PullStatus` for cache key
