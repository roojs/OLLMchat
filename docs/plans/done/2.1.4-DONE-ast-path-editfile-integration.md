...
✅ **DONE** - All phases complete (core infrastructure, AST path resolution, code block format support, edit operation types, AST cache invalidation, Stream class refactor, mode separation, incremental AST path mode, UI validation, documentation/testing).
...
- [x] Add `resolve_ast_path()` method to RequestEditMode similar to ReadFile's implementation
  ...
  ```
- [x] Use same resolution pattern: `project_manager.tree_factory(file)` → `tree.parse()` → `tree.lookup_path()`
...
- [x] See separate plan: **2.1.6** - Stream Class Refactoring for EditMode

### Phase 7: Line Number Mode (Legacy) and Mode Separation
- [x] See separate plan: **2.1.7** - Line Number Mode (Legacy) and Mode Separation

### Phase 8: AST Path Mode (Incremental Application)
- [x] See separate plan: **2.1.8** - AST Path Mode (Incremental Application)

### Phase 9: UI Messages and Validation
- [ ] See separate plan: **2.1.10** - AST Path UI Messages and Validation
...
1 code block(s) were ignored due to mode mismatch:
  • Change to `ast-path:OLLMchat-Client-chat` was ignored: AST path format was specified, but line number mode is active (use_line_numbers=true). Use format: `type:startline:endline` instead.
...
1 code block(s) were ignored due to mode mismatch:
  • Change at lines 45-67 was ignored: Line number format was specified, but AST path mode is active (use_line_numbers=false). Use format: `type:Namespace-Class-Method` instead.
...
2 code block(s) were ignored due to mode mismatch:
  • Change at lines 100-120 was ignored: Line number format specified but AST path mode is active (use_line_numbers=false). Use format: `type:Namespace-Class-Method` instead.
  • Change at lines 150-170 was ignored: Line number format specified but AST path mode is active (use_line_numbers=false). Use format: `type:Namespace-Class-Method` instead.
