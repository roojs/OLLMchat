...
**Archived:** 2026-05-26 — **`run_task_list_continue`**, **`task_list_continue.md`**, **`parse_task_list_continue()`**, **`PhaseEnum`** continue phases, **`user_prompts`** / replay **`on_replay`** case, **`iteration_prompt`** follow-up placeholders; user text stripped in **`Session.send()`** at source.
...
- ✅ **On success:** **`user_prompts.add(in_message.content)`** (stripped in **`Session.send()`**); parser sets **`pending.goals_summary_md`** + **`pending`** tasks only.
