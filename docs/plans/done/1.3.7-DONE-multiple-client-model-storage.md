...
- [x] **Note**: Display methods (`display_markup()`) need sorting out later - removed from Response.Model, needs proper implementation
- [x] **Note**: Connection filtering and sorting moved to UI layer (libollmchatgtk) - removed GTK-4 dependency from libollmchat
...
- [x] Update `ollmchat/SettingsDialog/Rows/Model.vala` to use ConnectionModels with connection filter
...
When implementing this phase, follow the coding standards in `docs/coding-standards.md`:
...
- [x] Update `ollmchat/SettingsDialog/ModelsPage.vala` to use ConnectionModels
...
**Changes:**
- **Remove signal monitoring**: Remove `items_changed` signal connections. Page updates on display, refresh button click, and when download completes (but download completion doesn't trigger items_changed, so no monitoring needed).
...
- Create filtered/sorted ListModels per connection on each render (no need to store them)
- Load data on page display and refresh button click
- Use ListModels directly - never copy ModelUsage objects into local data structures
- Iterate through ListModels to create ModelRow widgets (acceptable for on-demand rendering)

**Implementation Details:**
See Phase 5 section above for detailed explanation of what this class does and how ConnectionModels will be used.

**Simplified approach:**
- No signal monitoring required - page updates on display and refresh button click only
- Create filtered/sorted ListModels fresh on each render (they're lightweight wrappers)
- Iterate through ListModels using `get_n_items()` and `get_item_typed(i)` to create widgets
- Refresh button calls `render_models()` which rebuilds the entire UI from current ConnectionModels state
...
✅ **All phases complete** - The Multiple Client Model Storage system has been fully implemented:
- Phase 1: ConnectionModels class created with ListModel interface and progressive refresh logic
- Phase 2: Integration with Window and History Manager completed
- Phase 3: Integration with ChatInput completed
- Phase 4: Integration with SettingsDialog.Rows.Model widget completed
- Phase 5: Integration with ModelsPage completed
- Phase 6: ModelConfigDialog does not exist, no integration needed
