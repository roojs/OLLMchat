...
ℹ️ Parent: [6.1](../CODER-6.1-agent-pi.md). Study: [6.1.1](../CODER-6.1.1-DONE-pi-agent.md); harness: [6.1.4](../CODER-6.1.4-DONE-pi-harness-what-it-actually-does.md).  
ℹ️ Landed core: `liboccoder/AgentPi/` (`Factory`, `Agent`, `PendingMessage`, `Skill`, `SkillSet`), `resources/pi-prompts/`, Agent Pi tool names, AGENTS inject, compact/`session_fetch`, licenses.  
ℹ️ Split-outs done: [6.1.3](6.1.3-DONE-base-skills.md), [6.1.6](6.1.6-DONE-urgent-follow-up.md) (+ .1/.2/.3), [6.1.7](6.1.7-DONE-project-summary-tool.md).  
ℹ️ Remaining (sibling plans): [6.1.9](../CODER-6.1.9-offer-agents-md.md), [6.1.10](../CODER-6.1.10-DONE-skills-configuration.md), [6.1.11](../CODER-6.1.11-extended-base-skills.md).  
ℹ️ Optional later (not split): Pi-style exact **`edit`** tool; AGENTS inject soft-cap (% of context) after [09](../CODER-6.1.9-offer-agents-md.md).

## Purpose

- ✅ liboccoder-weight Agent Pi (`agent-pi` / `OLLMcoder.AgentPi` / **Agent 𝚷**).
- ✅ AGENTS.md inject, soft skill catalog, `toolsReply`, permissions.
- ✅ Pi tool names (`read` / `write` / `bash`), prompt assembly, Phase 5 compact + reference tags.
- ✅ Base skills → [6.1.3](6.1.3-DONE-base-skills.md); urgent/follow-up → [6.1.6](6.1.6-DONE-urgent-follow-up.md); project summary → [6.1.7](6.1.7-DONE-project-summary-tool.md).
- ℹ️ Open work lives in [09](../CODER-6.1.9-offer-agents-md.md) / [10](../CODER-6.1.10-DONE-skills-configuration.md) / [11](../CODER-6.1.11-extended-base-skills.md) (plus optional edit / AGENTS soft-cap notes above).
...
- **🔷** Namespace **`OLLMcoder.AgentPi`** (logical **AgentPi**; GIR nesting under liboccoder); factory `name` / agent id **`agent-pi`**; human title **`Agent 𝚷`**; `long_title` **`implementation of the Pi agent harness`**.
...
- **🔷** `✔️` **Base set** — shipped under `resources/pi-skills/` (see **Phase 2.1** / [6.1.3](6.1.3-DONE-base-skills.md)).
...
- **🚫** Fake AGENTS path / `agent/` subdir / size policy (size = after [09](../CODER-6.1.9-offer-agents-md.md)).
- **ℹ️** Offer-to-create: [09](../CODER-6.1.9-offer-agents-md.md).
...
### Phase 2.1 — Base skill pack

- **🔷** `✔️` Base pack under `resources/pi-skills/` + gresource `/pi-skills` (detail: [6.1.3](6.1.3-DONE-base-skills.md)).
- **🔷** `✔️` `SkillSet.scan` includes resource pack first; user/project filesystem same-name override.
- **🔷** `✔️` `read` can open `resource://` skill paths.
- **🔷** Content shape: directory + `SKILL.md` (+ optional refs/scripts) — **not** Node packages / npm gallery.
- **🔷** User edit path: config **Export** resource → filesystem — still [6.1.10](../CODER-6.1.10-DONE-skills-configuration.md).
- **🔷** `⏳` **Initial offer** (subset of available → `{skills_md}`) + settings UI: [6.1.10](../CODER-6.1.10-DONE-skills-configuration.md).
- **🔷** `⏳` Extended / deferred skills (subagent-delegate, skipped superskills, …): [6.1.11](../CODER-6.1.11-extended-base-skills.md).
...
- **💩** Default tool subset ([2.30](../TOOLS-2.30-pretooler-tool-filtering.md)) — optional, not the core of Phase 3.
...
Study refs (background only): [01 §7](CODER-6.1.1-DONE-pi-agent.md#7-context-growth-pi-compaction-vs-chatter); Pi `packages/coding-agent/src/core/compaction/`; hash/`session_fetch` patterns in [2.31](CHAT-2.31-DONE-just-ask-summary-history.md). **🚫** Slash `/compact` — we do not support slash commands.
...
- ✅ Urgent / follow-up (agent + UI): [6.1.6](6.1.6-DONE-urgent-follow-up.md).
  - ✅ [6.1.6.1](6.1.6.1-DONE-chatwidget-can-queue.md) — ChatWidget `can_queue` + pending ColumnView.
  - ✅ [6.1.6.2](6.1.6.2-DONE-agent-queue-session.md) — Agent Pi MessageQueue bind.
  - ✅ [6.1.6.3](6.1.6.3-DONE-agent-session-switch.md) — agent change → new session.
- ✅ Project-summary tool: [07](6.1.7-DONE-project-summary-tool.md).
- **ℹ️** Offer to create `AGENTS.md`: [09](../CODER-6.1.9-offer-agents-md.md).
...
Core phases 0–5 / 8 and split-outs **03 / 06 / 07** are **DONE** (this archive).

Remaining:

1. [10](../CODER-6.1.10-DONE-skills-configuration.md) — initial offer + export UI  
2. [11](../CODER-6.1.11-extended-base-skills.md) — extended / deferred skills  
3. [09](../CODER-6.1.9-offer-agents-md.md) — offer to create `AGENTS.md`  
4. After [09](../CODER-6.1.9-offer-agents-md.md) — AGENTS inject soft-cap (% of context)  
5. Optional: Pi-style exact `edit` tool  
...
- **ℹ️** Study: [6.1.1](../CODER-6.1.1-DONE-pi-agent.md); siblings [03](6.1.3-DONE-base-skills.md), [04](../CODER-6.1.4-DONE-pi-harness-what-it-actually-does.md), [06](6.1.6-DONE-urgent-follow-up.md) (+ [6.1.6.1](6.1.6.1-DONE-chatwidget-can-queue.md) / [6.1.6.2](6.1.6.2-DONE-agent-queue-session.md) / [6.1.6.3](6.1.6.3-DONE-agent-session-switch.md)), [07](6.1.7-DONE-project-summary-tool.md), [09](../CODER-6.1.9-offer-agents-md.md), [10](../CODER-6.1.10-DONE-skills-configuration.md), [11](../CODER-6.1.11-extended-base-skills.md).
...
- **ℹ️** Chatter kernel: `libollmchat/Chatter/`, [2.31](CHAT-2.31-DONE-just-ask-summary-history.md).
- **ℹ️** liboccoder weight: `OLLMcoder.AgentFactory`, `OLLMcoder.Skill.Factory`.
- **🔷** Namespace **`OLLMcoder.AgentPi`**; agent id **`agent-pi`**; title **`Agent 𝚷`**; `long_title` **`implementation of the Pi agent harness`**.
