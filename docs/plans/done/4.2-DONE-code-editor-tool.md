...
✅ **INTEGRATION COMPLETE** - Core data layer, UI components, and integration complete. SourceView is now integrated into main window via agent widget system. CodeAssistant agent provides SourceView widget, Window manages it through WindowPane.

**Progress:** 6 of 11 phases complete (55%), integration phase complete

**Note:** Some remaining TODO phases have been moved to separate plan files:
- Phase 2D (Tree Navigation): See [CODER-4.2.1-tree-navigation-filedropdown.md](CODER-4.2.1-tree-navigation-filedropdown.md)
- Phase 7 (Code Editor Manager Tool): See [CODER-4.2.2-code-editor-manager-tool.md](CODER-4.2.2-code-editor-manager-tool.md)
- Phase 6 (SourceView inline diff): See [CODER-4.2.3-URGENT-source-view-diff.md](../CODER-4.2.3-URGENT-source-view-diff.md)
- Phase 8 (Background Sync and File Watcher): See [APP-4.2.3-background-sync-file-watcher.md](APP-4.2.3-background-sync-file-watcher.md)

**Current Status:**
- ✅ **Data Layer Complete**: All liboccoder classes implemented and working
- ✅ **UI Components Complete**: SourceView, ProjectDropdown, FileDropdown fully implemented
- ✅ **Integration Complete**: SourceView instantiated via CodeAssistant.get_widget(), added to WindowPane via WindowPane.add_or_show_agent_widget()
- ✅ **State Restoration Called**: `apply_manager_state()` called from CodeAssistant.get_widget() initialization
- ✅ **Agent Widget System**: BaseAgent.get_widget() async method, CodeAssistant override, Manager.agent_activated signal, Window integration complete

**Recent Updates (Latest Session):**
- ✅ Created ProjectList class (GLib.ListModel implementation for projects with deduplication and path-based lookup)
- ✅ Refactored ProjectManager to use ProjectList instead of Gee.ArrayList<Folder>
- ✅ Added ProjectList.get_active_project() method for finding active project
- ✅ Added ProjectFiles.get_active_file() method for finding active file
- ✅ Refactored ProjectManager.restore_active_state() to set active_project and active_file directly (removed output parameters)
- ✅ Renamed restore_session() to restore_active_state() to avoid confusing "session" terminology
- ✅ Implemented SourceView.apply_manager_state() to apply UI state from manager properties
- ✅ Refactored ProjectDropdown to use ProjectList directly as item model (no separate ListStore)
- ✅ Updated ProjectMigrate to use ProjectList.contains_path() for checking existing projects
- ✅ Applied coding standards throughout (this. prefix, reduced nesting, inlined variables)
- ✅ Implemented search functionality (GtkSource.SearchContext, search UI, keyboard shortcuts)
- ✅ Added search footer bar with search entry, results label, and navigation buttons
- ✅ Implemented forward/backward search navigation with Ctrl+G and Ctrl+Shift+G shortcuts
- ✅ Added search settings (case sensitive, regex, multiline) via popover menu
- ✅ **Integration Complete**: Implemented agent widget system
  - ✅ Added async `get_widget()` method to BaseAgent (libocagent)
  - ✅ CodeAssistant overrides `get_widget()` to create SourceView with ProjectManager
  - ✅ Added `agent_activated` signal to Manager (libollmchat)
  - ✅ ChatInput emits `agent_activated` signal when agent dropdown changes
  - ✅ Window handles `agent_activated` signal and manages widgets via WindowPane
  - ✅ WindowPane.add_or_show_agent_widget() method for widget management
  - ✅ WindowPane.intended_pane_visible property and schedule_pane_update() method
  - ✅ Separate database for files/projects at `.local/share/ollmchat/files.sqlite`
  - ✅ SourceView.manager made public property for CodeAssistant access
...
|-------|--------|-------------|
| Phase 1: Pane View Component | ✅ DONE | WindowPane component with visibility toggle and resizing behavior |
| Phase 1.5: Project Migration | ✅ DONE | ProjectMigrate class for importing from Cursor, roobuilder, VS Code |
| Phase 2A: Data Layer (liboccoder) | ✅ DONE | FileBase, File, Folder (with is_project flag), ProjectFiles, ProjectFile, and ProjectManager classes - fully implemented and compiling |
| Phase 2B: Database Schema and Sync | ✅ DONE | Database methods implemented, schema complete, save/load functionality working |
| Phase 2C: Searchable Dropdown Widgets | ✅ DONE | SearchableDropdown, ProjectDropdown, FileDropdown fully implemented. FileDropdown uses ProjectFiles directly. Sections/separators for recent items pending |
| Phase 2D: Tabbed FileDropdown | 📋 SEPARATE PLAN | See [CODER-4.2.1-tree-navigation-filedropdown.md](CODER-4.2.1-tree-navigation-filedropdown.md) - Add tabbed interface to FileDropdown: Tab 1 (search through ProjectFiles), Tab 2 (tree navigation of Folder.children with all expanded) |
| Phase 3: Source View Component | ✅ DONE | SourceView component fully implemented and integrated. Core functionality complete: buffer switching, file/project switching, state persistence, search functionality. **INTEGRATED**: Instantiated via CodeAssistant.get_widget(), added to WindowPane via agent widget system. Missing: diagnostics, user error dialogs |
| Phase 4: Configuration Persistence | ⏳ FUTURE | Save/restore UI state preferences |
| Phase 5: CodeAssistant Integration | ✅ DONE | CodeAssistant integrated with ProjectManager and editor via agent widget system. CodeAssistant.get_widget() creates SourceView with ProjectManager, handles initialization |
| Phase 6: SourceView inline diff | URGENT · DESIGN OPEN | See [CODER-4.2.3 URGENT - source-view-diff](../CODER-4.2.3-URGENT-source-view-diff.md) — diff UI; **blocked** on partial-approval / baseline design |
| Phase 7: Code Editor Manager Tool | 📋 SEPARATE PLAN | See [CODER-4.2.2-code-editor-manager-tool.md](CODER-4.2.2-code-editor-manager-tool.md) - Tool for LLM to manage code editor interface |
| Phase 8: Background Sync and File Watcher | 📋 SEPARATE PLAN | See [APP-4.2.3-background-sync-file-watcher.md](APP-4.2.3-background-sync-file-watcher.md) - Background sync process and file system watcher for real-time updates |

**Legend:**
- ✅ DONE - Fully implemented
- 🟡 PARTIAL - Partially implemented
- 🔴 NEEDS REDOING - Needs refactoring/updating to match new architecture
- ⏳ TODO - Not yet started
- ⏳ FUTURE - Planned for future implementation
- 📋 SEPARATE PLAN - Moved to separate plan file
...
✅ **IMPLEMENTED** - Basic data layer library exists and is functional. Architecture has evolved from original plan.
...
1. ✅ **Project class removed** - All functionality merged into Folder with `is_project` flag
2. ✅ **ProjectFiles class created** - Wrapper class for managing ProjectFile objects (ListModel implementation)
3. ✅ **Alias properties added to FileBase** - `is_alias` (computed), `points_to`, `points_to_id`, `target_path` for symlink tracking
4. ✅ **Database schema updated** - Alias fields added, base_type handling implemented
5. ✅ **.git exclusion** - Hidden/system directories skipped during scanning (implemented in Folder.read_dir())
6. ✅ **FileAlias class** - Wrapper for File aliases (throws errors for editor operations - aliases not used in editor)
7. ✅ **FolderFiles class** - ListModel implementation for Folder.children with integrated child_map (basename lookup)

**Architecture Differences from Original Plan:**
1. ⚠️ **Folder.read_dir() still in Folder** - Not moved to ProjectFiles as originally planned. Folder handles its own directory scanning.
2. ✅ **FolderAlias never intended** - Original plan incorrectly assumed FolderAlias was needed. Only FileAlias exists (for file symlinks). Folder symlinks are handled differently.
3. ⚠️ **ProjectFiles is a wrapper** - Stores ProjectFile objects (wrappers around File), not FileBase objects. Different from original plan.
4. ✅ **ProjectFile is a wrapper** - Wraps a File object for project-specific display (intended design - ProjectFiles provides the relationship context).
5. ⚠️ **FileBase has more properties** - Many properties (language, cursor_line, etc.) moved to FileBase for shared access.

**Files Status:**
- ✅ `FileBase.vala` - Complete with alias properties, database schema, shared properties
- ✅ `File.vala` - Complete, no `is_project` flag (correct)
- ✅ `Folder.vala` - Complete with `is_project` flag, `project_files`, `read_dir()` still present
- ✅ `ProjectFiles.vala` - Created as ListModel wrapper for ProjectFile objects (scan_project and get_flat_file_list not needed - Folder handles scanning, ProjectFiles provides ListModel directly)
- ✅ `ProjectManager.vala` - Updated to use Folder instead of Project
- ✅ `FileDropdown.vala` - Updated to use `project_files` (with fallback)
- ✅ `FileAlias.vala` - Created, throws errors for editor operations
- ✅ `FolderFiles.vala` - Created (named differently from planned FolderChildren)
- ✅ `ProjectFile.vala` - Created as File wrapper, not relationship tracker
...
- ❌ **NEVER INTENDED** - FolderAlias was a false assumption in the original plan.
...
- ⚠️ **Named differently from plan**: Implementation is called `FolderFiles`, not `FolderChildren`
...
- **Methods**:
  - `async read_dir(int64 check_time, bool recurse = false)` - ⚠️ **Still in Folder** (not moved to ProjectFiles as planned). Load children from filesystem, handles async file enumeration, symlink resolution, updates Folder.children. Excludes .git directories. Recursively reads subdirectories if recurse=true.
...
- ⚠️ **read_dir() is still in Folder** - Not moved to ProjectFiles as originally planned. Folder handles its own directory scanning.
...
- ✅ **Intended design**: ProjectFile is a wrapper around a File object for project-specific display (ProjectFiles provides the relationship context)
...
- ⚠️ **Different from plan**: ProjectFiles stores ProjectFile objects (wrappers), not FileBase objects
...
- **Methods**:
  - ✅ `update_from(Folder folder)` - Updates project files from folder tree recursively
  - ✅ `get_active_file()` - Returns the active File from project files list
  - ⚠️ `sync_with_filesystem()` - Not implemented (may not be needed)
  - ⚠️ `populate_from_folder()` - Not implemented (update_from() serves this purpose)
  - ⚠️ `clear()` - Not implemented (remove_all() exists)

**Note**: 
- ProjectFiles implements `GLib.ListModel` interface using `Gee.ArrayList<ProjectFile>` as the backing store.
- ⚠️ **Different from plan**: Stores ProjectFile objects (wrappers around File), not FileBase objects.
- The `child_map` provides O(1) lookup by file path for ProjectFile objects.
- ✅ **Core methods implemented**: `update_from()`, `get_active_file()`, ListModel interface methods. Some utility methods (`sync_with_filesystem()`, `clear()`) may not be needed as `update_from()` and `remove_all()` serve the core functionality.
...
- ✅ **FileDropdown updated**: Now uses ProjectFiles directly as ListModel (no copying). Extracts File objects from ProjectFile wrappers for display. Property bindings work with ProjectFile objects.
...
- **Project/Folder unification**: ✅ Implemented - Projects are folders with `is_project = true`, eliminating the need for a separate Project class
...
- **ProjectFiles class**: ✅ Implemented as ListModel wrapper for ProjectFile objects. Can be used directly as ListModel (no separate get_flat_file_list() needed).
- **Async file I/O in Folder**: ✅ **Intended design** - `read_dir()` is in Folder (correct design decision). Folder handles its own directory scanning.
- **Tree view structure**: ✅ Implemented - Uses Folder.children (FolderFiles ListModel) for hierarchical structure. Each Folder maintains its children list with integrated basename map.
- **FolderFiles class**: ✅ Implemented (named differently from planned FolderChildren) - Implements ListModel with integrated child_map for O(1) basename lookup
- **Alias/symlink scanning**: ✅ Implemented in Folder.read_dir() - Creates FileAlias objects when encountering file symlinks. Uses realpath() to resolve target_path. Folder symlinks are handled through normal Folder class with alias properties (no separate FolderAlias class needed).
- **FileAlias implementation**: ✅ Implemented - Alias files explicitly NOT used in editor (all editor operations throw errors). Maintains own path/parent for filesystem tracking.
- **Hidden directory exclusion**: ✅ Implemented - `.git` directories excluded during Folder.read_dir() scanning
...
- ✅ Keep file/directory objects in memory via `file_cache` (Gee.HashMap<string, FileBase>)
- ✅ **Stores**:
  - `file_cache` (Gee.HashMap<string, FileBase>) - Hashmap of path => FileBase for quick lookup
  - `projects` (ProjectList) - List of all projects (folders where `is_project = true`), implements GLib.ListModel
...
- ✅ **Methods**:
  ...
  - `load_projects_from_db()` - Load all projects from database into projects list
  - `async restore_active_state()` - Restore active project and file from in-memory data structures, sets this.active_project and this.active_file directly
- ✅ **Signals**:
...
- ⚠️ **Missing**: Methods to handle file changes (rename, move) - ProjectFiles manages the file-to-project relationship context
- Note: Database persistence is implemented (saveToDB, removeFromDB in FileBase)

#### Files Status

**✅ Created Files:**
- ✅ `liboccoder/` - Library directory exists
- ✅ `liboccoder/meson.build` - Meson build configuration exists
- ✅ `liboccoder/files/` - Subdirectory for file/folder/project classes exists
- ✅ `liboccoder/files/FileBase.vala` - Base class with shared properties, database operations, alias support
- ✅ `liboccoder/files/File.vala` - File class (extends FileBase)
- ✅ `liboccoder/files/FileAlias.vala` - FileAlias class (throws errors for editor operations)
- ✅ `liboccoder/files/FolderFiles.vala` - FolderFiles class (implements ListModel, named differently from planned FolderChildren)
- ✅ `liboccoder/files/Folder.vala` - Folder class (extends FileBase, has `is_project` flag, includes read_dir())
- ✅ `liboccoder/files/ProjectFile.vala` - ProjectFile class (wrapper around File, not relationship tracker)
- ✅ `liboccoder/files/ProjectFiles.vala` - ProjectFiles class (ListModel wrapper for ProjectFile objects, includes get_active_file() method)
- ✅ `liboccoder/files/ProjectList.vala` - ProjectList class (ListModel implementation for managing projects with deduplication and path-based lookup)
- ✅ `liboccoder/ProjectManager.vala` - Project and file management (uses Folder instead of Project, uses ProjectList for projects)
- ✅ `liboccoder/FileDropdown.vala` - File dropdown widget (updated to use project_files with fallback)

**❌ Not Created (Never Intended):**
- ❌ `liboccoder/files/FolderAlias.vala` - FolderAlias was a false assumption in original plan (not needed)
- ❌ `liboccoder/files/FolderChildren.vala` - Named FolderFiles instead

**✅ Removed:**
- ✅ `liboccoder/files/Project.vala` - Project class removed (functionality merged into Folder)

#### ⚠️ Known Issues and Missing Implementation

**Critical Missing Methods:**

1. **ProjectFiles utility methods** - ⚠️ **OPTIONAL - MAY NOT BE NEEDED**
   - ⚠️ `sync_with_filesystem()` - Not implemented (may not be needed - update_from() handles synchronization)
   - ⚠️ `populate_from_folder()` - Not implemented (update_from() serves this purpose)
   - ⚠️ `clear()` - Not implemented (remove_all() exists and serves this purpose)
   - ✅ `update_from(Folder folder)` - **IMPLEMENTED** - Updates project files from folder tree recursively
   - ✅ `get_active_file()` - **IMPLEMENTED** - Returns the active File from project files list
   - **Impact**: Limited ProjectFiles functionality

**Architecture Differences:**

3. **Folder.read_dir() in Folder** - ✅ **INTENDED DESIGN**
...
4. **ProjectFiles stores ProjectFile objects** - ✅ **INTENDED DESIGN**
...
5. **ProjectFile is a wrapper, not relationship tracker** - ✅ **INTENDED DESIGN**
...
6. **ProjectList implements GLib.ListModel** - ✅ **INTENDED DESIGN**
   - Created to replace Gee.ArrayList<Folder> in ProjectManager
   - Implements GLib.ListModel for GTK integration (ListView, FilterListModel, SortListModel)
   - Provides deduplication by ID and efficient path-based lookup via path_map
   - Includes get_active_project() method for finding active project
   - ProjectDropdown uses ProjectList directly as item model (no separate ListStore)
   - **No action needed**: This is the intended architecture

**Clarifications:**

7. **FolderAlias never intended** - ✅ **NOT AN ISSUE**
...
8. **Folder.read_dir_update() has syntax errors and incomplete logic** - ✅ **FIXED**
   - ✅ Fixed extra closing parenthesis
   - ✅ Fixed incomplete assignment (now correctly sets points_to_id)
   - ✅ Fixed alias resolution logic (now checks target_path correctly)
   - ✅ Fixed null check for points_to before calling saveToDB()
   - **Status**: All syntax errors resolved, alias resolution working correctly

9. **Folder.load_children() references undefined variable** - ✅ **FIXED**
   - ✅ Changed `file_base.target_id` to `file_base.points_to_id`
   - **Status**: Database loading of aliased files now works correctly

10. **FileAlias doesn't resolve points_to** - ⚠️ **INCOMPLETE**
...
11. **How should file-to-project relationships be tracked?** - ✅ **RESOLVED**
...
✅ **DONE** - ProjectMigrate class fully implemented with migration from Cursor, roobuilder, and VS Code.
...
✅ **DONE** - Database integration implemented: ProjectManager has `db` property, FileBase has `initDB()` and `saveToDB()` methods with sync parameter. Database schema complete, save/load functionality working. Background sync and file system watcher moved to Phase 8.
...
- ✅ **Implemented**: Database schema, save/load methods, backup functionality
- ✅ **Implemented**: Database dirty flag (`is_dirty`) - tracks unsaved changes in memory
- ✅ **Implemented**: Optional periodic autosave - Database constructor accepts `autosave` parameter (default: false)
...
- ⏳ **Moved to Phase 8**: Background sync process and file system watcher for real-time updates
...
✅ **DONE** - SearchableDropdown base class, ProjectDropdown, and FileDropdown widgets fully implemented. FileDropdown uses ProjectFiles directly as ListModel. Still need: sections/separators for "recent" vs "rest" items (optional enhancement).
...
- **Status**: ⏳ **Moved to Phase 2D** - See Phase 2D section for detailed implementation plan
...
📋 **MOVED TO SEPARATE PLAN** - See [CODER-4.2.1-tree-navigation-filedropdown.md](CODER-4.2.1-tree-navigation-filedropdown.md)

⏳ **TODO** - Add tabbed interface to FileDropdown widget for dual navigation modes.
...
🟡 **PARTIALLY DONE** - SourceView component implemented with core functionality. Approximately 85% complete. **CRITICAL: NOT INTEGRATED INTO MAIN WINDOW**.
...
- Search footer bar with entry, results label, and navigation buttons
- Search settings (case sensitive, regex, multiline) via popover menu
- `apply_manager_state()` method implemented (but not called)

**⚠️ Partially Implemented:**
- Session restoration: `apply_manager_state()` method exists but **NOT CALLED** from view switching code (~20%)
...
**❌ Not Implemented:**
- **Integration with main window**: SourceView not instantiated in `Window.vala`, not added to `WindowPane.tab_view` (WindowPane exposes tab_view, Window is responsible for adding widgets)
- **State restoration workflow**: `apply_manager_state()` not called when switching to code assistant view
- Selection save/restore
- Error markers and diagnostics (GtkSource.Mark, error highlighting)
- User-configurable editor settings UI

**Integration Required:**
- **Window does NOT create SourceView or ProjectManager** - agent provides the widget via `get_widget()`
- Window gets widget from agent: `var widget = agent.get_widget()` and casts to `Gtk.Widget?`
- Add agent's widget to `WindowPane.tab_view` in `Window.vala` (WindowPane exposes tab_view, Window is responsible for adding widgets)
- **Agent Activation Signal**: Add `agent_activated(OLLMagent.BaseAgent agent)` signal to Manager (libollmchat) - Manager is the signal hub to avoid circular dependencies
- **Generic Widget Method**: Add virtual `get_widget()` method to BaseAgent returning `Object?` (nullable, default: null)
  - BaseAgent stays GTK-free by using Object? instead of Gtk.Widget
  - Default implementation returns null (agents without UI)
  - Agents with UI override this method to return their widget (cast as Object)
- **Agent provides UI component**: Agents with UI (like CodeAssistant) override `get_widget()` and return their widget
- **Window checks widget**: Window calls `agent.get_widget()` and checks if result is not null
  - If not null, casts to `Gtk.Widget?` and adds to WindowPane
  - If null, hides WindowPane (agent has no UI)
- ChatInput fires `manager.agent_activated(agent)` when agent dropdown changes
- Window listens to `manager.agent_activated` and:
  - Calls `agent.get_widget()` to get UI widget (generic check, no agent-specific logic)
  - If widget is not null, casts result to `Gtk.Widget?` and adds to WindowPane.tab_view
  - Shows WindowPane via `window_pane.show_right_pane()`
  - If widget is null, removes previous UI and hides WindowPane
- **Agent handles initialization**: When CodeAssistant's `get_widget()` is called:
  - Agent creates ProjectManager with database
  - Agent creates SourceView with ProjectManager
  - Agent loads projects: `yield project_manager.load_projects_from_db()`
  - Agent restores state: `yield project_manager.restore_active_state()`
  - Agent applies UI state: `yield source_view.apply_manager_state()`
  - Window doesn't need to know about these details - agent handles everything
- Window already has WindowPane reference, so directly call `window_pane.show_right_pane()` or `window_pane.hide_right_pane()` (no need to pass references)
...
**Research Findings:**
- ✅ **Buffer switching is feasible**: `GtkSource.View.set_buffer()` (inherited from `Gtk.TextView`) allows switching buffers at runtime
- ✅ **API supports it**: The GTK4 API explicitly supports this pattern - one view can display different buffers
- ✅ **One buffer can be shared**: A single buffer can be displayed in multiple views (though we won't need this)
- ⚠️ **View state is separate**: Cursor position, scroll position, selection are view-specific, not buffer-specific
...
- ✅ **IMPLEMENTED** - Cursor position and last_viewed timestamp
- ✅ **IMPLEMENTED** - Scroll position (first visible line) saved/restored with rate limiting
- ❌ **NOT IMPLEMENTED** - Selection save/restore
- **State to store**: Cursor position, scroll position, selection (if any)
- **Storage location**: File object properties and database
- **Database fields** (✅ all exist in FileBase table):
  - ✅ `cursor_line` (int) - Last cursor line number - **SAVED AND RESTORED**
  - ✅ `cursor_offset` (int) - Last cursor character offset - **SAVED AND RESTORED**
  - ✅ `scroll_position` (double) - Last scroll position (first visible line) - **SAVED AND RESTORED WITH RATE LIMITING**
  - ✅ `last_viewed` (int64) - Unix timestamp of last view - **SAVED AND RESTORED**
- **Restore on open**: ✅ When switching to a file, restore cursor position and scroll position from stored state
- **Save on switch**: ✅ When switching away from a file, save current buffer state (cursor, scroll position, last_viewed) to File object and database
- **Scroll position rate limiting**: ✅ Debounced with 500ms delay - only saves after scrolling stops to reduce database writes
...
- ✅ **Initial load**: Read file from disk when creating buffer - **IMPLEMENTED**
- ⏳ **File changes**: Monitor file system for changes (future: Phase 8 file watcher) - **NOT IMPLEMENTED**
- ✅ **Unsaved changes**: Track via buffer's `modified` property - **IMPLEMENTED** (connected to file.is_unsaved)
- ⚠️ **Reload prompt**: `refresh_file()` method exists but errors out instead of prompting user - **PARTIALLY IMPLEMENTED**
- ✅ **File saving**: `save_file()` method writes buffer content to file - **IMPLEMENTED**
...
├── HeaderBar (Gtk.Box, horizontal)
│   ├── ProjectDropdown (left-aligned)
│   ├── [Spacer - space for future widgets]
│   └── FileDropdown (right-aligned)
└── ScrolledWindow
    └── GtkSource.View
        └── (buffer switched dynamically)
```
...
##### Methods to Expose
- ✅ `open_file(OLLMcoder.Files.File file, int? line_number = null)` - **IMPLEMENTED** - Open/switch to file, optionally navigate to line (calls manager.activate_file())
- ✅ `open_project(OLLMcoder.Files.Folder project)` - **IMPLEMENTED** - Switch to project (calls manager.activate_project(), project must have is_project = true)
- ✅ `current_file` (property) - **IMPLEMENTED** - Returns currently active File object
- ✅ `current_project` (property) - **IMPLEMENTED** - Returns currently active project Folder object (from manager, Folder with is_project = true)
- ✅ `current_buffer` (property) - **IMPLEMENTED** - Returns current GtkSource.Buffer
- ✅ `navigate_to_line(int line_number)` - **IMPLEMENTED** - Navigate to line in current file
- ⚠️ `refresh_file()` - **PARTIALLY IMPLEMENTED** - Reload current file from disk (exists but errors out instead of prompting for unsaved changes)
- ⚠️ `apply_manager_state()` - **NOT CALLED** - Apply active project and file from ProjectManager to UI (method exists in SourceView but not called from view switching code)
- ✅ `save_file()` - **IMPLEMENTED** - Save current buffer content to file with Ctrl-S shortcut and save button
...
- ⏳ `ollmchat/Window.vala` - **NEEDS UPDATE**: 
  - **Window does NOT create SourceView or ProjectManager** - agent provides the widget
  - Listen to `manager.agent_activated` signal
  - When agent is activated, call `agent.get_widget()` and check if result is not null
  - **Widget ID Management**:
    - Check if widget already has an ID set
    - If widget has no ID, set ID to `"${agent.name}-widget"` (e.g., "code-assistant-widget")
    - Before adding widget to WindowPane.tab_view, check if a child with that ID already exists
    - If child with ID already exists, skip adding (prevents duplicate widgets)
  - **Widget Show/Hide Management**:
    - Track current active widget reference (e.g., `Gtk.Widget? current_agent_widget = null`)
    - When leaving an agent (switching to another agent or agent with null widget): 
      - If `current_agent_widget` is not null, hide it: `current_agent_widget.visible = false`
      - Set `current_agent_widget = null`
    - When entering an agent (widget is not null): 
      - Show the widget: `widget.visible = true`
      - Update `current_agent_widget = widget`
    - Widget visibility is managed as part of the agent switch process
  - **WindowPane State Management** (prevents race conditions with multiple agent switches):
    - Use a boolean flag to track intended WindowPane visibility state (e.g., `intended_pane_visible`)
    - When agent provides widget: Set flag to `true` (intended to show)
    - When agent provides null widget: Set flag to `false` (intended to hide)
    - Use `Idle.add()` to actually update WindowPane visibility from the intended state
    - This ensures that if multiple agent switches happen quickly, only the final intended state is applied
    - The idle callback checks the flag and calls `window_pane.show_right_pane()` or `window_pane.hide_right_pane()` accordingly
  - WindowPane exposes tab_view, Window is responsible for adding widgets
- ✅ `libocagent/BaseAgent.vala` - **COMPLETED**: 
  - Added virtual async method `get_widget()` returning `Object?` (default: null) - agents with UI override this
  - **No GTK types here** - BaseAgent stays GTK-free, uses Object? for widget
  - Agents without UI return null (default implementation)
- ✅ `liboccoder/Prompt/CodeAssistant.vala` - **COMPLETED**: 
  - Overrides async `get_widget()` to create and return SourceView with ProjectManager (cast as Object)
  - **Agent creates ProjectManager**: 
    - Uses database instance passed via `db` property (set in Window.vala)
    - Creates ProjectManager with database: `var project_manager = new ProjectManager(db)`
    - Stores ProjectManager via SourceView (SourceView.manager is public property)
  - **Agent creates SourceView**: 
    - Creates SourceView with ProjectManager: `var source_view = new SourceView(project_manager)`
    - Stores SourceView as instance variable for reuse (lazy initialization)
  - **Agent handles initialization** (when widget is first created):
    - Loads projects from database: `yield project_manager.load_projects_from_db()`
    - Restores active state: `yield project_manager.restore_active_state()`
    - Applies UI state: `yield source_view.apply_manager_state()`
  - **Lazy initialization**: 
    - Creates widget on first call to `get_widget()`, reuses on subsequent calls
    - Returns cached widget if already created
  - Returns null if database is not available
- ✅ `libollmchat/History/Manager.vala` - **COMPLETED**: Added `agent_activated(OLLMagent.BaseAgent agent)` signal (Manager is the signal hub, avoids circular dependencies)
- ✅ `libollmchatgtk/ChatInput.vala` - **COMPLETED**: Emits `manager.agent_activated(agent)` signal when agent dropdown selection changes (after setting session.agent_name and session.client.prompt_assistant)

**Implementation Status:**
- ✅ Core component structure complete
- ✅ Buffer management complete
- ✅ File/project switching complete
- ✅ State persistence (cursor, scroll position, last_viewed) complete
- ✅ Scroll position persistence with rate limiting (500ms debounce)
- ✅ File saving implemented (save_file() method, Ctrl-S, save button)
- ✅ Database dirty flag and periodic autosave (optional, configurable in Database constructor)
- ✅ Editor state restoration implemented and **CALLED** from CodeAssistant.get_widget() initialization
- ✅ Search functionality implemented (GtkSource.SearchContext, search UI, keyboard shortcuts)
- ✅ **Integration complete**: SourceView instantiated via CodeAssistant.get_widget(), added to WindowPane via WindowPane.add_or_show_agent_widget()
- ❌ Error markers/diagnostics not implemented

**Integration Work Required:**
1. ✅ **Agent provides UI component**: Agent implements async `get_widget()` method returning `Object?`
   - CodeAssistant agent creates and manages its own ProjectManager and SourceView
   - **Agent creates ProjectManager**: Agent receives database instance via `db` property (set in Window.vala) and creates ProjectManager: `var project_manager = new ProjectManager(db)`
   - **Agent creates SourceView**: Agent creates SourceView with ProjectManager: `var source_view = new SourceView(project_manager)`
   - **Agent handles initialization**: When widget is first created, agent:
     - Loads projects: `yield project_manager.load_projects_from_db()`
     - Restores state: `yield project_manager.restore_active_state()`
     - Applies UI state: `yield source_view.apply_manager_state()`
   - Agent returns the UI widget (cast as Object) when asked
   - **Lazy initialization**: Create widget on first call, reuse on subsequent calls
2. ✅ Window calls `agent.get_widget.begin()` asynchronously and handles result in callback
3. ✅ If widget is not null, Window casts to `Gtk.Widget?` and uses WindowPane.add_or_show_agent_widget() to add/show widget in WindowPane.tab_view
4. **Agent Activation Signal Design** (Avoiding Circular Dependencies):
   
   **Problem**: ChatInput (libollmchatgtk library) doesn't have access to WindowPane (ollmchat application). If we pass WindowPane to agent signal, we'd create circular dependencies:
   - libollmchatgtk → ollmchat (❌ library shouldn't depend on application)
   - libocagent → ollmchat (❌ base library shouldn't depend on application)
   
   **Solution**: Use Manager (libollmchat) as signal hub - both ChatInput and Window have access to it:
   - **Add `agent_activated(OLLMagent.BaseAgent agent)` signal to Manager** (libollmchat)
   - ChatInput fires signal when agent dropdown changes (after setting `session.agent_name` and `session.client.prompt_assistant`)
   - **Add `get_widget()` method to BaseAgent returning `Object?` (nullable)**
     - BaseAgent (libocagent) has virtual method `get_widget()` returning `Object?` (default: null)
     - Agents without UI return null (default implementation)
     - CodeAssistant (in liboccoder, has GTK) overrides `get_widget()` to return `Gtk.Widget` (cast as Object)
     - Window calls `agent.get_widget()` and checks if result is not null
     - If not null, casts result to `Gtk.Widget?` and adds to WindowPane
     - ✅ **Simple and clean**: No GTK dependency in libocagent, no `has_ui` property needed
     - ✅ **Generic**: Window just checks `get_widget() != null` - no agent-specific logic
   - **No agent-specific logic in Window** - Window only checks if widget is null and casts if needed
   
   **Architecture Benefits**:
   - ✅ No circular dependencies (Manager is in libollmchat, accessible to both)
   - ✅ Clean separation: ChatInput doesn't know about WindowPane, Window doesn't know about ChatInput internals
   - ✅ **Generic design**: Window doesn't know about specific agent names - only checks if `get_widget() != null`
   - ✅ **Agent provides UI**: Agents with UI override `get_widget()` to return widget, others return null
   - ✅ **Agent provides UI component**: Agent creates and manages its own UI (ProjectManager, SourceView, etc.)
   - ✅ **Window doesn't create agent UI**: Window only asks agent for its UI component and displays it
   - ✅ Follows existing pattern: Manager is already the central signal hub (session_activated, stream_chunk, etc.)
   - ✅ Agent signal just passes agent object, Window checks generic properties to decide what UI to show/hide
   
   **Dependency Flow** (No Circular Dependencies):
   ```
   libocagent (BaseAgent) ← used by → libollmchat (Manager) ← used by → libollmchatgtk (ChatInput)
                                                                          ↓
                                                                    ollmchat (Window + WindowPane)
                                                                    (application, not a library)
   
   Signal Flow:
   ChatInput.agent_dropdown → manager.agent_activated(agent) → Window.agent_activated handler → WindowPane.show/hide
   
   Note: WindowPane is in ollmchat application (not libollmchatgtk), so ChatInput can't access it directly.
   Manager (libollmchat) is accessible to both ChatInput (libollmchatgtk) and Window (ollmchat).
   ```
   
   **Why This Works**:
   - Manager (libollmchat) is accessible to both ChatInput (libollmchatgtk) and Window (ollmchat)
   - ChatInput doesn't need to know about WindowPane
   - Window doesn't need to know about ChatInput internals
   - BaseAgent (libocagent) doesn't need to know about Window/WindowPane
   - All communication goes through Manager signal hub
   

5. ✅ Implement view switching logic when agent provides widget:
   - **When agent is activated** (agent_activated signal handler in Window.vala):
     - **Hide previous agent's widget** (if any):
       - Tracks current widget reference: `current_agent_widget`
       - If previous widget exists, hide it: `previous_widget.visible = false`
     - Call `agent.get_widget.begin()` asynchronously to get UI component
     - **If widget is not null**:
       - **Widget ID Management**:
         - Set widget ID: `widget.name = agent.name + "-widget"` (before calling WindowPane method)
         - WindowPane.add_or_show_agent_widget() handles:
           - Checks if widget with ID already exists in tab_view using `tab_view.get_child_by_name(widget_id)`
           - If exists, uses existing widget (avoids unparenting)
           - If not exists, adds widget to tab_view (only if not already parented)
           - Shows widget and sets as visible child
       - **Set WindowPane intended state**: Set property `intended_pane_visible = true`
       - **Schedule WindowPane update**: Call `window_pane.schedule_pane_update()` which uses `Idle.add()` to update visibility
       - Agent's UI component handles its own initialization (loading projects, restoring state, etc.)
     - **If widget is null** (or when switching away):
       - **Set WindowPane intended state**: Set property `intended_pane_visible = false`
       - **Schedule WindowPane update**: Call `window_pane.schedule_pane_update()`
   - **WindowPane.schedule_pane_update() method**:
     - Uses `Idle.add()` to apply `intended_pane_visible` state
     - If `true`: Calls `window_pane.show_right_pane()`
     - If `false`: Calls `window_pane.hide_right_pane()`
     - Returns `false` to remove from idle queue (one-time callback)
   - **WindowPane.intended_pane_visible property**:
     - Simple property that stores intended state
     - `schedule_pane_update()` applies the state via idle callback
   
6. ✅ **Add generic method to BaseAgent** (libocagent):
   - Added virtual async `get_widget()` method returning `Object?` (default: null)
   - CodeAssistant agent:
     - Overrides async `get_widget()` to create/return SourceView with ProjectManager (cast as Object)
     - Agent manages ProjectManager lifecycle and SourceView creation
     - Returns null if database is not available
   - JustAsk and other agents:
     - Use default implementation (returns null)
...
- ✅ **IMPLEMENTED** - Methods exist but **NOT CALLED** from view switching code
- **Purpose**: Automatically restore the last active project, active file, cursor position, and scroll position when user switches to code assistant view
- **Separation of Concerns**:
  - `ProjectManager` is responsible for all database access and data loading
  - `SourceView` is responsible for applying UI state from manager properties
- **Implementation**:
  1. **ProjectManager.load_projects_from_db()**: Loads all projects from database (called when switching to code assistant view)
  2. **ProjectManager.restore_active_state()**: Finds active project and file from in-memory data, sets `this.active_project` and `this.active_file` directly using `activate_project()` and `activate_file()` methods
  3. **SourceView.apply_manager_state()**: Applies UI state from `manager.active_project` and `manager.active_file` properties
- **Internal Methods**:
  - `ProjectList.get_active_project()`: Returns active project from projects list
  - `ProjectFiles.get_active_file()`: Returns active file from project files list
- **Workflow** (when switching to code assistant view):
  1. Call `manager.load_projects_from_db()` to load projects from database
  2. Call `yield manager.restore_active_state()` to restore active project and file (sets manager properties)
  3. Call `yield source_view.apply_manager_state()` to apply UI state (opens project/file in editor)
- **Status**: Methods implemented in both ProjectManager and SourceView, but **NOT CALLED** from view switching code. **Integration required in Window.vala or ChatWidget.vala**.
...
⏳ **NOT IMPLEMENTED** - Error markers and diagnostics not yet added to SourceView.
...
✅ **IMPLEMENTED** - Search functionality fully implemented in SourceView.

**Feature**: In-buffer search with highlighting and navigation.

**Implementation**:
- ✅ **GtkSource.SearchContext**: Uses `GtkSource.SearchContext` with `GtkSource.SearchSettings`
- ✅ **Search options**: Case sensitive, regex, multiline support (via SearchSettings checkboxes)
- ✅ **Navigation**: Forward/backward search with keyboard shortcuts (Ctrl+G, Ctrl+Shift+G)
- ✅ **Search UI**: Search entry widget in footer bar below the code editor
- ✅ **Result display**: Shows match count (e.g., "3 Matches" or "No Matches")
- ✅ **Highlighting**: Uses `searchcontext.set_highlight(true)` to highlight all matches
- ✅ **Keyboard shortcuts**:
  - Ctrl+F: Focus search entry and select all text
  - Ctrl+G: Forward search (find next)
  - Ctrl+Shift+G: Backward search (find previous)
  - Enter: Forward search when search entry is focused

**UI Components**:
- Search entry (`Gtk.SearchEntry`) with placeholder "Search..."
- Results label showing match count
- Next/Previous navigation buttons
- Settings menu button with popover containing:
  - Case sensitive checkbox
  - Regex checkbox
  - Multi-line checkbox (converts `\n` to newlines)

**Implementation Details**:
- Search context is created when search text changes
- Search context resets when switching files
- Search position (`last_search_end`) tracks current match position
- Forward/backward navigation methods handle wrap-around (disabled)
- Search settings can be changed dynamically and trigger re-search

**Status**: Fully functional and integrated into SourceView component.
...
⏳ **TODO** - CodeAssistantProvider still uses OpenFile tracking, not integrated with ProjectManager yet.
...
### Phase 6: SourceView inline diff (URGENT)

📋 **SPLIT TO SEPARATE PLAN** — See [CODER-4.2.3-URGENT-source-view-diff.md](../CODER-4.2.3-URGENT-source-view-diff.md)

**URGENT** · **DESIGN OPEN** — Visual diff in `SourceView` (secondary gutter, green/red). **Implementation blocked** until partial-approval / diff-baseline design closes — see [CODER-4.2.3-URGENT-source-view-diff.md](../CODER-4.2.3-URGENT-source-view-diff.md). Whole-file Approve/Reject on `Approvals` bar already shipped (2.10.4.26).

**Original Phase 6 notes (superseded by 4.2.3 for UI; keep for context):**
...
📋 **MOVED TO SEPARATE PLAN** - See [CODER-4.2.2-code-editor-manager-tool.md](CODER-4.2.2-code-editor-manager-tool.md)

⏳ **TODO** - CodeEditorManager tool not yet implemented.

---

### Phase 8: Background Sync and File Watcher

📋 **MOVED TO SEPARATE PLAN** - See [APP-4.2.3-background-sync-file-watcher.md](APP-4.2.3-background-sync-file-watcher.md)

⏳ **TODO** - Background sync process and file system watcher for real-time updates.
...
**liboctools/ (EditMode Tool, namespace: `OLLMtools`):**
- `liboctools/RequestEditMode.vala` - Need to save file copy before applying changes
- `liboctools/EditMode.vala` - May need to track which files were edited
