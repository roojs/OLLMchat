...
- **Tool-Specific Settings UI Extensions**: Allow tools to add custom UI elements to their settings panel (e.g., buttons, reports). For example, the run_command tool could add a "Cleanup Playground" button and a playground size report to its settings panel.
