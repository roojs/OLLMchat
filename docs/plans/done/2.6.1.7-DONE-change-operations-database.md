...
⏳ **PLANNING** - Design phase. Working out details in planning.
...
- For **new files**: `filebase_id` = 0, `backup_path` = "" (empty string, no backup needed), `change_type="added"`, `base_type` = type of new file ("f", "d", "fa")
  - **Note**: Even for new files, FileHistory constructor requires a FileBase object (create a fake/temporary one if needed)
    ...
    public class FileHistory : Object
    {
        ...
        public async void commit() throws Error
        {
            ...
            if (this.change_type == "modified" || this.change_type == "deleted") {
                if (this.filebase_object.path == "") {
                    throw new GLib.IOError.FAILED("Cannot create backup: path is missing");
                }
...
- `filebase_object` (FileBase) - FileBase object (File or Folder) - required (even for new files, create a fake/temporary one)
...
- Create `libocfiles/FileHistory.vala` - FileHistory class with database operations
  ...
  - Implement constructor, commit(), create_backup(), save_to_db(), init_db()
  - Query methods will be added later when needed
...
**Phase 2: Integration (See 2.6.1.8)**
- Phase 2 has been moved to a separate plan: `2.6.1.8-integrate-change-system.md`
- This will cover integration with runcommand, overlay, editmode, and filebase
