...
- **Coding standards:** Verify this plan against **docs/coding-standards.md** (nullable types, null checks, string interpolation, temporaries, brace placement, `this.` prefix, GLib prefix, property init, line length, Gee.HashMap `.set()`/`.get()`, etc.) before implementing.
...
GLib.Idle.add(() => {
	...
	var cur = this.cellwidths.has_key(local_col) ? this.cellwidths.get(local_col) : 0;
