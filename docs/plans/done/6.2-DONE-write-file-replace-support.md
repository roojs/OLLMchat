# Plan 6.2 (DONE): Write tool — `search_text` replace (`liboctools/WriteFile` only)
...
**Done** — Shipped in [`Request.vala`](../../../liboctools/WriteFile/Request.vala)
and [`Tool.vala`](../../../liboctools/WriteFile/Tool.vala): **`search_text`**
JSON field, **`validate()`** (**`locate`**, exactly one match), **`execute_request`**
derives **`start_line` / `end_line`** then uses the existing line-range
**`FileChange`** path (**`apply_edit`**). **Coding standards:**
