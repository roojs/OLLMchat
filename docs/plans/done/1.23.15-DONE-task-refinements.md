...
**Scope:** Task classes only (liboccoder/Task/: List, Step, Details, ResultParser, **Tool**). Runner → 1.23.14. PromptTemplate / template changes → **1.23.17**.

**Coding standards:** Follow `docs/coding-standards.md`. When implementing:
...
- **TODO:** Remove references to `#project-description` in the skill prompts (e.g. `task_refinement.md`, `task_creation_initial.md`, `task_list_iteration.md`, `task_execution.md`). Project description is now always injected as a dedicated **## Project Description** section in refinement and executor prompts, so tasks do not need to list it in References and the template copy that tells the model to use `[Project description](#project-description)` can be dropped or simplified.
...
- 1.23.13 (task-related code, DONE); 1.23.14 (existing skill code updates); 1.23.12 (prompt refinements); 1.23.16 (heading_text_to_anchor); 1.23.17 (template changes).
