...
**Coding standards:** Follow `docs/coding-standards.md`. Checklist: nullable avoided where possible; null checks only where design requires; no `@"..."`; no one-use temporaries (inline); braces inline for control flow; **this.**; **GLib.** prefix; default init on properties; **string[]** and **joinv** for string arrays; **continue** in loops; no character loops; try/catch only around minimal throwing code; no leading underscore; no get_* accessors. This plan uses: **step.children**, **children.get(0)** (Gee .get()); **steps** as public property (not steps()).
