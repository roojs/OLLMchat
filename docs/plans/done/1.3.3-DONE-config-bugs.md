...
✅ **COMPLETED** - Configuration migration working. Gee collection serialization fixed. Options serialization with underscore/hyphen handling fixed. All code references to old config properties have been updated.

## Completed Items

✅ **Embed Config Loading** - Integrated into Config2 with auto-creation via static methods  
✅ **Gee Collection Serialization** - Fixed serialization for connections, model_options, usage, and hidden_models maps  
✅ **Options Serialization** - Fixed to save all properties including num_ctx for embed/analysis configs  
✅ **Underscore/Hyphen Conversion** - Options serialize with hyphens for config, convert to underscores for Ollama API  
✅ **Config Migration** - Config1 to Config2 migration working correctly  
...
**Status**: ✅ **IMPLEMENTED** - Auto-creation of embed/analysis config in Config2 via static methods. Config registration working. Options serialization with all properties (including num_ctx) working correctly. Gee collection serialization fixed.
...
When converting Config1 to Config2 (see [1.3.1-configuration-classes.md](1.3.1-DONE-configuration-classes.md) for Config2 structure):
...
**Note**: This is a refactoring task, not adding new functionality. We are preserving existing behavior using the Config2 design (see [1.3.1-configuration-classes.md](1.3.1-DONE-configuration-classes.md)).
...
- ✅ `libollmchat/Settings/Config1.vala` - `toV2()` method implemented, preserves model and title_model values
- ✅ `libollmchat/Settings/Config2.vala` - Properties and storage for model/title_model implemented in usage map. Gee collection serialization fixed. Default types registered.
- ✅ All code that accesses `client.config.model`, `client.config.title_model`, `client.config.think` - Verified: no references found, code has been updated to use new structure
