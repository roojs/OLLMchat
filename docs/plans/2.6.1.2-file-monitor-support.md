...
- **Multiple events for same file**: Lists handle duplicates (check before adding)
- **Create-then-delete quickly**: File removed from `new_files`, added to `deleted_files`
- **Modify-then-delete**: File removed from `modified_files`, added to `deleted_files`
...
- [ ] Implement `is_directory()` helper method
...
6. Handle method → Updates change lists and known_files
7. Directory created → New Monitor created and started (recursive)
8. stop() called → All monitors stopped, lists finalized
9. Properties accessed → Change lists available for use
```
