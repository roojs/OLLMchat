...
- [ ] Create `libocvector/BackgroundScan.vala`
  ...
  - **Thread Management**: 
    ...
    - Thread is started when window initializes
    - Thread remains running for the lifetime of the application
  - **Queue System**:
    - Maintains a thread-safe queue of files to scan (using `Gee.ArrayList<string>` with mutex protection, or similar thread-safe queue structure)
    - Queue stores file paths (strings) that need to be indexed
    - Queue is accessed from both main thread (when adding items) and background thread (when processing)
    - Queue processing runs in background thread context
  - **Methods** (called from main thread):
    - `scanProject(File project)` - Called when user switches to a project in UI
      - If background thread not running, start it (via `ensure_thread()`)
      - Dispatches `queueProject(string path)` to background thread (uses MainContext.invoke or similar)
    - `scanFile(File file)` - Called when a file is saved
      - If background thread not running, start it (via `ensure_thread()`)
      - Dispatches `queueFile(string path)` to background thread (uses MainContext.invoke or similar)
  - **Methods** (executed in background thread):
    - `queueProject(string path)` - Processes project queue item
      - Does standard project load: Uses ProjectManager to load project from database
      - Iterates through project files (via `project.project_files` or `project.children.items`)
      - For each file, checks if modified since last scan (`file.last_scan < file.mtime_on_disk()`)
      - For files that need scanning, calls `queueFile(string path)` to add to file queue
    - `queueFile(string path)` - Adds file to scan queue
      - Adds file path to queue (thread-safe operation with mutex)
      - If queue processing not running, starts `startQueue()` method (via Idle.add or similar)
    - `startQueue()` - Processes file queue
      - Runs in background thread context (via Idle callback or async method)
      - Continuously pops items from file queue (thread-safe pop operation)
      - For each file path:
        - Loads File object from database (via ProjectManager)
        - Creates Indexer instance (if not already created, reuse existing instance)
        - Calls `indexer.index_file(file)` to scan the file (async operation)
      - Keeps pulling items from queue until queue is empty
      - Stops queue processing when queue is empty
  - **Dependencies**:
    ...
    - Requires `OLLMfiles.ProjectManager` for file operations
  - **Signals**:
    - `file_scanned(string file_path)` - Emitted when a file scan completes
    - `project_scan_started(string project_path)` - Emitted when project scan starts
    - `project_scan_completed(string project_path, int files_indexed)` - Emitted when project scan completes
- [ ] Window Integration
  - **Window Initialization**: 
    - When window starts, create `BackgroundScan` instance
    - Start background thread (call `ensure_thread()` or similar)
    - Store `BackgroundScan` instance in window
  - **Project Switching**:
    - Connect to `ProjectManager.active_project_changed` signal
    - When project changes, call `background_scan.scanProject(project)`
  - **File Saving**:
    - When a file is saved, the code calls `ProjectManager.notify_file_changed(file)`
    - Window connects to a signal from ProjectManager (either existing `notify_file_changed` or new `file_saved` signal)
    - When signal is emitted, window calls `background_scan.scanFile(file)`
    - **Implementation Note**: The file save happens in `SourceView.save_file()` or `FileBuffer.sync_to_file()`, which then calls `ProjectManager.notify_file_changed()`. Window should connect to this signal to trigger background scanning.
- [ ] ProjectManager Integration (if needed)
  - **File Save Signal**: 
    - Option 1: Use existing `notify_file_changed(File file)` signal - Window can connect to this and call `BackgroundScan.scanFile()` when emitted
    - Option 2: Add new signal `file_saved(File file)` to ProjectManager specifically for file saves (more explicit, but requires ProjectManager changes)
    - **Recommendation**: Start with Option 1 (use existing signal), add Option 2 later if needed for better separation of concerns
- [ ] Build System Updates
  - Add `libocvector/BackgroundScan.vala` to `libocvector/meson.build`
  - Ensure all dependencies are available (libocvector, libocfiles, libollmchat, libocsqlite)
