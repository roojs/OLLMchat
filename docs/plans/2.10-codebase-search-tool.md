...
- [ ] Add GPU support detection and usage
  - **NOTE**: Cannot be done at present - FAISS library was only built with CPU support
  - See TODO at bottom of document for investigating FAISS packaging and Debian build modifications
- [ ] Implement CPU fallback
  - **NOTE**: Since GPU support is not available at present, this will use CPU-only operations
  - Ensure all FAISS operations use CPU-based implementations
...
- [ ] Create `libocvector/Search/QueryProcessor.vala`
  - High-level query processing class that coordinates search operations
  - Handles query preprocessing (normalization, expansion)
  - Manages query vectorization (converting text to embeddings)
  - Coordinates between Searcher, metadata lookup, and result formatting
  - Provides unified API for different query types
- [ ] Implement query type detection and routing
  - Analyze query text to determine search intent:
    - Natural language queries: "find function that validates user input"
    - Code pattern searches: "factory pattern", "singleton", "observer"
    - Function/class name searches: "DatabaseManager", "execute_query"
    - Description-based searches: "code that handles authentication"
  - Route to appropriate search handler based on detected type
  - Support query expansion for pattern searches (e.g., "factory" → "factory pattern creation instantiation")
  - Handle multi-intent queries (combine different search strategies)
- [ ] Implement result ranking and formatting
  - **Ranking enhancements:**
    - Beyond FAISS similarity score, consider:
      - Recency (recently modified files may be more relevant)
      - File importance (e.g., core library files vs test files)
      - Element type relevance (function vs class vs variable)
      - Context matching (exact name matches rank higher)
    - Group related results (same file, same class, etc.)
    - Deduplicate near-identical results
  - **Result formatting:**
    - Extract code snippets from file_path + line_range (from metadata)
    - Format for display: file path, line numbers, element name, code snippet
    - Highlight relevant portions of code snippets
    - Generate summary descriptions for each result
    - Format for different output types (CLI, JSON, tool response)
