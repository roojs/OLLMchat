...
- [x] Code snippet extraction: Uses `CodeFile.lines` array with Vala array slicing for efficient extraction
...
- [x] Create `libocvector/Indexing/VectorBuilder.vala`
  - ✅ Done: Takes `CodeFile` from Analysis layer (single file with elements)
  - ✅ Done: Formats each `CodeElement` into document for vectorization
