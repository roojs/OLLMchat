...
- [ ] Implement batch processing for vector generation
  - ⚠️ Partially done: `Database.add_documents()` accepts multiple texts
  - ⚠️ Currently processes embeddings one-by-one in loop
  - ⚠️ Should use `embed_array()` for true batch processing (10-20 documents per batch)
...
- [ ] Implement metadata storage in SQL database
  ...
  - Implement lookup for search results (vector_id → file path + code snippet)
