...
- [ ] ProjectManager Integration
  ...
  - **Update FileBuffer Integration**:
    ...
    - Should also call `file.manager.on_file_contents_change(file)` after writing content
    - This ensures both `sync_to_file()` and `write()` trigger the signal
  - **Edit Mode Integration**:
    - After `RequestEditMode.apply_all_changes()` successfully applies edits, it should call `project_manager.on_file_contents_change(file)` to trigger the signal
    - Currently edit mode uses `FileBuffer.write()` and `FileBuffer.apply_edits()` which emit `file.changed()` signal
    - Since `FileBuffer.update_file_metadata_after_write()` will call `on_file_contents_change()`, edit mode should automatically trigger the signal
