...
- [ ] Create `libocvector/` directory structure
- [ ] Create `libocvector/Files/` directory for FileBase structure
- [ ] Set up `libocvector/meson.build` for the library
- [ ] Create namespace structure (`OLLMvector.Files`, `OLLMvector.Indexing`, `OLLMvector.Search`, `OLLMvector.Tools.Search`, `OLLMvector.Tool`)

### Phase 2: FAISS Migration
- [ ] Migrate `VectorSearch/Index.vala` → `libocvector/Index.vala` (FAISS namespace)
- [ ] Migrate `VectorSearch/Database.vala` → `libocvector/Database.vala`
- [ ] Update Database namespace to `OLLMvector`
- [ ] Integrate Database with `OLLMvector.Files.File` for metadata tracking

### Phase 3: Analysis Layer Implementation
- [ ] Create `libocvector/Indexing/Analysis.vala`
- [ ] Implement LLM API integration with structured outputs (`format` parameter)
- [ ] Implement JSON schema for code analysis response
...
- [ ] Copy `liboccoder/Files/FileBase.vala` → `libocvector/Files/FileBase.vala`
- [ ] Copy `liboccoder/Files/File.vala` → `libocvector/Files/File.vala`
- [ ] Copy `liboccoder/Files/Folder.vala` → `libocvector/Files/Folder.vala`
...
- [ ] Create `examples/oc-vector-index.vala` (single file indexing tool)
...
- [ ] Update root `meson.build` to include `libocvector` subdirectory
- [ ] Add dependencies to `libocvector/meson.build` (libocsqlite, libollmchat, FAISS)
- [ ] Note: liboccoder is NOT a dependency (FileBase structure is copied)
- [ ] Generate VAPI files for `libocvector`
- [ ] Generate GIR files for `libocvector`
- [ ] Update library dependencies in consuming projects
...
- **Format Parameter**: JSON schema object to enforce structured output:
  ```json
  {
    "type": "object",
    "properties": {
...
Alongside each vector, the following metadata is stored in the FAISS index mapping:
- Original file path
- Element type and name
- Line range (start_line, end_line)
- Document text (for result display)
- Vector ID in FAISS index
- Timestamp of vectorization

This metadata enables:
- Precise result location (file + line numbers)
- Result context display (showing relevant code snippets)
- Incremental updates (re-vectorizing only changed elements)
