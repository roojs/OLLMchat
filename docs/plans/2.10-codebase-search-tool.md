...
- [ ] Window Integration
  - **Window Initialization**: 
    ...
    - Store `BackgroundScan` instance in window (thread starts lazily on first use)
  - **Project Switching**:
    - Connect to `ProjectManager.active_project_changed` signal
    - When project changes, call `background_scan.scanProject(project)`
  - **File Saving**:
    - ProjectManager's `notify_file_changed(File file)` method needs to be updated to also trigger background scanning
    - When `notify_file_changed(file)` is called, it should call `background_scan.scanFile(file, active_project)`
    - **Implementation Note**: `notify_file_changed()` is currently a method (not a signal) that saves file to database. It needs to be updated to also trigger background scanning with the active project.
    - **Current callers of `notify_file_changed()`**:
      - `SourceView.save_current_file_state()` - called when switching files or scrolling
      - `SourceView.open_file()` - called when opening a file
      - `SourceView` scroll timeout - called when scroll position changes
    - **Edit Mode**: Edit mode uses `FileBuffer.write()` and `FileBuffer.apply_edits()` which call `file.changed()` signal, but do NOT call `notify_file_changed()`. Need to ensure edit mode also calls `notify_file_changed()` after applying edits.
- [ ] ProjectManager Integration
  - **Update `notify_file_changed()` method**:
    - Add `BackgroundScan` parameter or property to ProjectManager
    - When `notify_file_changed(File file)` is called, also call `background_scan.scanFile(file, this.active_project)` if background_scan is available
    - This ensures both file save button and edit mode trigger background scanning
  - **Edit Mode Integration**:
    - After `RequestEditMode.apply_all_changes()` successfully applies edits, it should call `project_manager.notify_file_changed(file)` to trigger background scanning
    - Currently edit mode uses `FileBuffer.write()` and `FileBuffer.apply_edits()` which emit `file.changed()` signal but don't call `notify_file_changed()`
    - **Options**:
      - Option A: Add `notify_file_changed()` call in `RequestEditMode.apply_all_changes()` after successful edit
      - Option B: Connect to `file.changed()` signal in ProjectManager and call `notify_file_changed()` from there
    - **Recommendation**: Option A is cleaner - explicitly call `notify_file_changed()` after edits are applied
