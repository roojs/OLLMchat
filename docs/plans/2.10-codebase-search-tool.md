...
- [ ] Window Integration
  ...
  - **File Saving**:
    - ProjectManager needs two separate methods:
      - `notify_file_metadata_changed(File file)` - for metadata changes (cursor position, scroll position, last_viewed) - saves to database but does NOT trigger background scanning
      - `notify_file_content_changed(File file)` - for content changes (file saved, edited) - saves to database AND triggers background scanning
    - When `notify_file_content_changed(file)` is called, it should call `background_scan.scanFile(file, active_project)` if background_scan is available
    - **Current callers that should use `notify_file_metadata_changed()`**:
    ...
    - **Current callers that should use `notify_file_content_changed()`**:
...
- [ ] ProjectManager Integration
  - **Split `notify_file_changed()` into two methods**:
    - `notify_file_metadata_changed(File file)` - replaces current `notify_file_changed()` for metadata-only updates
      - Saves file to database
      - Does NOT trigger background scanning
    - `notify_file_content_changed(File file)` - new method for content changes
      - Saves file to database
      - Triggers background scanning: calls `background_scan.scanFile(file, this.active_project)` if background_scan is available
  - **Add `BackgroundScan` property to ProjectManager**:
    - Add optional `BackgroundScan? background_scan` property to ProjectManager
    - Set by Window during initialization
    - Used by `notify_file_content_changed()` to trigger background scanning
  - **Update FileBuffer Integration**:
    - `FileBuffer.update_file_metadata_after_write()` currently calls `file.changed()` signal
    - Should also call `file.manager.notify_file_content_changed(file)` after writing content
    - This ensures both `sync_to_file()` and `write()` trigger background scanning
  - **Edit Mode Integration**:
    - After `RequestEditMode.apply_all_changes()` successfully applies edits, it should call `project_manager.notify_file_content_changed(file)` to trigger background scanning
    - Currently edit mode uses `FileBuffer.write()` and `FileBuffer.apply_edits()` which emit `file.changed()` signal
    - Since `FileBuffer.update_file_metadata_after_write()` will call `notify_file_content_changed()`, edit mode should automatically trigger scanning
    - **Note**: May need to verify that `FileBuffer.write()` and `apply_edits()` both go through `update_file_metadata_after_write()`
