...
- [ ] Add `last_scan` field to `filebase` table
  ...
  - For files: `last_scan` = timestamp when scan completed
  - For folders: `last_scan` = timestamp when scan started (to prevent re-recursion)
...
- [ ] Implement incremental update logic
  ...
  - For folders: check `folder.last_scan` to avoid re-recursion
  - Update `file.last_scan` after successful indexing (time of completion)
  - Update `folder.last_scan` at start of folder scan (time of start)
- [ ] Implement folder-based indexing
  - Starting point: folder in database (query by path or id)
  - Option to recurse into subfolders
...
- [ ] Update `oc-vector-index` example tool
  - Use `~/.local/share/ollmchat/files.sqlite` as database path
  - Add `--folder` option to specify starting folder (path or id)
  - Add `--recurse` flag for recursive indexing
  - Store vector database at `~/.local/share/ollmchat/codedb.faiss.vectors`
  - Load existing folder from database as starting point
