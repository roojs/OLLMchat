...
🚧 **IN PROGRESS** - Foundation, FAISS migration, Tree layer (Phase 3a), and Analysis layer (Phase 3) completed. Vector Building refactoring and Search layer pending.
...
- [ ] Create `libocvector/Search/SearchBuilder.vala`
  - Builder class that creates and configures search operations
  - **Usage pattern**: Create one instance per query, set properties, then call `build()`
  - **Properties** (set directly, no setters):
    - `query` (string, required) - Search query text
    - `folder` (OLLMfiles.Folder?, optional) - Filter by Folder (project/folder scope)
    - `language` (string?, optional) - Filter by File.language property
    - `element_type` (string?, optional) - Filter by VectorMetadata.element_type (e.g., "class", "method", "function")
    - `max_results` (int, default: 10) - Maximum number of results
    - `embedding_client` (OLLMchat.Client, required) - Embedding client for query vectorization
  - **Methods**:
    - `build()` - Returns configured `Search` object ready to execute
    - Builds SQL filter query to get list of vector_ids matching filter criteria
      - Uses SQL JOINs: `vector_metadata` → `filebase` (File) → `project_file` (for folder/project filtering)
      - Example SQL filter query:
        ```sql
        SELECT vm.vector_id FROM vector_metadata vm
        JOIN filebase f ON vm.file_id = f.id
        WHERE f.language = 'vala'
        AND vm.element_type = 'method'
        ```
- [ ] Create `libocvector/Search/Search.vala`
  - Executes the actual search operation
  - **Constructor**: Takes SearchBuilder or all required dependencies
  - **Dependencies**:
    ...
    - `OLLMfiles.ProjectManager` - For file operations (passed to SearchResult for code snippet extraction)
  - **Methods**:
  ...
  - **Implementation**:
    ...
    - Get filtered vector_ids from SearchBuilder (builder builds the SQL filter query)
    ...
    - Create SearchResult[] array, passing SQ.Database and ProjectManager to each SearchResult constructor
  ...
  - **GPU/CPU support**:
    ...
    - Structure code to support future GPU fallback when GPU support becomes available
    - Log which backend (CPU/GPU) is being used for debugging
    - Future enhancement: When GPU support is added, implement automatic fallback to CPU if GPU operations fail
- [ ] Create `libocvector/Search/SearchResult.vala`
  - Represents a single search result
  - **Constructor dependencies**:
    - `SQ.Database` - SQL database for file lookup and VectorMetadata access
    - `OLLMfiles.ProjectManager` - For file operations (to read code snippets)
  - **Properties**:
    ...
    - `code_snippet` (string) - Realized property: Extracts code snippet from file_path + line_range on demand
      - Uses metadata.file_id to lookup File via ProjectManager
      - Reads file content and extracts lines using metadata.start_line and metadata.end_line
  - **Methods**:
    - `to_string()` - Format for display (CLI/LLM tool response - same format)
    - `to_json()` - Serialize to JSON format
    - Search class can loop through SearchResult[] and call these methods for output
...
├── Search/
│   ├── SearchBuilder.vala      # Builder for creating and configuring searches
...
- `libocvector/Search/SearchBuilder.vala`
