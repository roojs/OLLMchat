...
🚧 **IN PROGRESS** - Foundation and FAISS migration completed. Core indexing and search components pending.
...
- [ ] Implement tree-sitter parser integration using `TreeSitter` namespace from `vapi/tree-sitter.vapi`
- [ ] Implement language detection and tree-sitter language selection based on file language
- [ ] Implement AST traversal to extract code elements (classes, functions, methods, etc.)
- [ ] Create `VectorMetadata` objects from tree-sitter nodes with line numbers
- [ ] Extract code documentation blocks using tree-sitter API (doc comments, JSDoc, etc.) related to each element
- [ ] Add `codedoc_start` and `codedoc_end` properties to `VectorMetadata` class (line numbers only, not stored in DB, used for analysis)
- [ ] Tree class: Constructor takes `OLLMfiles.File` as argument, stores as property
- [ ] Tree class: Add `elements` property (Gee.ArrayList<VectorMetadata>) to store VectorMetadata array
- [ ] Tree class: Add `lines` property (string[]) containing file content split into lines
- [ ] Tree class: Add `lines_to_string(int start_line, int end_line)` method using Vala string range syntax `lines[start:end]`

### Phase 3: Analysis Layer Implementation
- [x] Create `libocvector/Indexing/Analysis.vala`
- [ ] Refactor Analysis constructor to receive `Tree` object as argument
- [ ] Analysis iterates over `Tree.elements` and updates VectorMetadata descriptions
- [ ] Load prompt templates from resources (use `---` separator between system and user prompts)
- [ ] Implement simple LLM call that sends code + documentation for summarization
- [ ] Return one-line text descriptions (not structured JSON)
- [ ] Store descriptions in `VectorMetadata` objects as property (not stored in DB, used by VectorBuilder)
...
- [x] Create `libocvector/Indexing/VectorBuilder.vala`
  ...
  - [ ] Refactor: Update constructor to receive `Tree` object as argument
  - [ ] Refactor: Use `Tree.elements` for VectorMetadata array
  - [ ] Refactor: Use `Tree.lines_to_string()` method for code snippet extraction
...
- ✅ CodeFile/CodeElement refactoring: Removed `raw_code`, added `lines` array for efficient snippet extraction, added `file` reference
- ✅ Kebab-case handling: Automatic conversion from JSON (`property-type` → `property_type`) using `replace("-", "_")`
...
- [ ] **NEW**: Tree layer will use tree-sitter to create `VectorMetadata` objects directly (replacing CodeFile/CodeElement flow)
- [ ] **NEW**: `VectorMetadata` will have `codedoc_start` and `codedoc_end` properties (line numbers only, not stored in DB) for documentation blocks

### Phase 5: Indexer Orchestration
- [ ] Create `libocvector/Indexing/Indexer.vala`
- [ ] Integrate Tree, Analysis, and VectorBuilder components
...
The `VectorMetadata` class will be extended with:
...
- `description` property (string, not stored in DB): Contains LLM-generated one-line text description
  - Populated by Analysis layer (one-line text, not structured JSON)
...
#### Layer 2: Analysis
- **Purpose**: Send code elements (from Tree layer) to LLM for summarization, generating one-line text descriptions
- **Input**: 
  - `Tree` object from Tree layer (contains `elements` array with VectorMetadata objects)
- **Output**: `Tree` object with `elements` array updated (each `VectorMetadata.description` populated)
- **Responsibilities**:
  - Receive `Tree` object from Tree layer
...
The Analysis layer receives `VectorMetadata[]` from the Tree layer and generates simple text descriptions using LLM.
