...
- **Minimum required fields** (for LLM tool):
...
- **Optional fields** (for advanced features):
  - `file_path` - Denormalized path (can get via join)
...
- **Optional**: Supports queries like "find all vectors for file X" or "find vectors updated after timestamp Y" (if optional fields are included)
...
5. Store metadata in SQL:
   INSERT INTO vector_metadata VALUES (42, 123, '/path/to/file.vala', 'method', 'execute_query', 85, 105, ...)
   -- vector_id=42, file_id=123 (from OLLMfiles.File.id), file_path, element_type, element_name, start_line, end_line, ...
```
...
```sql
-- Find all vectors for a file that needs re-indexing (by file_id - preferred)
SELECT vector_id FROM vector_metadata WHERE file_id = 123;

-- Or by path using join (when you have path but need to find file_id first)
...
-- Optional: Find all classes indexed (for filtering)
SELECT * FROM vector_metadata WHERE element_type = 'class';

-- Optional: Find vectors for files matching a path pattern
SELECT vm.*, f.path, f.mtime FROM vector_metadata vm
JOIN files f ON vm.file_id = f.id
WHERE f.path LIKE '%database%';

-- Optional: Find recently indexed elements (for incremental updates)
SELECT * FROM vector_metadata WHERE timestamp > 1234567890;
```

### 4. **Change Detection**
Link with `OLLMfiles.File.mtime`:
```sql
-- Find vectors that need re-indexing (file changed since vectorization)
SELECT vm.vector_id FROM vector_metadata vm
JOIN files f ON vm.file_id = f.id
WHERE f.mtime > vm.timestamp;
```
...
### Minimum Schema (for LLM tool use case)

For the primary use case (LLM tool search), the minimum required fields are:
...
Additional fields that may be useful but aren't strictly required:

```sql
CREATE TABLE vector_metadata (
    vector_id INTEGER PRIMARY KEY,
    file_id INTEGER NOT NULL,
    file_path TEXT,                      -- Denormalized: for convenience (can get from files table)
    start_line INTEGER NOT NULL,
    end_line INTEGER NOT NULL,
    element_type TEXT NOT NULL,
    element_name TEXT NOT NULL,
    signature TEXT,                      -- Optional: full signature for better context
    description TEXT,                   -- Optional: LLM description (already in vectorized text)
    timestamp INTEGER,                   -- Optional: for incremental updates/change detection
    FOREIGN KEY (file_id) REFERENCES files(id)
);

CREATE INDEX idx_vector_metadata_file_id ON vector_metadata(file_id);
CREATE INDEX idx_vector_metadata_vector_id ON vector_metadata(vector_id);
CREATE INDEX idx_vector_metadata_element_type ON vector_metadata(element_type);
CREATE INDEX idx_vector_metadata_timestamp ON vector_metadata(timestamp);
```

**When to add optional fields:**
- `file_path`: If you need path-based queries without joins (rare for LLM tool)
- `signature`: If you want to display signatures without reading file
- `description`: If you want quick preview without reading file
- `timestamp`: If you need incremental updates/change detection

**Note on `file_id` vs `file_path`:**
- `file_id` is the **primary reference** - foreign key to `OLLMfiles.File.id` for proper relational integrity
- `file_path` is **optional/denormalized** - can be obtained via join: `SELECT f.path FROM files f WHERE f.id = ?`
- **For LLM tool**: `file_path` not needed - join to `files` table to get path when needed
...
## Minimum vs Extended Schema

### For LLM Tool (Minimum Required)
**6 fields:**
...
The LLM tool needs to return a **string** that the LLM can use to cite code. Based on the citation format requirements:

**Required for citation:**
- File path (for `filepath` in citation)
- Start line, end line (for `startLine:endLine` in citation)
- Code snippet (the actual code content)

**Helpful context (but not strictly required):**
- Element name (e.g., "DatabaseManager.execute_query") - helps LLM understand what it found
- Element type (e.g., "method", "class") - helps LLM provide better context

**Example LLM tool return format:**
```
Found 3 results for "database connection":

1. DatabaseManager.execute_query (method)
   File: /path/to/database.vala, Lines: 85-105
...
2. Connection.acquire (method)
   File: /path/to/connection.vala, Lines: 42-58
...
**Note:** The element name and type help the LLM provide better context in its response, but the **core requirement** is the code snippet with file path and line numbers for citation.
