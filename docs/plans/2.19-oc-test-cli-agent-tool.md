...
🚧 **IN PROGRESS** - Phase 7 complete (Streaming output via Manager signals for both agent tool and standard agent modes)
...
- ✅ **Temporary variables**: Plan avoids one-use temporaries and aliased variables (all inlined: `available` inlined in error message, `tool` inlined in execute call, `agent` inlined as `session.agent.chat()`, `tool_call` inlined in execute call, `result` kept as it's used in printf - acceptable for readability; `opt_project` and `opt_create_project` used directly instead of aliased to `project_path` and `allow_create`)
...
6. **Tool Registration**:
   - Create a shared `ToolRegistry` class in `ollmapp/ToolRegistry.vala` for tool registration
   - ToolRegistry has two entry points:
   ...
   - Both Window and ollmchat-cli can use the same ToolRegistry class
...
**Location**: `ollmapp/ollmchat-cli.vala`
...
**File**: `ollmapp/ollmchat-cli.vala`
...
**Changes**:
1. Add Manager, ProjectManager, and ToolRegistry properties to `OllmchatCliApp`:
   ```vala
   ...
   private OLLMapp.ToolRegistry tools_registry { get; set; }
   ```
...
**Design Note**: `setup_project_manager()` uses static option variables (`opt_project` and `opt_create_project`) directly instead of taking them as parameters. This avoids creating aliased variables and simplifies the method signature since these values are already available as class-level static variables.
...
**File**: `ollmapp/ToolRegistry.vala` (NEW)

**Status**: ✅ **COMPLETE** (but will be replaced in Phase 7.5 with library-level registries)

**Purpose**: Shared tool registration class used by both Window and ollmchat-cli to avoid code duplication

**Note**: This implementation will be replaced in Phase 7.5 with library-level registries (`OLLMtools.Registry` and `OLLMvector.Registry`). The current `OLLMapp.ToolRegistry` is a temporary solution that will be removed.

**Changes**:
1. Create new ToolRegistry class with two entry points:
   ```vala
   namespace OLLMapp
   {
       /**
...
2. Add to `ollmapp/meson.build`:
   ```meson
   'ToolRegistry.vala',
   ```
   (Add to `sources` array, e.g., after `'Application.vala'`)

3. Add to `docs/meson.build` for valadoc:
   ```meson
   '../ollmapp/ToolRegistry.vala',
   ```
   (Add to `valadoc_docs` input files list, e.g., after `'../ollmapp/Application.vala'`)

**Note**: ToolRegistry is in the `OLLMapp` namespace (not `OLLMtools`) since it's application-level code shared between Window and ollmchat-cli.
...
**Changes**:
1. Add ToolRegistry property:
   ```vala
   private OLLMapp.ToolRegistry tools_registry { get; set; }
   ```

2. Call config initialization in constructor (before loading config):
   ```vala
   public OllmchatCliApp()
   {
       ...
       this.tools_registry = new OLLMapp.ToolRegistry();
...
**Note**: This will be implemented in Phase 5 when `run_agent_tool()` is created.
...
**Changes**:
1. Updated `run_test()` to use Manager's session instead of creating Chat directly:
   - Call `setup_manager()` to create Manager (which already has session with just-ask agent)
   - Register tools using `tools_registry.fill_tools()` so agent has access to them
   - Ensure session is activated with just-ask agent (already done in Manager constructor, but explicitly set)
   - Set `num_ctx` on session's model_usage options if provided via command line
   - Use `manager.send(manager.session, message)` to send query (Manager signals handle streaming output)
   - Extract last assistant message from `session.messages` for final response
...
1. Add method to execute agent tool:
   ...
   private async void run_agent_tool(string tool_name, string query, ApplicationCommandLine command_line) throws Error
   {
       ...
       if (opt_project != "") {
           ...
           yield this.setup_project_manager(project_db);
       }
...
**Implementation**:
- ✅ Connect to `manager.stream_chunk` signal (filters out thinking content with `!is_thinking` check) - Done in `setup_manager()` (lines 320-325)
- ✅ Connect to `manager.tool_message` signal for tool status messages - Done in `setup_manager()` (lines 328-331)
...
**Note**: Since Manager relays from the active session, we only need to connect to Manager signals once (e.g., in `setup_manager()` or constructor), and they will work for both paths. The signals are connected in `setup_manager()`, which is called by both `run_test()` and `run_agent_tool()` before execution, ensuring streaming works for both modes.

### Phase 7.5: Refactor Tool Registration to Library-Level Registries

**Purpose**: Move tool registration from application-level (`OLLMapp.ToolRegistry`) to library-level registries. Each library (`liboctools`, `libocvector`) registers its own tools, allowing better separation of concerns and making it easier to add new tool libraries.

**Files to Create/Modify**:

1. **Create `libollmchat/Tool/RegistryInterface.vala`** (NEW):
   ```vala
   namespace OLLMchat.Tool
   {
        ...
        * 
        * Allows libraries to register their own tools while maintaining
        * a consistent interface for config initialization and tool registration.
        */
       public interface RegistryInterface : Object
       {
           /**
            * Phase 1: Initializes tool config types with Config2.
            * 
            ...
            * Does not require project_manager.
            */
           public abstract void init_config();
            ...
            * @param project_manager Optional project manager (null if not available)
            */
           public abstract void fill_tools(
               OLLMchat.History.Manager manager,
               OLLMfiles.ProjectManager? project_manager = null
           );
       }
...
2. **Create `liboctools/Registry.vala`** (NEW):
   ```vala
   namespace OLLMtools
   {
       /**
        ...
        */
       public class Registry : Object, OLLMchat.Tool.RegistryInterface
       {
           private Gee.ArrayList<OLLMchat.Tool.RegistryInterface> other_tools { get; set; default = new Gee.ArrayList<OLLMchat.Tool.RegistryInterface>(); }
           
           /**
            * Add another registry to delegate to (e.g., OLLMvector.Registry).
            */
           public void add(OLLMchat.Tool.RegistryInterface registry)
           {
               this.other_tools.add(registry);
           }
           ...
           public void init_config()
           {
               ...
               // Delegate to other registries
               foreach (var registry in this.other_tools) {
           ...
           )
           {
               ...
               // Delegate to other registries
               foreach (var registry in this.other_tools) {
                   registry.fill_tools(manager, project_manager);
               }
           }
       }
   }
...
3. **Create `libocvector/Registry.vala`** (NEW):
   ```vala
...
4. **Update `ollmapp/Application.vala`** constructor:
   ```vala
   // In Application constructor (around line 61-74)
   // Replace manual tool type registration with library registries
   var tools_registry = new OLLMtools.Registry();
   tools_registry.add(new OLLMvector.Registry());
...
5. **Update `ollmapp/Window.vala.register_tools()`**:
   ```vala
   // In Window.register_tools() (around line 954-997)
   // Replace manual tool registration with library registries
   var tools_registry = new OLLMtools.Registry();
   tools_registry.add(new OLLMvector.Registry());
...
6. **Update `ollmapp/ollmchat-cli.vala`**:
   - In constructor: Replace `new OLLMapp.ToolRegistry()` with `new OLLMtools.Registry()` and add `tools_registry.add(new OLLMvector.Registry())`
   - In `run_test()` and `run_agent_tool()`: No changes needed (already uses `tools_registry.fill_tools()`)

7. **Remove `ollmapp/ToolRegistry.vala`**:
   - Delete the file
   - Remove from `ollmapp/meson.build`
   - Remove from `docs/meson.build` (valadoc)

8. **Update build files**:
   - Add `Registry.vala` to `liboctools/meson.build`
   - Add `Registry.vala` to `libocvector/meson.build`
   - Add `RegistryInterface.vala` to `libollmchat/meson.build` (if Tool/ directory has meson.build)

**Benefits**:
- Each library registers its own tools (better separation of concerns)
- Easy to add new tool libraries (just create a Registry class implementing RegistryInterface)
- Application code is simpler (just creates registries and chains them)
- Removes application-level tool knowledge (tools are registered by their libraries)
- Maintains consistent interface across all registries
...
- `OLLMapp.ToolRegistry` - Shared tool registration class (NEW)
...
4. ✅ **Phase 2a**: Create ProjectManager infrastructure (if --project provided) - *Note: Combined with Phase 2*
5. ✅ **Phase 3**: Create shared ToolRegistry class in ollmapp (with two entry points)
6. ✅ **Phase 3a**: Use ToolRegistry.init_config() in ollmchat-cli constructor
7. ✅ **Phase 3b**: Use ToolRegistry.fill_tools() in ollmchat-cli - *Note: Completed in Phase 4*
8. ✅ **Phase 4**: Use Manager's just-ask agent (replaced direct Chat creation with Manager-based flow)
9. ✅ **Phase 5**: Look up and execute agent tool
10. ✅ **Phase 6**: Update command-line processing (support both agent tool and standard agent modes, fixed query detection to check query.length, fixed interactive mode with blocking IOChannel)
11. ✅ **Phase 7**: Handle streaming output (Manager signals connected in setup_manager(), works for both agent tool and standard agent modes)
12. ⏳ **Phase 7.5**: Refactor Tool Registration to Library-Level Registries (create RegistryInterface, OLLMtools.Registry, OLLMvector.Registry, remove OLLMapp.ToolRegistry)
...
1. **Refactor Tool Registration to Library-Level Registries**:
   - Create `RegistryInterface` in `libollmchat/Tool/`
   - Create `OLLMtools.Registry` and `OLLMvector.Registry` classes
   - Update Application and Window to use library registries instead of `OLLMapp.ToolRegistry`
   - Remove `OLLMapp.ToolRegistry` (temporary solution)
   - This allows each library to register its own tools and makes it easier to add new tool libraries
   - *Note: This is Phase 7.5 in the implementation plan*
