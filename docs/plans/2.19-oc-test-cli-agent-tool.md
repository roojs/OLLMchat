...
🚧 **IN PROGRESS** - Phase 3 complete (ToolRegistry created and integrated in ollmchat-cli)
...
### Phase 7.5: Update Window to Use ToolRegistry

**File**: `ollmapp/Window.vala` and `ollmapp/Application.vala`

**Changes**:
1. Update `Application.vala` constructor to use ToolRegistry for config initialization:
   ```vala
   // In Application constructor (around line 61-74)
   // Replace manual tool type registration with ToolRegistry.init_config()
   var tools_registry = new OLLMapp.ToolRegistry();
...
2. Update `Window.vala.register_tools()` to use ToolRegistry for tool registration:
   ```vala
   // In Window.register_tools() (around line 954-997)
   // Replace manual tool registration with ToolRegistry.fill_tools()
   var tools_registry = new OLLMapp.ToolRegistry();
...
3. Remove the manual tool registration code:
   ...
   - Remove ToolBuilder and Parser calls (ToolRegistry.fill_tools() handles these)
...
- Makes it easier to add new tools (only need to update ToolRegistry)
...
7. ⏳ **Phase 3b**: Use ToolRegistry.fill_tools() in ollmchat-cli (will be done in Phase 5)
...
12. ⏳ **Phase 7.5**: Update Window to use ToolRegistry
...
1. **Refactor Application to Use ToolRegistry for Config Initialization**:
   - Replace manual `typeof().ensure()` calls and `BaseTool.register_config()` with `new OLLMapp.ToolRegistry().init_config()`
   - This centralizes config initialization logic
   - *Note: This is Phase 7.5 in the implementation plan*

2. **Refactor Window to Use ToolRegistry for Tool Registration**:
   - Update `Window.register_tools()` to use `new OLLMapp.ToolRegistry().fill_tools()`
