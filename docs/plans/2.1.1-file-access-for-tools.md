...
- [ ] **Phase 8: Testing and Validation**
  - [ ] Refactor `oc-test-files` to extend `TestAppBase`
    - [ ] Change class to extend `TestAppBase` instead of standalone `Application`
    - [ ] Implement `run_test()` method (abstract from `TestAppBase`)
    - [ ] Add command-line options using `get_options()` override
    - [ ] `--help` is automatically provided by `TestAppBase` via `OptionContext.set_help_enabled(true)`
    - [ ] Add `--test-db` option: Specify test database path (optional, for testing only)
      - [ ] Usage: `--test-db=PATH` (optional - only use when specified)
      - [ ] **Default behavior**: Uses main/real database (same as normal ollmchat operation)
      - [ ] **When `--test-db` is specified**: All database operations use the test database instead of main database
      - [ ] **Critical**: Only use test database when `--test-db` is explicitly provided
  - [ ] Add `--ls` option (current scan/print functionality)
    - [ ] Move existing `run_scan()` logic to `--ls` command
    - [ ] Output format: `%y %Ts %p is_ignored is_repo\n` (same as current)
    - [ ] Scan current directory or specified path
  - [ ] Add `--read` option: Read file with line ranges (action only, no verification)
    - [ ] Usage: `--read=PATH [--start-line=N] [--end-line=M] [--output=FILE] [--backend=sourceview|dummy]`
    - [ ] `--backend` option: Choose buffer backend (`sourceview` = GtkSourceFileBuffer, `dummy` = DummyFileBuffer, default: `dummy`)
    - [ ] Perform `file.buffer.read_async()` for entire file (if no line range)
    - [ ] Perform `file.buffer.get_text(start_line, end_line)` for line ranges
    - [ ] Convert 1-based to 0-based line numbers internally
    - [ ] Support files in project and fake files
    - [ ] Output: Write read content to stdout or specified file
    - [ ] Output metadata: File path, line range (if specified), line count, buffer type, backend used
  - [ ] Add `--write` option: Write file with backups (action only, no verification)
    - [ ] Usage: `--write=PATH [--content=TEXT] [--content-file=FILE] [--backup-dir=DIR] [--backend=sourceview|dummy]`
    - [ ] Content source priority: `--content-file` > `--content` > stdin (if none provided, read from stdin)
    - [ ] `--backend` option: Choose buffer backend (`sourceview` = GtkSourceFileBuffer, `dummy` = DummyFileBuffer, default: `dummy`)
    - [ ] Note: `sourceview` backend requires GTK dependencies but should work without `gtk.init()` (use GtkSourceFileBuffer directly)
    - [ ] Perform `file.buffer.write()` (creates backup automatically for database files)
    - [ ] Output: Backup path (if created), file path written, file ID, backup status, backend used
    - [ ] Output format: Machine-readable (e.g., `BACKUP: /path/to/backup` or `NO_BACKUP: fake_file`, `BACKEND: dummy`)
  - [ ] Add `--create-fake` option: Create fake file (action only, no verification)
    - [ ] Usage: `--create-fake=PATH`
    - [ ] Perform `File.new_fake(manager, path)` constructor
    - [ ] Output: File path, file ID (should be -1), buffer status, fake file properties
  - [ ] Add `--check-project` option: Check if file is in active project (action only, no verification)
    - [ ] Usage: `--check-project=PATH`
    - [ ] Perform `project_manager.active_project.project_files.child_map.has_key(path)` check
    - [ ] Output: File path, project membership status (IN_PROJECT or NOT_IN_PROJECT), project path
  - [ ] Add `--cleanup-backups` option: Cleanup old backups (action only, no verification)
    - [ ] Usage: `--cleanup-backups [--backup-dir=DIR] [--age-days=N]`
    - [ ] Call `ProjectManager.cleanup_old_backups()` or static cleanup method
    - [ ] Output: List of backup files removed (one per line), backup directory path, age threshold
  - [ ] Add `--list-buffers` option: List current buffers (action only, no verification)
    - [ ] Usage: `--list-buffers [--max-buffers=N]`
    - [ ] Output: List of files with buffers (path, last_viewed timestamp, is_open status)
    - [ ] Output format: Machine-readable (e.g., `BUFFER: /path/to/file TIMESTAMP: 1234567890 OPEN: true`)
  - [ ] Add `--create-project` option: Create test project (action only, no verification)
    - [ ] Usage: `--create-project=PATH [--test-db=DB]`
    - [ ] If `--test-db` is specified: Use test database for ProjectManager
    - [ ] If `--test-db` is NOT specified: Use main database (normal operation)
    - [ ] Create Folder with is_project=true
    - [ ] Scan directory and populate project files
    - [ ] Output: Project path, database path, file count, project ID
  - [ ] Add `--info` option: Show file information (action only, no verification)
    - [ ] Usage: `--info=PATH`
    - [ ] Output: File path, file ID, is_fake status, buffer status, in_project status, backup path (if any)

**Test Scripts in `/tests` Folder:**

- [ ] Create `/tests` directory structure
  - [ ] Create `tests/test-file-ops.sh` shell script
  - [ ] Script will run series of `oc-test-files` commands
  - [ ] Use `diff` (or `oc-diff`) and other Unix commands to verify operations
  - [ ] **Test file location**: Use `~/.cache/ollmchat/testing/*` for all test files, directories, and databases
  - [ ] Each test scenario should:
    - Set up test environment in `~/.cache/ollmchat/testing/` (temp directories, test files)
    - Run `oc-test-files` commands to perform actions
    - Capture output from `oc-test-files`
    - Use verification commands (`diff`, `test`, `grep`, etc.) to check results
    - **Cleanup behavior**:
      - If test succeeds: Delete all files in `~/.cache/ollmchat/testing/*` (clean up completely)
      - If test fails: Leave files in `~/.cache/ollmchat/testing/*` for debugging (don't clean up)
    - Report pass/fail status

**Example Test Scenarios for `test-file-ops.sh`:**

- [ ] **Test 1: File reading with line ranges**
  - Create test file with known content in `~/.cache/ollmchat/testing/`
  - Run `oc-test-files --read=~/.cache/ollmchat/testing/test.txt --start-line=2 --end-line=5 --output=~/.cache/ollmchat/testing/read_output.txt`
  - Use `diff` to compare `read_output.txt` with expected content
  - Verify line count matches expected
  - Clean up on success, leave files on failure

- [ ] **Test 2: File writing with backups**
  - Create test file in project (database file with `id > 0`) in `~/.cache/ollmchat/testing/`
  - Run `oc-test-files --write=~/.cache/ollmchat/testing/test.txt --content="new content" --test-db=~/.cache/ollmchat/testing/test.db`
  - Parse output for backup path (e.g., `BACKUP: /path/to/backup`)
  - Use `test -f` to verify backup file exists
  - Use `diff` to compare backup with original content
  - Verify backup path format matches `{id}-{date YY-MM-DD}-{basename}`
  - Clean up on success, leave files on failure

- [ ] **Test 3: Fake file creation and access**
  - Run `oc-test-files --create-fake=~/.cache/ollmchat/testing/fake.txt --test-db=~/.cache/ollmchat/testing/test.db`
  - Parse output to verify `file ID: -1`
  - Run `oc-test-files --read=~/.cache/ollmchat/testing/fake.txt --test-db=~/.cache/ollmchat/testing/test.db` and verify it works
  - Run `oc-test-files --write=~/.cache/ollmchat/testing/fake.txt --content="test" --test-db=~/.cache/ollmchat/testing/test.db` and verify no backup created
  - Clean up on success, leave files on failure

- [ ] **Test 4: Project context detection**
  - Create test project with `oc-test-files --create-project=~/.cache/ollmchat/testing/testproj --test-db=~/.cache/ollmchat/testing/test.db`
  - Run `oc-test-files --check-project=~/.cache/ollmchat/testing/testproj/file.txt --test-db=~/.cache/ollmchat/testing/test.db`
  - Parse output to verify `IN_PROJECT` status
  - Run `oc-test-files --check-project=~/.cache/ollmchat/testing/outside.txt --test-db=~/.cache/ollmchat/testing/test.db`
  - Parse output to verify `NOT_IN_PROJECT` status
  - Clean up on success, leave files on failure

- [ ] **Test 5: Backup cleanup**
  - Create test backup files with various ages in `~/.cache/ollmchat/testing/backups/` (using `touch -t` to set dates)
  - Run `oc-test-files --cleanup-backups --backup-dir=~/.cache/ollmchat/testing/backups --age-days=3`
  - Parse output for removed backups
  - Use `test -f` to verify old backups removed
  - Use `test -f` to verify recent backups kept
  - Clean up on success, leave files on failure

- [ ] **Test 6: Buffer cleanup/management**
  - Create multiple files in `~/.cache/ollmchat/testing/` and access them to create buffers
  - Run `oc-test-files --list-buffers --test-db=~/.cache/ollmchat/testing/test.db` to get initial buffer list
  - Create more buffers (exceeding max limit)
  - Run `oc-test-files --list-buffers --test-db=~/.cache/ollmchat/testing/test.db` again
  - Verify only top N most recent buffers are kept (using timestamps)
  - Clean up on success, leave files on failure

- [ ] **Test 7: Permissions skipped for project files**
  - Create test project with files in `~/.cache/ollmchat/testing/` using `--test-db=~/.cache/ollmchat/testing/test.db`
  - Use tool that requires permissions (mock or actual) with `--test-db=~/.cache/ollmchat/testing/test.db`
  - Verify permission check output shows skip for project files
  - Verify permission check output shows prompt for non-project files
  - Clean up on success, leave files on failure
