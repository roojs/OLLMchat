...
- [ ] **Phase 8: Testing and Validation**
  ...
  - [ ] Add `--test-read` option: Test file reading with line ranges
    - [ ] Usage: `--test-read=PATH [--start-line=N] [--end-line=M]`
    - [ ] Test `file.buffer.read_async()` for entire file
    - [ ] Test `file.buffer.get_text(start_line, end_line)` for line ranges
    - [ ] Verify 1-based to 0-based line number conversion
    - [ ] Test with files in project and fake files
    - [ ] Output: Show read content and line counts
  - [ ] Add `--test-write` option: Test file writing with backups
    - [ ] Usage: `--test-write=PATH [--content=TEXT] [--backup-dir=DIR]`
    - [ ] Test `file.buffer.write()` creates backup for database files (`id > 0`)
    - [ ] Verify backup path format: `{id}-{date YY-MM-DD}-{basename}`
    - [ ] Verify backup only created once per day (don't overwrite)
    - [ ] Verify `file.last_approved_copy_path` is updated
    - [ ] Test that fake files (`id < 0`) don't create backups
    - [ ] Output: Show backup path and verify backup file exists
  - [ ] Add `--test-fake` option: Test fake file creation and access
    - [ ] Usage: `--test-fake=PATH`
    - [ ] Test `File.new_fake(manager, path)` constructor
    - [ ] Verify `id = -1` for fake files
    - [ ] Test buffer creation for fake files
    - [ ] Test read/write operations on fake files
    - [ ] Verify `saveToDB()` is skipped for fake files
    - [ ] Output: Show fake file properties and operations
  - [ ] Add `--test-project-context` option: Test project context detection
    - [ ] Usage: `--test-project-context=PATH`
    - [ ] Create test project with files
    - [ ] Test `project_manager.active_project.project_files.child_map.has_key(path)`
    - [ ] Verify files in active project are detected correctly
    - [ ] Test with files outside project (should return false)
    - [ ] Output: Show project membership status for test paths
  - [ ] Add `--test-backup-cleanup` option: Test cleanup of old backups
    - [ ] Usage: `--test-backup-cleanup [--backup-dir=DIR] [--age-days=N]`
    - [ ] Create test backup files with various ages (older than 3 days, newer)
    - [ ] Call `ProjectManager.cleanup_old_backups()` or static cleanup method
    - [ ] Verify backups older than 3 days are removed
    - [ ] Verify backups newer than 3 days are kept
    - [ ] Output: List backups before/after cleanup
  - [ ] Add `--test-buffer-cleanup` option: Test buffer cleanup/management
    - [ ] Usage: `--test-buffer-cleanup [--max-buffers=N]`
    - [ ] Create multiple files and buffers (more than 10)
    - [ ] Set `last_viewed` timestamps on files
    - [ ] Trigger buffer cleanup by creating new buffer
    - [ ] Verify top 10 most recently used buffers are kept
    - [ ] Verify buffers for files with `is_open == true` are kept
    - [ ] Verify other buffers are set to `null`
    - [ ] Output: Show buffer count before/after and which buffers were kept/cleared
  - [ ] Add `--test-permissions` option: Verify permissions are skipped for active project files
    - [ ] Usage: `--test-permissions=PATH`
    - [ ] Create test project with files
    - [ ] Test permission check logic in tools (mock or actual tool)
    - [ ] Verify files in active project skip permission prompts
    - [ ] Verify files outside project require permissions
    - [ ] Output: Show permission check results for test paths
  - [ ] Add `--test-all` option: Run all tests in sequence
    - [ ] Usage: `--test-all [--test-dir=DIR]`
    - [ ] Creates temporary test directory structure
    - [ ] Runs all test options in logical order
    - [ ] Cleans up test files/directories after completion
...
protected const OptionEntry[] local_options = {
    ...
    { "test-read", 0, 0, OptionArg.STRING, ref opt_test_read, "Test file reading with line ranges", "PATH" },
    ...
    { "test-write", 0, 0, OptionArg.STRING, ref opt_test_write, "Test file writing with backups", "PATH" },
    ...
    { "test-fake", 0, 0, OptionArg.STRING, ref opt_test_fake, "Test fake file creation and access", "PATH" },
    { "test-project-context", 0, 0, OptionArg.STRING, ref opt_test_project_context, "Test project context detection", "PATH" },
    { "test-backup-cleanup", 0, 0, OptionArg.NONE, ref opt_test_backup_cleanup, "Test cleanup of old backups", null },
    { "age-days", 0, 0, OptionArg.INT, ref opt_age_days, "Age in days for backup cleanup (default: 3)", "N" },
    { "test-buffer-cleanup", 0, 0, OptionArg.NONE, ref opt_test_buffer_cleanup, "Test buffer cleanup/management", null },
    { "max-buffers", 0, 0, OptionArg.INT, ref opt_max_buffers, "Maximum buffers to keep (default: 10)", "N" },
    { "test-permissions", 0, 0, OptionArg.STRING, ref opt_test_permissions, "Test permission skipping for project files", "PATH" },
    { "test-all", 0, 0, OptionArg.NONE, ref opt_test_all, "Run all tests in sequence", null },
    { "test-dir", 0, 0, OptionArg.STRING, ref opt_test_dir, "Test directory for --test-all (default: /tmp/oc-test-files)", "DIR" },
    { null }
};
```
...
- Use `validate_args()` to ensure at least one test option is specified (or `--ls`)
- Each test option should be mutually exclusive or combinable as appropriate
- Use temporary directories/files for tests that modify the filesystem
- Provide clear, structured output for each test showing what was tested and results
