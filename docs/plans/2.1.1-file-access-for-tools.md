...
- [ ] **Phase 1: FileBuffer Interface Refactor**
  - [ ] Create `FileBuffer` interface in `libocfiles`
  - [ ] Create `DummyFileBuffer` class in `libocfiles`
  - [ ] Create `GtkSourceFileBuffer` class in `liboccoder`
  - [ ] Update `File` class (add buffer property, remove read_async/write)
  - [ ] Simplify `BufferProviderBase` (keep only detect_language and create_buffer)
  - [ ] Simplify `BufferProvider` (GTK) (keep only detect_language and create_buffer)
  - [ ] Add buffer cleanup/management logic (age check, keep top 10)
  - [ ] Update `SourceView` to use file.buffer directly

- [ ] **Phase 2: Fake File Support**
  - [ ] Add `File.new_fake()` constructor
  - [ ] Update `FileBase` to handle fake files (skip DB operations when `id < 1`)
  - [ ] Ensure buffer provider works with fake files

- [ ] **Phase 3: Project Context in Tools**
  - [ ] Create `liboctools/` directory and `meson.build`
  - [ ] Move `libollmchat/Tools/*` to `liboctools/` (change namespace to `OLLMtools`)
  - [ ] Update `libollmchat/meson.build` (remove tools sources)
  - [ ] Update `ollmchat/Window.vala` (import from `OLLMtools`)
  - [ ] Add `ProjectManager` property to tool classes (ReadFile, EditMode, etc.)
  - [ ] Update permission system to skip prompts for files in active project

- [ ] **Phase 4: File Backup System**
  - [ ] Add backup directory creation (`~/.cache/ollmchat/edited/`)
  - [ ] Update `FileBuffer.write()` in `GtkSourceFileBuffer` (with backup logic)
  - [ ] Update `FileBuffer.write()` in `DummyFileBuffer` (with backup logic)
  - [ ] Add cleanup method for old backups (>3 days)

- [ ] **Phase 5: Migrate RequestReadFile**
  - [ ] Get/create File object from path
  - [ ] Ensure buffer exists (create if needed)
  - [ ] Use `file.buffer.read_async()` for entire file
  - [ ] Use `file.buffer.get_text()` for line ranges
...
1. **RequestReadFile** (`libollmchat/Tools/RequestReadFile.vala`)
...
2. **RequestEditMode** (`libollmchat/Tools/RequestEditMode.vala`)
...
public File.new_fake(ProjectManager manager, string path) {
    ...
    this.base_type = "f";
    this.path = path;
    this.id = 0; // Indicates not in database
}
```

**Behavior**:
- `id = 0` indicates fake file (not in database)
- `saveToDB()` should skip if `id < 1`
