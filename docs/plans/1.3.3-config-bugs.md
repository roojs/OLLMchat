...
✅ **MOSTLY COMPLETE** - Configuration migration working. Gee collection serialization fixed. Options serialization with underscore/hyphen handling fixed.
...
- ✅ `libollmchat/Settings/Config2.vala` - Properties and storage for model/title_model implemented in usage map. Gee collection serialization fixed.
- ⏳ All code that accesses `client.config.model`, `client.config.title_model`, `client.config.think` - Need to verify handling of empty strings/defaults
