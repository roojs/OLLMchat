 ...
 * 
 * Emitted by DeleteManager.cleanup() after cleanup is complete.
 * This allows tools to perform bulk cleanup operations efficiently.
 */
public signal void on_cleanup_deleted();
...
// - Emits on_cleanup_deleted signal after cleanup is complete (for bulk VectorMetadata cleanup)
```
...
- After cleanup is complete, `DeleteManager.cleanup()` emits `on_cleanup_deleted` signal
...
public async void cleanup_deleted()
{
    ...
    // Note: VectorMetadata cleanup happens via ProjectManager.on_cleanup_deleted signal
...
// The on_cleanup_deleted signal is for bulk VectorMetadata cleanup.
...
public CodebaseSearchTool(...)
{
    ...
    // Connect to ProjectManager.on_cleanup_deleted signal for bulk VectorMetadata cleanup
    this.project_manager.on_cleanup_deleted.connect(() => {
 ...
 * not folders.
 * 
 * Called automatically via ProjectManager.on_cleanup_deleted signal after
...
// - Emitting on_cleanup_deleted signal after cleanup is complete (for bulk VectorMetadata cleanup)
...
### Files to Modify
- `libocfiles/ProjectManager.vala` - Add `delete_manager` property, initialize in constructor, add `on_cleanup_deleted()` signal
...
- [ ] Add `ProjectManager.on_cleanup_deleted()` signal
...
- [ ] Emit `on_cleanup_deleted()` signal in DeleteManager.cleanup() after cleanup is complete
...
- [ ] Connect CodebaseSearchTool to `on_cleanup_deleted` signal for VectorMetadata cleanup
...
### Phase 6: Database Record Retention (No Removal)

- [ ] **Policy Decision**: Database records are NOT removed - they remain with `delete_id > 0`
...
- [ ] No `cleanup_deleted_from_db()` method needed - records are kept
...
- **Vector Metadata**: Vector metadata for deleted files is removed via `ProjectManager.on_cleanup_deleted` signal. The signal is emitted by `DeleteManager.cleanup()` after all cleanup is complete. CodebaseSearchTool (in libocvector) listens to this signal and calls `VectorMetadata.cleanup_all_deleted()` which performs a bulk DELETE query: `DELETE FROM vector_metadata WHERE file_id IN (SELECT id FROM filebase WHERE delete_id > 0 AND base_type = 'f')`. This avoids circular dependency since libocfiles cannot depend on libocvector.
