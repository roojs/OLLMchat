...
**Status:** PLAN — Outstanding items to address.
...
- **Solution (Tool class)**
  ...
  - **ResultParser:** "## Tool Calls" walk: for each fenced block, **new Tool(task)**, **tool.parse(block)**, task.tools.add(tool).
  - **Details:** Holds list or map of **Tool** instances. run_tools(): each Tool parses/validates then execute.
...
- **Lifecycle**
  - ResultParser: **new Tool(task)**, **tool.parse(block)**, task.tools.add(tool). run_tools(): for each tool, **tool.validate()** then **tool.execute()** (caller does parse/validate before execute).
- **Details integration**
  - Details holds list of Tool. ResultParser adds Tool(task, block) per ## Tool Calls block. run_tools() iterates Tools; for each: parse(), validate(), execute().
...
- **run_tools():** Caller has already had parse(block) called per tool (by ResultParser). Replace body: foreach tool in this.tools, **if (tool.validate()) yield tool.execute()**. (execute() assumes parse and validate already done.)

```vala
// Details.vala — add after code_blocks property
public Gee.ArrayList<Tool> tools { get; set; default = new Gee.ArrayList<Tool>(); }

// run_tools() — replace existing body
...
- After the existing "Refined task" logic, add:
  - If document has heading for Tool Calls (anchor **tool-calls** per section anchor standard): get section contents; foreach node, if Block and FENCED_CODE: **var tool = new Tool(task); tool.parse((Markdown.Document.Block) node); task.tools.add(tool)**.
...
### 4. Build / meson (deferred)

- **Do not add Tool.vala to meson yet.** Tool and the new flow depend on APIs and behaviour that are not in place. Add Tool.vala to liboccoder Task sources only after the items below are done.

### 5. Prerequisites — what needs to exist or change before Tool can be wired in

- **OLLMchat.Tool.BaseTool.example_call** — Property `string example_call { get; set; default = ""; }` not yet added. Tool.to_instructions() uses it (check != "").
- **Markdown.Document headings keyed by anchor** — Document currently keys headings by heading text (e.g. `"Refined task"`, `"Result summary"`). Plan assumes anchor keys (e.g. `"tool-calls"`). Either: (1) implement `heading_text_to_anchor(string)` and use it in `Document.register_heading()` so keys are anchors, or (2) keep using title key for Tool Calls (e.g. `has_key("Tool Calls")`) until anchor migration is done.
- **Details.tools** — Property `Gee.ArrayList<Tool> tools` and run_tools() iterating it and calling `yield tool.execute()` are not yet in place. Current run_tools() builds a list from `this.tool_calls` and calls `execute_tools()`; no Tool instances.
- **ResultParser.extract_refinement()** — No walk of the Tool Calls section yet; and that walk depends on document having that section under the chosen key (anchor or title).
- **Task.Tool class (Tool.vala)** — New code uses: `task.runner.session.manager.tools`, `BaseTool.function` (name, description, parameters), `BaseTool.example_call`, `task.execute_tools()`, `task.tool_outputs`, `task.tool_calls`. All exist except `example_call`; and Tool.vala is not in the build yet so ResultParser/Details cannot reference it until Tool is added to meson after the above are fixed.
...
- 1.23.13 (task-related code, DONE); 1.23.14 (existing skill code updates); 1.23.12 (prompt refinements — ## Tool Calls section, JSON format, "how to use" = name+description+parameters).
