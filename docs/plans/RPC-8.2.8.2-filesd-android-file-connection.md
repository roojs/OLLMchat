...
- **🔷** `⏳` Mint/load device client cert; present on TLS; call `RPC-ClientCert.request_registration` when unknown.
  - Include a **requester** on registration — best-effort device-extracted string (e.g. Android `Build.MODEL`) — (see [`8.2.8.1`](RPC-8.2.8.1-filesd-desktop-connections-ui.md)) so the desktop can label the client.
