...
When creating a new file (complete_file mode) within the active project, create a proper File object (id = 0) instead of a fake file (id = -1) so it can be saved to the database:

- [ ] In `apply_all_changes()`, before creating fake file, check if path is within active project:
  - Check if `active_project != null` and `normalized_path.has_prefix(active_project.path)`
  - If within project and file doesn't exist in tree yet:
    - Get parent directory path: `GLib.Path.get_dirname(normalized_path)`
    - Find parent folder in project tree (walk from `active_project` down the path)
    - If parent folder not found in tree, use `active_project` as parent (fallback)
    - Create proper File object: `new File(project_manager)` (not `new_fake()`)
    ...
    - Query file info from disk after write: `GLib.File.query_info()` to get FileInfo, then set `is_text`, `last_modified`, etc.
  ...
  - If file already exists in project tree:
    - Use existing file object (already has proper id)
  - If file is outside active project:
    - Use fake file (`File.new_fake()`) - id = -1, won't be saved to DB
- [ ] After writing via `file.buffer.write()`:
  - For project files (id >= 0): `buffer.write()` automatically saves to DB via `update_file_metadata_after_write()`
