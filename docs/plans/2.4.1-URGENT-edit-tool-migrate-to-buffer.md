...
**Status: Already handled by current implementation**

The current implementation already handles new files correctly:
- Uses `project_manager.get_file_from_active_project()` to check if file exists in project
- If file is not in project tree, creates fake file (`File.new_fake()`) - this is the same approach as the reference implementation
- `file.buffer.write()` automatically handles:
  - DB saving for project files (id > 0) via `update_file_metadata_after_write()`
  - Backup creation for project files
  - Emitting `file.changed()` signal
- New files created within project path but not yet in tree will use fake files (id = -1)
- Files will be discovered and added to project tree on next project scan (via `project.read_dir()`)
- No special handling needed - matches reference implementation behavior
