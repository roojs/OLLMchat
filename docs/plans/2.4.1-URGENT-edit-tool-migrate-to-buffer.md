...
- [ ] Add `project_manager` property to `libollmchat/Tools/EditMode` class
- [ ] Update `EditMode` constructor to accept optional `ProjectManager` parameter
- [ ] Ensure `project_manager` is passed from `Window.vala` when creating the tool
...
- [ ] For complete_file mode: Count from `changes[0].replacement` string
- [ ] For edit mode: Use `file.buffer.get_line_count()` after applying edits
- [ ] Remove all `DataInputStream` usage for line counting
- [ ] Remove `reconstruct_final_content()` if not needed (use buffer instead)
