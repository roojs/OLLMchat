...
- [ ] Add `file_added` signal to ProjectFiles:
  - Add signal declaration: `public signal void file_added(File file)`
  - Emit signal in `append()` method after adding file: `this.file_added(item.file)`
  - Emit signal in `insert()` method after inserting file: `this.file_added(item.file)`
  - This allows UI components to react when new files are added to the project
