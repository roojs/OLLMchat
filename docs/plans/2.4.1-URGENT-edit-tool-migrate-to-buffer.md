...
- [ ] Create new method `deal_with_new_file(File file)`:
  - Check if file path is within active project: `active_project != null && normalized_path.has_prefix(active_project.path)`
  - If within project and file is fake (id = -1):
  ...
  - If file already exists in project tree:
    - File already has proper id, no conversion needed
  - If file is outside active project:
    - Keep fake file (id = -1) - no conversion needed
- [ ] In `create_new_file_with_changes()`, after `yield file.buffer.write()`:
  - Call `yield this.deal_with_new_file(file)` to handle conversion if needed
