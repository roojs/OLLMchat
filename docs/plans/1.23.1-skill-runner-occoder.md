...
- **Method:** `Gee.HashMap<string, Skill> map_available()` — for each directory in the array, enumerate `.md` files, exclude `system.template.md` and `user.template.md`, build path, create `Skill(path)`, call `skill.load()` in try; on success add to a map with key = skill name (`skill.header.get("name")`), value = Skill (skip if name empty). Return the map so callers can look up by name. “Available skills” = skills that exist and load in the configured directories.
- **Method:** `int64 get_system_inputs_max_mtime(string skill_filename)` — returns the max modification time of: `system.template.md` in primary directory, the current skill file (primary + skill_filename), and all skill files in `map_available()`. Used by SkillRunner for cache invalidation. Returns 0 if primary is empty.
...
{
	/**
	 * Holds an array of skills directories and lists available skills by scanning them.
	 */
	public class SkillManager : Object
	{
		...
		/**
		 * Scan all configured directories and return skills that load successfully, keyed by skill name.
		 */
		public Gee.HashMap<string, Skill> map_available()
		{
			var map = new Gee.HashMap<string, Skill>();
			foreach (var skills_base_path in this.skills_directories) {
				...
				try {
					...
					GLib.warning("SkillManager.map_available: %s: %s", skills_base_path, e.message);
				}
			}
			return map;
		}
		...
		/** Max mtime of system.template, current skill file, and all skills in map_available(). For cache invalidation. */
		public int64 get_system_inputs_max_mtime(string skill_filename)
		{
			...
			m = this.get_file_mtime(GLib.Path.build_filename(primary, skill_filename));
			if (m > max) {
				max = m;
			}
			var available = this.map_available();
			foreach (var skill in available.values) {
				m = this.get_file_mtime(skill.path);
				if (m > max) {
					max = m;
				}
			}
