# 1.14. URGENT - Add Current Date and Time to System Prompt Injection

## Overview

Add the current date and time to the system prompt injection in "Just Ask" and "CodeAssistant" agents. The system prompt injection already exists via the `generate_user_info_section()` method, which includes OS Version, Workspace Path, and Shell. This change will add the current date and time to that section so the LLM has temporal context.
