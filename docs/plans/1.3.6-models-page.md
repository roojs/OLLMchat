...
### ⏳ Missing / TODO
- **Temp Model Creation/Deletion**: Create and delete models using ollmchat-temp
  - When user tries to run a chat with a model that has a custom `num_ctx`, create temp model variant if needed
  - Handle temp model lifecycle (create, use, cleanup)
  - Delete temp models that are no longer in use
...
✅ **MOSTLY COMPLETE** - Main components implemented, some features may need refinement
