# 8.2.8.9 URGENT — Android Agent Pi when the desktop environment is live

**Status:** **URGENT** · Phase 1 **✔️** · Phases 2–8 **⏳**
...
- [`BWRAP-2.10.4.15`](BWRAP-2.10.4.15-DEFERRED-execution-rpc-sandbox.md) — daemon `Bubble.exec` (Android `bash`). Not on the wire yet.

**Layout:** `docs/guide-to-writing-plans.md` — **Checklist for plans**

Proposed Vala follows `docs/coding-standards.md`. Phase 1 is applied. Later hunks wait until those phases are approved.
...
- **ℹ️** `bash` does not exec on the daemon. `RunCommand` calls in-process `OLLMbwrap.Bubble.exec` (or `GLib.Subprocess` when bwrap is missing). Overlay apply is File.* RPC ([`done/2.10.4.19-DONE-runcommand-overlay-index.md`](done/2.10.4.19-DONE-runcommand-overlay-index.md)). Daemon `Bubble.exec` is [`BWRAP-2.10.4.15-DEFERRED-execution-rpc-sandbox.md`](BWRAP-2.10.4.15-DEFERRED-execution-rpc-sandbox.md).
- **🔷** Android Agent Pi needs `bash` on the desktop. That is `BWRAP-2.10.4.15`, not this plan’s hunks. Registering `Bash` on the phone would run on the phone.
...
`bash` and `AgentPi.Factory.register_config` wait on [`BWRAP-2.10.4.15`](BWRAP-2.10.4.15-DEFERRED-execution-rpc-sandbox.md). Do not register `Bash` here.
...
- **🔷** Hello succeeds → state `LIVE` (remote) or `SOCKET` (local, empty `url`), then Phase 2's Agent Pi visibility. Remote `LIVE` still switches the session.
...
- **🚫** Registering in-process `Bash` on Android. Exec stays on the daemon (`BWRAP-2.10.4.15`).
