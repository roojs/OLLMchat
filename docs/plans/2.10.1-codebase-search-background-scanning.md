...
private void set_active_project(Folder? project) {
    ...
    if (this.active_project != null && this.active_project != project) {
        ...
        this.active_project.children.remove_all();
        // Clear flat file list
        this.active_project.project_files.remove_all();
...
- [libocvector/BackgroundScan.vala](libocvector/BackgroundScan.vala)
  ...
  - Update `startQueue()` to be async, process BackgroundScanItem, find project by path, call `set_active_project()`, reload project files (will use Folder's optimization), find file in project
