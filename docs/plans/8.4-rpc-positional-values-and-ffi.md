...
- **🔷** Phase 2 (this parent): typelib path in [`8.4.2`](8.4.2-rpc-ffi-typelib-invoke.md) — register types, RPC `new` (no handle), then invoke any GI method on the lease. Construction and calls use **libffi** / `GI.FunctionInfo.invoke`. Vala `out` / `ref` / `[CCode]` are compile-time; they join this path only when a GIR/typelib exists.
...
- **🔷** `⏳` [`8.4.2`](8.4.2-rpc-ffi-typelib-invoke.md) — register types from the typelib; RPC `Alias.new` (no handle) leases the instance; then any typelib method on that handle. No property dump on handled objects. Allowlist stays in the caller (plugin). ollmfilesd `call_*` handlers unchanged.
