...
📋 **PLANNED** - Part of 2.1.4 AST Path Support for EditFile Integration. Not urgent or planned in the near term.

> **Note — Not urgent:** We already support editing via AST path syntax (e.g. `type:Namespace-Class-Method` in code blocks). This plan only adds *incremental* behavior so that *multiple* AST path edits in the same message resolve correctly (tree reparse marking, etc.). Single AST path edits work today.
...
- [ ] **AST Path Mode Behavior (`use_line_numbers = false` and `edit_mode == "ast_path"`):**
  - [ ] AST paths are required in code block format: `type:Namespace-Class-Method`
...
- [ ] **Delayed Disk Write:**
  ...
  - [ ] At end of session (`on_message_completed()` → `apply_all_changes()`), check `incremental_changes_applied` flag in addition to `changes.size`
