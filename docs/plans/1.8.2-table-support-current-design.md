...
- **on_block(table cell start)** – cell start.
...
- **on_block(table cell end)** – cell end.
...
- **Line 2** (separator): consume, parse alignment; don't emit table start yet.
- **Line 3** (first body row): NOW emit on_table(start), then emit header row (tr + cells), then alignment, then this row (tr + cells). **Subsequent rows (4th, 5th, …)**: same path – each is one line; we call handle_table_line(line) once per line (tr start → cells with format.eat() → tr end).
...
- **on_table_hcell(bool start)** – header cell start or end (first row only).
- **on_table_cell(bool start)** – body cell start or end.

Cell content is sent via the same path as paragraph text (format handlers: on_text, on_strong, on_em, etc.) between cell start and cell end.
...
|-------|-----------|---------|
| **1.10.1** | [1.10.1-renderer-interface.md](1.10.1-renderer-interface.md) | Add table methods to RenderBase (on_table, on_table_row, on_table_hcell, on_table_cell). |
| **1.10.2** | [1.10.2-html-dummy.md](1.10.2-html-dummy.md) | Add simple support in HtmlRender and DummyRenderer (HTML tags / log). |
