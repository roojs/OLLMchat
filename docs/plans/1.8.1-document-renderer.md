...
**JSON:** Serialize `children` as a single array. Because children are polymorphic (Block | List | ListItem | Format), use a **type discriminator** per element (e.g. `"node_type": "block"|"list"|"list_item"|"format"`) or a wrapper `{ "type": "...", "value": { ... } }` so deserialization can call the correct `Json.gobject_deserialize(typeof(Block), ...)` etc. Round-trip must reproduce the tree.
...
**Parser.vala change for 1.8.1:**

- **Add static from_string** — `public static FormatType from_string(string s)` with a switch over every enum name; default return `FormatType.NONE` or `FormatType.PARAGRAPH` for unknown. (FormatType enum values are already present from 1.8.2.)

---

## File and class layout

- **Parser.vala:** **FormatType** already includes TABLE_ROW, TABLE_HCELL, TABLE_CELL, LIST_ITEM, TEXT, IMAGE, BR, U, OTHER, CODE_TEXT, SOFTBR, ENTITY (from 1.8.2). Add static **FormatType.from_string(string s)** in Parser.vala for JSON deserialization (switch on all enum names; default NONE or PARAGRAPH for unknown).
- **Subfolder:** `libocmarkdown/document/` — one file per class, all in namespace **Markdown.Document**:
  - **Node.vala** — Abstract base: `parent`, `children`, `node_type`, `adopt()`.
  - **Format.vala** — Inline/format node (FormatType, text, href, src, tag, etc.; `from_text` secondary ctor).
  - **Block.vala** — Block node (FormatType kind, level, lang, align, task_checked, code_text); polymorphic children deserialization.
...
Implement as **one file per class** in `libocmarkdown/document/`. The skeleton below is split by file (see comment headers). Coding standards: `this.` prefix, brace style (line breaks for namespace/class/method, inline for control structures), `Gee.ArrayList` via `.get()`/`.add()`/`.remove_at()`, property defaults. In **switch** statements, return directly from each case (no `break`). All document types implement **Json.Serializable** with enums as strings, **node_type** and **parent** (exclude parent from JSON); round-trip reproduces the tree.
...
public abstract class Node : Object, Json.Serializable
{
	...
	/** Polymorphic child deserialization: dispatch by node_type / node-type. Shared by all subclasses. */
	protected virtual Node? deserialize_child(Json.Node elem)
	{
		...
		var type_str = type_node != null ? type_node.get_string() : "";
		// Normalize kebab-case to snake_case for type string
...
**Enum deserialization:** Implement **FormatType.from_string(string s)** in Parser.vala and use it in Block/Format deserialize_property for the `kind` property. Serialize with `this.kind.to_string()` as in the skeleton.
...
- [ ] **Parser.vala:** Add static `FormatType.from_string(string s)` (switch on all enum names; default NONE or PARAGRAPH for unknown). FormatType enum values are already present from 1.8.2.
...
- [ ] Implement **Json.Serializable** on Document, Block, Format, List, ListItem: serialize `kind` as string via `this.kind.to_string()`; deserialize via `FormatType.from_string(property_node.get_string())` and `Json.gobject_deserialize` for nested collections.
...
| Item | Description |
|------|-------------|
| **Parser** | Add `FormatType.from_string(string s)` for JSON deserialization. FormatType enum already has required values (1.8.2). |
