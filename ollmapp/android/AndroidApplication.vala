...
namespace OLLMapp
{
	...
	public class AndroidApplication : Adw.Application, OLLMchat.ApplicationInterface
	{
		...
		{
			/* XDG_DATA_HOME is externalFilesDir/share (GTK asset tree,
			 * re-extracted on APK update). App data lives beside share/,
			 * not under it. */
		...
		static string config_storage_dir()
		{
			...
			return GLib.Path.build_filename(GLib.Environment.get_user_config_dir(), "ollmchat");
		}

		static void ensure_directory(string path) throws GLib.Error
		{
			var dir = GLib.File.new_for_path(path);
			if (dir.query_file_type(GLib.FileQueryInfoFlags.NONE) == GLib.FileType.DIRECTORY) {
				return;
			}
			try {
				...
				if (dir.query_file_type(GLib.FileQueryInfoFlags.NONE) != GLib.FileType.DIRECTORY) {
					throw new GLib.IOError.EXISTS("Path exists but is not a directory: %s", path);
				}
			}
		}

		/**
		 * Ensures app data, history, config, and model-cache directories
		 * exist.
		 *
		 * Uses EXISTS-tolerant creation so History.Manager does not abort
		 * when a concurrent mkdir races (Manager uses FileUtils /
		 * query_file_type + make_directory).
		 *
		 * @param data_dir Application data directory ({@link data_dir})
		 */
		public static void ensure_app_data_directories(string data_dir) throws GLib.Error
		{
			ensure_directory(data_dir);
			ensure_directory(GLib.Path.build_filename(data_dir, "history"));
			ensure_directory(GLib.Path.build_filename(data_dir, "config"));
		 ...
		 *
		 * Config lives under XDG_CONFIG_HOME (external etc/ollmchat/), not
		 * under the GTK asset tree in share/, so it survives APK asset
		 * re-extraction.
		 *
		...
		{
			var to_save = config != null ? config : this.config;
			var target_dir = config_storage_dir();
			var target_path = GLib.Path.build_filename(target_dir, "config.2.json");
			OLLMchat.Settings.Config2.config_path = target_path;
			try {
				ensure_directory(target_dir);
			} catch (GLib.Error e) {
				GLib.warning("config dir: %s", e.message);
				return;
			}
			var root = Json.gobject_serialize(to_save);
			if (root == null) {
				GLib.warning("config serialize failed");
				return;
			}
			...
			try {
				...
				GLib.message("saved config to %s", target_path);
			} catch (GLib.Error e) {
		 ...
		 *
		 * Sets Config2.config_path under XDG_CONFIG_HOME/ollmchat/ then
		 * loads or returns empty Config2. Migrates from legacy
		 * {data_dir}/config/ or share/ollmchat/config/ paths when present.
		 * Does not use base_load_config() because that hardcodes
		 * ~/.config/ollmchat.
		 *
		...
		public OLLMchat.Settings.Config2 load_config()
		{
			...
			if (!GLib.FileUtils.test(target_path, GLib.FileTest.EXISTS)) {
				var legacy_paths = new string[] {
					GLib.Path.build_filename(this.data_dir, "config", "config.2.json"),
					GLib.Path.build_filename(
						GLib.Environment.get_user_data_dir(),
						"ollmchat", "config", "config.2.json")
				};
				foreach (var legacy_path in legacy_paths) {
					if (!GLib.FileUtils.test(legacy_path, GLib.FileTest.EXISTS)) {
						continue;
					}
					try {
						ensure_directory(target_dir);
						...
						GLib.message("migrated config from %s", legacy_path);
						break;
					} catch (GLib.Error e) {
						GLib.warning("config migration: %s", e.message);
					}
				}
			}
			try {
			...
			if (GLib.FileUtils.test(target_path, GLib.FileTest.EXISTS)) {
				...
				GLib.message("load_config path=%s connections=%u", target_path, loaded.connections.size);
				return loaded;
			}
			GLib.message("load_config path=%s connections=0 (no file)", target_path);
