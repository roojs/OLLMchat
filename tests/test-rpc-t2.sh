...
DELETE_PATH="$REGISTER_PATH"
mkdir -p "$RPC_PROJECT_PATH/src"
printf 'seed\n' >"$NEW_FILE_PATH"

# --- Wire round-trip (single script session) ---
run_t2_case "$SCRIPT_DIR/rpc/t2.script.in"

jq_resp_ok "T2.2 File.read (no error)" 5 "$RPC_LAST_OUT" '.error == null'
jq_resp_ok "T2.2 File.read (id > 0)" 5 "$RPC_LAST_OUT" '(.result.id // .result["id"]) > 0'
jq_resp_args_ok "T2.2 File.read (path)" 5 "$RPC_LAST_OUT" \
    --arg p "$HELLO_PATH" '.result.path == $p'

jq_resp_ok "T2.1 File.write (no error)" 6 "$RPC_LAST_OUT" '.error == null and .msg == "ok"'

jq_resp_ok "T2.3 File.changed.check (NO_CHANGE)" 7 "$RPC_LAST_OUT" \
    '.error == null and .msg == "0"'

if grep -q "no signal rpc_activate on File" "$RPC_LAST_ERR"; then
    test_pass "T2.7 File.activate (no server handler)"
else
    test_fail "T2.7 File.activate (no server handler)"
fi
if [ -z "$(rpc_resp_by_id 8 "$RPC_LAST_OUT")" ]; then
    test_pass "T2.7 File.activate (no wire response)"
else
    test_fail "T2.7 File.activate (no wire response)"
fi

file_content_ok "T2.2 File.read (disk content)" "$HELLO_PATH" "hello from rpc fixture"

# --- File.register (skip scan; file on disk only) ---
run_t2_case "$SCRIPT_DIR/rpc/t2-register.script.in"
jq_resp_ok "T2.5 File.register (no error)" 4 "$RPC_LAST_OUT" '.error == null'
jq_resp_ok "T2.5 File.register (id > 0)" 4 "$RPC_LAST_OUT" \
    '(.result.id // .result["id"]) > 0'
jq_resp_args_ok "T2.5 File.register (path)" 4 "$RPC_LAST_OUT" \
    --arg p "$REGISTER_PATH" '.result.path == $p'
sqlite_count_ok "T2.5 File.register (sqlite row)" "$RPC_DB" \
    "SELECT COUNT(*) FROM filebase WHERE delete_id = 0 AND path = '$REGISTER_PATH';" \
    "1"

# --- changed.check with external touch ---
touch "$HELLO_PATH"
run_t2_case "$SCRIPT_DIR/rpc/t2-changed-dirty.script.in"
jq_resp_ok "T2.4 File.changed.check (CHANGED_HAS_UNSAVED)" 5 "$RPC_LAST_OUT" \
    '.error == null and .msg == "1"'

# --- Write persistence (isolated data_dir; disk covered in main t2.script) ---
run_t2_case_isolated "$SCRIPT_DIR/rpc/t2-write-persist.script.in"
sqlite_count_ok "T2.1 File.write (sqlite row)" "$ISOLATED_DB" \
    "SELECT COUNT(*) FROM filebase WHERE delete_id = 0 AND path = '$ISOLATED_HELLO';" \
    "1"

# --- Delete (immediate wire ok; persistence deferred — fire-and-forget) ---
run_t2_case_isolated "$SCRIPT_DIR/rpc/t2-delete.script.in"
jq_resp_ok "T2.6 File.delete (no error)" 7 "$RPC_LAST_OUT" '.error == null and .msg == "ok"'
