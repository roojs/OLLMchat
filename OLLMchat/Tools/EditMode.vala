...
@param create {boolean} [optional] If true, create or overwrite the entire file. Code blocks should only have language tag (no line numbers). Default is false.""";
		 ...
		 */
		public override async string execute(OLLMchat.Call.Chat chat_call, Json.Object parameters)
		{
			// Read parameters
		 ...
		 */
		private void on_stream_chunk(OLLMchat.Response.Chat response)
		{
		...
		/**
		 * Resets the internal state of the EditMode tool.
		 */
		 ...
		 */
		private void apply_all_changes(OLLMchat.Response.Chat response) throws Error
		{
			if (this.changes.size == 0) {
				return;
			}
