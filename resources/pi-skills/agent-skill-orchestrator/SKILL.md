...
Intelligent task planning and resource orchestration engine that analyzes user requirements and creates strategic execution plans using the best available resources. This skill matches against the skill catalog already in the system prompt, scores options, and generates comprehensive plans with alternatives.

The orchestrator operates as a planning assistant—it recommends approaches but always requests explicit user approval before execution.
...
**OLLMchat Agent Pi** — skill catalog in the system prompt + Agent Pi tools.
...
```
[██░░░░░░░░░░░░░░░░░░] 15% — Step 0: Using Catalog Already in Context
...
### Step 0: Use the Skill Catalog Already in Context

**Objective:** Inventory skills available for matching.

Agent Pi injects an `<available_skills>` catalog in the system prompt (`{skills_md}`).

**Actions:**

1. Treat the skills listed in that catalog as the inventory (name + description + location).
2. Optionally `read` a skill’s `SKILL.md` from its catalog `location` when you need more detail for scoring.
3. Match plans to Agent Pi tools (`read` / `write` / `edit` / `bash` / `browser`, and others registered for the agent).

**Why This Step is Critical:**

- Uses the host catalog already in context
- Keeps recommendations limited to offered skills
...
**Ask the user in chat.** Offer clear choices, for example:
...
- ✅ Assess prompt quality before planning (Step -1)
- ✅ Call prompt-engineer if the request is too vague for matching
- ✅ Start from the system-prompt skill catalog (Step 0) on every invocation
...
- ✅ Request explicit approval in chat before execution (Step 5)
- ✅ Use Agent Pi tool names (`read` / `write` / `edit` / `bash` / `browser`, …)
...
- ✅ Stay within the offered catalog; tell the user if it is empty

### **ORCHESTRATION PRINCIPLES:**

- **Catalog first:** Plan from the skills already listed in context
