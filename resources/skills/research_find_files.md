...
- Use **markdown file links** `[label](/absolute/path/to/file)` for paths that appear in the output; add `#anchor` only when the output gives a stable line or you can point to a known section. Do **not** paste long command logs — summarize and link.
...
Do **not** add extra body sections or fenced deliverables unless **What is needed** explicitly asks for them. Default: **## Result summary** only.
