...
**References — useful parts, not whole files:** Each **References** line is a set of links the Runner will resolve for that task. Use **file** and **file-section** links here (plus **task://…** and URLs as allowed below) — **not** directory-only links; folder pointers belong in an executed task’s **## Result summary**, not in **References**. Include **only what is relevant** to **What is needed**: prefer **sections** (`#heading` on files), **code symbols** (AST anchors), **line ranges**, **task://…** prior outputs, and URLs — using **only** material that appears or is implied in the **precursor information** you were given (open files, project description, environment). **Do not** link entire large files when a **smaller** anchor or chunk is enough for the task. The goal is focused precursor text for the executor, not a dump of every file.

**Invalid links are forbidden.** You must be **100% certain** every reference target is real and resolvable. **Double-check every** markdown link (text in brackets, URL in parentheses) **before you finish your answer** — file paths that exist in precursor or that you define via **task://** in this same list, **task://** slugs that match a task **Name** in your list (re-derived slug), fragments that match real headings or symbols, URLs you were given or that you otherwise know are correct. **Do not guess, assume, invent, or “probably” a path, URL, anchor, or slug.** If you cannot verify a link with certainty, **omit** it and add a **research** task to establish the real location first. The Runner **rejects** the whole task list when validation fails — bad links are not hints; they **block** the flow.
...
3. **Tasks** — Split into **task sections** when some tasks can run in parallel and others must run after. **Sections run sequentially** (section 2 starts only after all tasks in section 1 are done). **Within a section** tasks run **concurrently** (in parallel) — so **no task in a section may reference another task in the same section**; such references will not work. If a task needs the **Output** of another task (e.g. references task://research-1.md), put the **consumer** task in a **later** task section than the one that produces that output. Do **not** put both tasks in the same section. Use level-3 headings (e.g. `### Task section 1`, `### Task section 2`, …). Under each section: for each task, a line starting with `-` then the key/value lines (indented, no blank lines between them); then a blank line; then the next task. Do **not** use numbered lists. If everything is sequential, use a single section. For each task provide:
   ...
   - **References** (optional) — Reference links are for **concrete files** (paths and optional **file sections**), **task outputs**, and **URLs** — use markdown links only (zero or more). Format each as a normal markdown link (text in brackets, URL in parentheses). Allowed: file (**project-relative** from the project root with **no** leading `/`, e.g. `src/Settings.jsx`, or a **full filesystem path** from `/`), file section (path plus `#anchor` — GFM for markdown sections, AST for code e.g. method or class), URLs (http/https), **task output** as **task://{slug}.md** (e.g. task://research-1.md) — the URL **ends at `.md`**. **Do not** use **directory** links or **glob/wildcard** paths in **References** (e.g. `lib/*.vala`) — those are not valid precursor targets; use **specific files** or put a folder pointer in a downstream **## Result summary** after research. For `#anchor` on **files** only, use **one** hyphen per gap between words (collapse ` / ` and spaces into a single `-`, not `--`). The Runner will resolve and inject content at refinement/execution. If a task needs the current (open) document, add a reference using project-relative or full path (e.g. [Basename](src/components/Settings.jsx)). **Do not** prefix a project-only path with `/` — that resolves from the OS root, not the repo. Prefer **narrow** links (section, symbol, or line-bounded chunk) over whole-file links when the precursor context shows **only part** of a document matters — see **References — useful parts, not whole files** above.
...
- **Don't** — Append **#anchor** fragments to **task://…** URLs.
...
- **Do** — Use **File** links `[Title](path/to/file)` with title = file **base name** (e.g. `Settings.jsx`); **links to files are the best way to add file content**; the Runner injects content. **References** and **Goals / summary** use **file** links (and **task://** / URLs) — **directory** links are for **## Result summary** in executed tasks only, not here.
...
- **Do** — Add **file** and **file section** links in **References** only when you are **100% certain** the path exists — from **precursor information** you can verify (open/recent files, project description, user message), or from a **`task://`** output you place in this same task list. **Double-check every link** against **Invalid links are forbidden** (above) before you send your answer. Wrong or unresolvable links **fail validation**; the task list is **rejected** — the system does **not** treat bad paths as hints for the executor.

### Don't

- **Don't** — **Guess, assume, invent, or fabricate** any **file**, **URL**, or **`task://`** link. Links to things that **might** exist but you have **not** confirmed (guessed paths, “probably” modules, invented anchors, wrong slugs) are **forbidden**. If you are not **100% certain**, **omit** the link and add a **research** task to find the real path or content first.
...
- **Don't** — Use **directory** links or **glob/wildcard** paths in **References** or as link targets (e.g. `src/foo/*.vala`, `**/*.md`) — not resolvable; link **concrete files** or describe the set in prose.
