...
5. **Tasks** — Split into **task sections** as in the current list. **Sections run sequentially**; **within a section**, tasks may run **in parallel**. Use level-3 headings for each section (e.g. `### Task section 1`, `### Task section 2`, …). For each task provide:
   - **Name** (optional) — Short stable name (e.g. "Research 1") when another task will refer to this task's output; then later tasks use e.g. `[Research 1 Results](#research-1-results)`. Omit if no later task references this output.
...
*(Shared context for every task, if present; unchanged or lightly updated.)*
...
*(Omit new tasks when the goals are already complete. When rectifying issues, fix the tasks that lack output and keep the same structure.)*
