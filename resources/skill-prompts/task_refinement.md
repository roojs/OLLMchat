...
- **Don't** — Put **directory** paths in **Shared references** — **forbidden**. Links must target **files** (optional `#…` fragment), **`task://…`** outputs, or resolvable **URLs** — not folders (e.g. no `src/components/` as a link target). If the executor needs a whole area, link **concrete files** or rely on **## Tool Calls** / discovery; never use a directory as a reference. (**Directory** links are allowed **only** in a completed task’s **## Result summary**, not in refinement **Shared references**.)
- **Don't** — Use **glob or wildcard** link targets (e.g. `lib/*.vala`, `**/*.md`) — not resolvable.
