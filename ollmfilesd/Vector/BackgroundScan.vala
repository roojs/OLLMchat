...
namespace OLLMfilesd.Vector {
    ...
    public class BackgroundScan : GLib.Object {
        ...
        private OLLMvector2.Database vector_db;               // OLLMvector2.Database (FAISS) - thread-safe via mutex in Index class
        private SQ.Database sql_db;               // SQ.Database for metadata - thread-safe (SERIALIZED mode)
        private OLLMfilesd.GitProviderBase git_provider;     // Git provider instance (each thread needs its own instance for thread safety)
         ...
         */
        public BackgroundScan (OLLMvector.Tool.CodebaseSearchTool tool,
        ...
        private OLLMfilesd.ProjectManager? worker_project_manager = null;   // ProjectManager instance for background thread
        private OLLMfilesd.Folder? active_project = null;   // Track currently active project in background thread (for memory management)
        ...
        private void ensure_project_manager () 
		{
            if (this.worker_project_manager == null) {
                ...
                this.worker_project_manager = new OLLMfilesd.ProjectManager(this.sql_db);
         ...
         */
        private void set_active_project (OLLMfilesd.Folder? project) 
		{
         ...
         */
        private async void set_active_project_and_load (OLLMfilesd.Folder project) 
		{
        ...
        private async void startQueue () {
            ...
            while (true) {
                ...
                if (this.indexer == null) {
                    this.indexer = new Indexer (this.config, this.vector_db, this.sql_db, this.worker_project_manager);
