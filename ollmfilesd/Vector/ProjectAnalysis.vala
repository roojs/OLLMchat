...
{
	public class ProjectAnalysis : OLLMvector2.VectorBase
	{
		private SQ.Database sql_db;
		private OLLMfilesd.Folder root_folder;
		 ...
		 */
		public Gee.HashMap<int, OLLMvector2.SQT.VectorMetadata> metadata_keymap {
			 get; set; default = new Gee.HashMap<int, OLLMvector2.SQT.VectorMetadata>(); }
		...
		public ProjectAnalysis(OLLMchat.Settings.Config2 config, SQ.Database sql_db, OLLMfilesd.Folder root_folder)
		{
			base(config);
			this.sql_db = sql_db;
		...
		public async void analyze_dependencies() throws GLib.Error
		{
			...
			var build_rows = new Gee.ArrayList<OLLMvector2.SQT.VectorMetadata>();
			yield OLLMvector2.SQT.VectorMetadata.query(this.sql_db).select_async(
			...
			OLLMvector2.SQT.VectorMetadata.query(this.sql_db).select(
				...
				existing);
			var deps_meta = new OLLMvector2.SQT.VectorMetadata() {
		...
		private async Gee.HashMap<int, OLLMvector2.SQT.VectorMetadata> build_keymap() throws GLib.Error
		{
			if (this.metadata_keymap != null && this.metadata_keymap.size > 0) {
				return this.metadata_keymap;
			}
			var keymap = new Gee.HashMap<int, OLLMvector2.SQT.VectorMetadata>();
			string[] id_strs = {};
		...
		public async OLLMvector2.SQT.VectorMetadata analyze() throws GLib.Error
		{
			var keymap = yield this.build_keymap();
			if (keymap.size == 0) {
				return new OLLMvector2.SQT.VectorMetadata() {
			...
			OLLMvector2.SQT.VectorMetadata.query(this.sql_db).select(
				...
				+ " AND element_type = 'project' ORDER BY id DESC",
				existing);
			var project_meta = new OLLMvector2.SQT.VectorMetadata() {
				element_type = "project",
				element_name = GLib.Path.get_basename(this.root_folder.path),
				start_line = 0,
				end_line = 0,
