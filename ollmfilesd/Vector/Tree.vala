...
{
	/**
	 * Tree-sitter AST parsing and OLLMvector2.SQT.VectorMetadata creation.
	 * 
	 * Parses source code files using tree-sitter to extract code elements
	 * and create OLLMvector2.SQT.VectorMetadata objects with line numbers and documentation.
	 */
	public class Tree : OLLMfilesd.TreeBase
	{
		/**
		 * Array of OLLMvector2.SQT.VectorMetadata objects extracted from the AST.
		 */
		public Gee.ArrayList<OLLMvector2.SQT.VectorMetadata> elements { 
				get; private set; default = new Gee.ArrayList<OLLMvector2.SQT.VectorMetadata>(); }
		 ...
		 */
		public Gee.HashMap<string, OLLMvector2.SQT.VectorMetadata> cached_metadata { 
				get; private set; default = new Gee.HashMap<string, OLLMvector2.SQT.VectorMetadata>(); }
		
		/**
		 * Constructor.
		 * 
		 * @param file The OLLMfilesd.File to parse
		 */
		public Tree(OLLMfilesd.File file)
		{
			base(file);
		}
		...
		/**
		 * Extract code element information from AST node and create OLLMvector2.SQT.VectorMetadata.
		 * 
		 * @param node AST node
		 * @param code_content Source code content for text extraction
		 * @return OLLMvector2.SQT.VectorMetadata object, or null if node is not a code element
		 */
		private OLLMvector2.SQT.VectorMetadata? extract_element_metadata(TreeSitter.Node node, string code_content, string? parent_enum_name = null, string? namespace = null, string? parent_class = null)
		{
			...
			// Create OLLMvector2.SQT.VectorMetadata object early and assign values as we compute them
			var metadata = new OLLMvector2.SQT.VectorMetadata() {
		...
		public async void load_cached_metadata(SQ.Database sql_db) throws GLib.Error
		{
			...
			var results = new Gee.ArrayList<OLLMvector2.SQT.VectorMetadata>();
			yield OLLMvector2.SQT.VectorMetadata.query(sql_db).select_async("WHERE file_id = " + this.file.id.to_string(), results);
		 ...
		 * 
		 * @param metadata The OLLMvector2.SQT.VectorMetadata element to match
		 */
		private void match_element_with_cache(OLLMvector2.SQT.VectorMetadata metadata)
		{
		 ...
		 * @param metadata OLLMvector2.SQT.VectorMetadata object to set codedoc_start and codedoc_end on
		 */
		private void extract_documentation_lines(
			TreeSitter.Node node, OLLMvector2.SQT.VectorMetadata metadata)
		{
