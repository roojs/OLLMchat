...
namespace OLLMfilesd.Vector
{
	 ...
	 */
	public class FolderAnalysis : OLLMvector2.VectorBase
	{
		 ...
		 * metadata_keymap with the new folder OLLMvector2.SQT.VectorMetadata (store full object, do not copy description).
		 *
		 * @param folder The folder to process
		 * @param metadata_keymap Id-indexed map ((int)file_id → OLLMvector2.SQT.VectorMetadata); key type is int (cast from int64); updated when this folder is analyzed
		 * @return Number of folders analyzed in this subtree
		 */
		public async int analyze_children(
			OLLMfilesd.Folder folder, 
			Gee.HashMap<int, OLLMvector2.SQT.VectorMetadata> metadata_keymap) throws GLib.Error
		{
			int count = 0;
			...
			foreach (var child in folder.children.items) {
				if (child is OLLMfilesd.Folder) {
					count += yield this.analyze_children((OLLMfilesd.Folder)child, metadata_keymap);
					continue;
				}
				var fb = (OLLMfilesd.FileBase)child;
				if (fb.base_type == "fa" && fb.points_to is OLLMfilesd.Folder) {
					count += yield this.analyze_children((OLLMfilesd.Folder)fb.points_to, metadata_keymap);
				}
			}

			// Then analyze this folder and update keymap with full OLLMvector2.SQT.VectorMetadata (analyze returns non-null; empty description = skip)
		 ...
		 *
		 * Builds context from metadata_keymap: get OLLMvector2.SQT.VectorMetadata by (int)child.id/(int)file_id, use
		 * .description and .element_name from the stored object (do not copy out description only).
		 *
		 * @param folder The folder to analyze
		 * @param metadata_keymap Id-indexed map ((int)file_id → OLLMvector2.SQT.VectorMetadata); read for children/files, write for (int)folder.id when done
		 * @return OLLMvector2.SQT.VectorMetadata for this folder; on failure returns OLLMvector2.SQT.VectorMetadata with description = "" (never null)
		 */
		private async OLLMvector2.SQT.VectorMetadata analyze(
			OLLMfilesd.Folder folder, 
			Gee.HashMap<int, OLLMvector2.SQT.VectorMetadata> metadata_keymap) throws GLib.Error
		{
			...
			foreach (var child in folder.children.items) {
				...
				switch (fb.base_type) {
					...
					case "fa": {
						var alias_fb = (OLLMfilesd.FileBase)child;
						...
						var  fa_tag = (alias_fb.points_to is OLLMfilesd.Folder)
			...
			try {
				var tool_config = this.config.tools.get("codebase_search") as OLLMvector2.VectorToolConfig;
			...
			var existing = new Gee.ArrayList<OLLMvector2.SQT.VectorMetadata>();
			OLLMvector2.SQT.VectorMetadata.query(this.sql_db).select(
				"WHERE file_id = " + folder.id.to_string()
				+ " AND element_type = 'folder' ORDER BY id DESC",
				existing);
			var folder_metadata = new OLLMvector2.SQT.VectorMetadata() {
				element_type = "folder",
