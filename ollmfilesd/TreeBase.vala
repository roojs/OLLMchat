...
namespace OLLMfilesd
{
	...
	public abstract class TreeBase : Object
	{
		 ...
		 * 
		 * @param node_type_lower Lowercase node type string
		 * @param node AST node
		 * @param code_content Source code content for text extraction
		 * @param current_namespace Current namespace string (may be null)
		 * @return Updated namespace string, or current_namespace if node is not a namespace
		 */
		protected string? update_namespace_from_node(string node_type_lower, TreeSitter.Node node, string code_content, string? current_namespace)
		{
			if (node_type_lower == "namespace_declaration") {
				...
				if (namespace_name != null && namespace_name != "") {
					...
					if (current_namespace != null && current_namespace != "") {
						return "%s.%s".printf(current_namespace, namespace_name);
					} else {
		...
		/**
		 * Update parent class string if node is a class/struct/interface declaration.
		 * 
		 * @param node_type_lower Lowercase node type string
		 * @param node AST node
		 * @param code_content Source code content for text extraction
		 * @param parent_class_name Current parent class string (may be null)
		 * @return Updated parent class string, or parent_class_name if node is not a class/struct/interface
		 */
		protected string? update_parent_class_from_node(string node_type_lower, TreeSitter.Node node, string code_content, string? parent_class_name)
		{
			if (node_type_lower == "class_declaration" || node_type_lower == "class" ||
			    node_type_lower == "struct_declaration" || node_type_lower == "struct" ||
			    node_type_lower == "interface_declaration" || node_type_lower == "interface") {
				...
				if (class_name != null && class_name != "") {
					...
					if (parent_class_name != null && parent_class_name != "") {
						return "%s-%s".printf(parent_class_name, class_name);
					} else {
						return class_name;
					}
				}
		 ...
		 * @param node AST node
		 * @param code_content Source code content for text extraction
		 * @param parent_enum_name Current parent enum string (may be null)
		 * @return Updated parent enum string, or parent_enum_name if node is not an enum
		 */
		protected string? update_parent_enum_from_node(string node_type_lower, TreeSitter.Node node, string code_content, string? parent_enum_name)
		{
